...
Checked-in Shopify-fidelity fixture JSON must be a recorded real Shopify
interaction from a committed capture script. Do not hand-author, synthesize, or
regenerate Shopify fidelity fixture data from assumptions, local proxy output,
runtime tests, snapshot output, generated payloads, or edited examples. If a
capture cannot be run yet, leave the gap in Linear/workpad notes instead of
adding fixture JSON.

The forbidden anti-pattern is making non-Shopify data look like Shopify evidence.
Do not satisfy a request for parity, conformance, captured evidence, or “a
scenario that would have failed before this change” with any fixture, spec,
cassette, or expected payload whose source is the proxy itself, a runtime test
harness, a generator, an edited old response, or a guess. Naming or moving
that artifact under a different directory does not make it evidence. Use focused
runtime tests for proxy-only mechanics, and reserve the conformance corpus for
real Shopify captures or explicit legacy-evidence retirement.
...
6. Decide the evidence path before recording: every new fixture, recorder, or
   parity spec that claims Shopify fidelity must have a same-change consumer
   that runs as strict proxy parity against a real Shopify capture. A proxy-only
   regression belongs in runtime/unit tests, not in the conformance corpus under
   a new name.
7. Add or update exactly one captured, working parity spec for each real Shopify
   scenario under `config/parity-specs/`. `scenarioStatus: "captured"` means the
   referenced behavior came from Shopify, not from local proxy output.
...
- `scenarioStatus`: `captured`; new planned-only or blocked-only specs are not acceptable. For new specs, `captured` means the compared behavior was recorded from real Shopify, not generated by the proxy, a local harness, a snapshot, or an edited prior response.
- `assertionKinds`: what confidence the scenario builds, such as `payload-shape`, `user-errors-parity`, or `downstream-read-parity`; do not add assertion kinds that imply proxy-generated data is Shopify-fidelity evidence.
- `liveCaptureFiles`: fixture paths under `fixtures/conformance/<store>/<api-version>/...` for captured scenarios. New specs must not point at proxy-generated, snapshot, runtime-test, or hand-authored artifacts regardless of where those artifacts are stored.
...
`corepack pnpm parity -- <scenario-id>` runs the convention-driven Rust parity runner that discovers parity specs under `config/parity-specs/**` and exercises the captured request through the DraftProxy runtime with the recorded `upstreamCalls` cassette installed as the proxy's upstream transport. Adding or promoting a parity spec is enough to get CI coverage through the shared runner; do not write a new test that runs the same scenario explicitly. If you need richer per-scenario assertions (e.g. specific comparison target names), encode them in the parity spec itself; the runner validates them from the spec.
...
1. Runtime tests prove local staging/overlay behavior and proxy-only regressions.
2. Captured live fixtures settle Shopify payload shape, nullability, ordering, timestamps, and user errors.
3. Proxy request files make local replay deterministic against those live fixtures.
4. `conformance:check` runs the repo's Vitest structural checks for discovered scenarios.
5. `pnpm rust:test` runs the Rust route and meta suites, while `pnpm parity:run` is the parity entry point for recorded live scenarios. Parity specs install each capture's exact-GraphQL `upstreamCalls` cassette as the proxy's upstream transport and execute strict comparisons. Specs without a recorded live cassette are not ready; do not fill the gap with proxy-generated fixtures, descriptor cassettes, or other non-Shopify stand-ins.
