...
Successful upstream null reads also establish an instance-owned, API-version-scoped exact Node miss. Later generic `node` / `nodes` reads for that same ID may return null without another transport call, while cold IDs still hydrate upstream. `POST /__meta/reset` clears these observations.
...
The Rust operation registry maps operation names to capability metadata and is the local dispatch inventory used by runtime gates and tests. TypeScript tooling reads that same metadata through the Rust `operation-registry-json` exporter instead of a second checked-in JSON registry.

The `implemented` flag marks every canonical root the proxy answers locally instead of sending upstream. Anything without an implemented canonical registry entry passes through rather than erroring. `implemented` is therefore a routing fact, not a fidelity claim.
