...
- `createdAt` and `updatedAt` from the proxy mutation clock at create time
...
Definition limit handling uses Shopify's default entitlement caps where the proxy cannot know shop-specific overrides locally. Captured Admin GraphQL 2026-04 evidence in `fixtures/conformance/harry-test-heelo.myshopify.com/2026-04/metaobjects/metaobject-definition-limit-caps.json` confirms merchant-owned public creates are capped at 128 counted definitions: the live preflight saw six definitions, two standard-template definitions were excluded from the counted bucket, 124 disposable creates succeeded, and the next create returned `MAX_DEFINITIONS_EXCEEDED` with message `Total definition count exceeds the limit of 128`. Shop Pay's 256-definition override and custom entitlement plans are not modeled, so the proxy prefers conservative over-rejection to silently staging writes that would fail at commit. Definition create/update also rejects post-operation field sets with more than 40 field definitions. The same fixture confirms 40 `adminFilterable` field definitions are accepted, while 41 field definitions with `adminFilterable` enabled return both captured `INVALID` userErrors at `field: ["definition", "fieldDefinitions"]`: `Maximum 40 fields per metaobject definition` and `Maximum 40 admin filterable fields per metaobject definition`.

Update support stages scalar definition changes, access/capability merges, field definition create/update/delete operations, and `resetFieldOrder` ordering. Field definition updates preserve existing values for omitted fields; created fields append unless `resetFieldOrder` is set, in which case fields touched by the update input lead the resulting order and untouched fields follow in their previous relative order. Successful updates preserve `createdAt` and advance `updatedAt` from the proxy mutation clock.
...
Public Admin GraphQL 2026-04 rejects merchant creates of `shopify--form-*` definitions before field-count validation with the same `NOT_AUTHORIZED` reserved-type userError. The local public mutation model therefore uses the captured 40-field limit for mutable definitions rather than carrying an unverified 100-field `shopify--form-*` allowance.
...
Bulk delete support accepts the current 2026-04 `where.ids` and `where.type` branches, with the older local `ids` branch retained only for already-checked-in local replay evidence. It stages tombstones for found rows, ignores missing or stale explicit IDs, updates definition counts per type, and keeps all effects local. Empty `where.ids` and type selections that find a known definition with no rows return a no-work `Job` payload with no userErrors. Unknown `where.type` returns `RECORD_NOT_FOUND` on `where.type` with `job: null`. Supplying both `type` and `ids`, or neither selector, is modeled as Shopify's top-level `INVALID_FIELD_ARGUMENTS` validation error rather than a payload userError. `where.ids` selection is capped to the first 250 IDs with no truncation error; live-hybrid hydration for explicit IDs uses a single bounded `nodes(ids:)` request for those at-most 250 IDs instead of one upstream request per ID.

Type-scoped bulk delete first uses local definition and row state when available, so deleting locally created or already hydrated rows does not call upstream. For cold live-hybrid types, the proxy issues one bounded `MetaobjectBulkDeleteHydrateByType` catalog-and-definition read (`metaobjects(type:, first: 250)` plus `metaobjectDefinitionByType`) before staging tombstones or returning the unknown-type error; it does not page beyond that bound. Live 2026-04 introspection shows `where` is the required argument and direct `ids` is not accepted by Shopify.

Explicit-ID bulk delete hydrates cold live-hybrid rows with one `MetaobjectBulkDeleteHydrateByIds` `nodes(ids:)` request for the first 250 IDs in selector order. Captured 2026-04 behavior accepts stale IDs without per-index userErrors and still returns an async `Job`; the proxy mirrors that by ignoring missing IDs and tombstoning every found row locally. The 2026-04 fixture `fixtures/conformance/harry-test-heelo.myshopify.com/2026-04/metaobjects/metaobject-bulk-delete-ids-lifecycle.json`, replayed by `config/parity-specs/metaobjects/metaobject-bulk-delete-ids-lifecycle.json`, records the exact batched `nodes(ids:)` upstream call used for cold hydration and the settled downstream ID reads after the live job hides valid selected rows.

The companion 2026-04 survivor fixture `fixtures/conformance/harry-test-heelo.myshopify.com/2026-04/metaobjects/metaobject-bulk-delete-ids-survivor-lifecycle.json`, replayed by `config/parity-specs/metaobjects/metaobject-bulk-delete-ids-survivor-lifecycle.json`, records a disposable definition and three rows, deletes one explicit ID, and compares immediate deleted-row, survivor-row, type connection, and definition readback.
...
- Broader bulk delete selection semantics still need additional live conformance before widening beyond the local ids/type branches. Captured evidence covers the `where.type` branch and confirms Shopify returns an async job while immediate downstream reads already hide selected rows and report the definition's `metaobjectsCount` as zero. Explicit-ID evidence covers both a mixed existing/stale `where.ids` selector with exact batched cold hydration through `nodes(ids:)` and selected-row deletion with unselected survivor ID, connection, and definition readback. Edge-case evidence covers empty `where.ids`, unknown `where.type`, known-empty `where.type`, and invalid combined selectors.
