...
`config/parity-specs/metaobjects/metaobject-bulk-delete-type-lifecycle.json`, `config/parity-requests/metaobjects/metaobject-bulk-delete-type-*.graphql`, and a live 2026-04 fixture cover `metaobjectBulkDelete(where: { type })`. The recorder creates a disposable definition with two rows, captures the seeded read, bulk deletes by type, waits for downstream ID/catalog reads to hide the rows, and then cleans up the definition. The parity runner replays the captured public setup mutations before the local type-scoped bulk delete, compares mutation `userErrors` and downstream row/catalog reads strictly, and records Shopify's async job id/`done` timing plus delayed definition `metaobjectsCount` aggregate as the accepted payload volatility.
