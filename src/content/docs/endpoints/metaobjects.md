...
Bulk delete support accepts the current 2026-04 `where.ids` and `where.type` branches, with the older local `ids` branch retained only for already-checked-in local replay evidence. It stages tombstones for found rows, preserves ordered `elementIndex` `RECORD_NOT_FOUND` userErrors for missing IDs, updates definition counts per type, and keeps all effects local. Empty `where.ids` and type selections that find a known definition with no rows return a no-work `Job` payload with no userErrors. Unknown `where.type` returns `RECORD_NOT_FOUND` on `where.type` with `job: null`. Supplying both `type` and `ids`, or neither selector, is modeled as Shopify's top-level `INVALID_FIELD_ARGUMENTS` validation error rather than a payload userError. `where.ids` selection is capped to the first 250 IDs with no truncation error; live-hybrid hydration for explicit IDs uses a single bounded `nodes(ids:)` request for those at-most 250 IDs instead of one upstream request per ID, and missing/deleted rows are still reported at their original input index.

Type-scoped bulk delete first uses local definition and row state when available, so deleting locally created or already hydrated rows does not call upstream. For cold live-hybrid types, the proxy first asks for the definition by type, which is enough to answer unknown-type errors without reading the type catalog. Only when Shopify reports a known upstream definition and the proxy has no local rows for that type does it issue one bounded `MetaobjectBulkDeleteHydrateByType` catalog read (`metaobjects(type:, first: 250)`) before staging tombstones; it does not page beyond that bound. Live 2026-04 introspection shows `where` is the required argument and direct `ids` is not accepted by Shopify.
...
- Broader bulk delete selection semantics still need additional live conformance before widening beyond the local ids/type branches. Captured evidence covers the `where.type` branch and confirms Shopify returns an async job while immediate downstream reads already hide selected rows and report the definition's `metaobjectsCount` as zero. Explicit-ID evidence covers a mixed existing/stale `where.ids` selector and exact batched cold hydration through `nodes(ids:)`. Edge-case evidence covers empty `where.ids`, unknown `where.type`, known-empty `where.type`, and invalid combined selectors.
