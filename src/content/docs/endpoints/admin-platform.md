...
- `flowGenerateSignature` short-circuits proxy-local Flow trigger IDs, returns a deterministic 44-character standard-base64 local signature shaped like Shopify's HMAC-SHA256 output but not signed with Shopify's secret, stores only payload/signature SHA-256 hashes in meta state, and keeps the raw mutation in the mutation log for commit replay. Unknown Flow trigger IDs return Shopify's captured `RESOURCE_NOT_FOUND` top-level error.
