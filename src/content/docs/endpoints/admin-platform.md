...
- `node(id:)` and `nodes(ids:)` dispatch by GID type to an existing local detail handler or serializer. They preserve input order for `nodes(ids:)`; well-formed but absent, unsupported, or unknown-type GIDs return `null` in Snapshot mode, while malformed global IDs fail before execution with Shopify's top-level `Invalid global id '<value>'` coercion error envelope and no `data` payload. In LiveHybrid, legitimate registered types that have no local record model remain unresolved and use the caller's authoritative upstream response; exact local tombstones and request-cached authoritative misses still resolve to `null`. Generic Node dispatch does not create domain support by itself. Mixed operations that select `shop` plus generic `node` / `nodes` use the same central Node dispatcher for the Node roots, so store-property overlay reads do not have a separate, narrower Node implementation.
- Locally modeled generic Node families include records that already exist in normalized local state for products, product options and option values, product variants, catalog and inventory records (`InventoryItem`, `InventoryLevel`, `InventoryQuantity`, `InventoryAdjustmentGroup`, `InventoryTransfer`, `InventoryTransferLineItem`, `InventoryShipment`, and `InventoryShipmentLineItem`), metafields, selling plans, customers, customer mailing addresses, customer payment methods, store-credit accounts and credit/debit transaction records, B2B companies and selected nested records, app billing/access records, store/shop/location/business-entity records, files, saved searches, payment terms, bulk operations, metafield/metaobject definitions, orders/fulfillments/returns/draft orders, gift cards and credit/debit transaction records, delivery profiles and selected nested records, discount wrappers, marketing/events/webhooks/segments, markets and price lists, taxonomy categories, and supported online-store records. Cash-tracking sessions, point-of-sale devices, and Shopify Payments disputes are legitimate Node types with LiveHybrid read-through but no local lifecycle model; Snapshot returns `null` when they are absent from local state.
- In Snapshot mode, unsupported generic Node implementors and resource families without a local lifecycle/read model return Shopify-like `null` entries instead of partial fabricated objects. LiveHybrid preserves authoritative upstream values for legitimate registered Node types that are merely unmodeled locally.
