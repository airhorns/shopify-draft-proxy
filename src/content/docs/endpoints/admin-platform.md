...
- `node(id:)` and `nodes(ids:)` dispatch registered GID types to an existing local detail handler or serializer. They preserve input order for `nodes(ids:)`; well-formed but absent, unsupported, or unknown-type GIDs return `null` in Snapshot mode, while malformed global IDs fail before execution with Shopify's top-level `Invalid global id '<value>'` coercion error envelope and no `data` payload. In LiveHybrid, unregistered GID types remain unresolved and use the caller's authoritative upstream response; exact local tombstones and cached authoritative misses still resolve to `null`. Generic Node dispatch does not create domain support by itself. Mixed operations that select `shop` plus generic `node` / `nodes` use the same central Node dispatcher for the Node roots, so store-property overlay reads do not have a separate, narrower Node implementation.
- Locally modeled generic Node families include records that already exist in normalized local state for products, product options and option values, product variants, catalog and inventory records (`InventoryItem`, `InventoryLevel`, `InventoryQuantity`, `InventoryAdjustmentGroup`, `InventoryTransfer`, `InventoryTransferLineItem`, `InventoryShipment`, and `InventoryShipmentLineItem`), metafields, selling plans, customers, customer mailing addresses, customer payment methods, store-credit accounts and credit/debit transaction records, B2B companies and selected nested records, app billing/access records, store/shop/location/business-entity records, files, saved searches, payment terms, bulk operations, metafield/metaobject definitions, orders/fulfillments/returns/draft orders, gift cards and credit/debit transaction records, delivery profiles and selected nested records, discount wrappers, marketing/events/webhooks/segments, markets and price lists, taxonomy categories, and supported online-store records.
- In Snapshot mode, unsupported generic Node implementors and resource families without a local lifecycle/read model return Shopify-like `null` entries instead of partial fabricated objects. LiveHybrid preserves authoritative upstream values for unregistered Node types. Successful upstream nulls establish an exact API-version-scoped miss for the requested GID until reset.
...
- Generic Node dispatch remains unsupported for families without an owning local lifecycle/read model, including product taxonomy, product variant components, quantity price breaks, delivery profile item IDs, order delivery methods, B2B staff/catalog nested records, `CashTrackingSession`, `PointOfSaleDevice`, and `ShopifyPaymentsDispute`. Those three finance/POS types are not local resolver registrations; LiveHybrid read-through is generic unsupported-type forwarding, not lifecycle support. `StoreCreditAccountDebitRevertTransaction` is projected only when a normalized transaction row already exists; the current public local store-credit mutation slice does not create debit-revert rows.
