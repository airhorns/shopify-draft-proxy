...
Owner-metafield hydration tracks the observed parent shape separately from both
parent record presence and local metafield effects. A narrow Product,
ProductVariant, Collection, Customer, Order, or Company observation can resolve
the requested metafield without making unobserved parent fields authoritative.
Later singular or generic Node reads hydrate disjoint parent fields from
Shopify, merge them with the observation, and retain the local metafield
overlay. A metafield effect without an observed parent does not create an
identity-only parent, and an exact parent tombstone remains authoritative.
