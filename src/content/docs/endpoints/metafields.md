...
Normalized state stores definition records separately from metafield values. Definition records are owner-type scoped and can be staged for non-product owner types. Definition-scoped `metafields` and `metafieldsCount` read staged `metafieldsSet` values whose owner type, namespace, and key match the effective definition, while preserving any hydrated baseline `metafieldsCount` for real definitions. Staged definition lifecycle mutations update this same normalized catalog, so downstream definition reads use the effective staged definition state.
...
Create supports the normalized fields represented by `MetafieldDefinitionRecord`: identity (`ownerType`, `namespace`, `key`), `name`, `description`, `type`, `validations`, selected `access`, selected `capabilities`, optional `pin`, selected `constraints`, and `validationStatus: ALL_VALID`. The identity is owner-type scoped: a duplicate create for the same `(ownerType, namespace, key)` returns `createdDefinition: null` with field `["definition", "key"]`, code `TAKEN`, and Shopify's captured `Key is in use for Product metafields on the 'custom' namespace.` message shape, while the same namespace/key can coexist across distinct owner types. In LiveHybrid, create-time duplicate checks, owner-type resource limits, admin-filter capability limits, and pin-cap checks run against the effective catalog formed from the relevant hydrated upstream owner/namespace slice plus staged creates, updates, and tombstones; matching a real upstream namespace/key therefore returns `TAKEN` without staging a synthetic local definition. Product-owner creates reject Shopify-incompatible namespace/key lengths and characters, the literal business-reserved namespaces `shopify_standard` and `protected`, overlong `name` / `description`, unsupported custom-data type names, constrained `pin: true` inputs, owner-type pin-cap violations, and the captured 256 non-standard-definition resource-type cap before staging any local definition. Definition `validations[]` are checked against the resolved metafield type on create and update: unsupported option names return field `["definition", "validations"]`, code `INVALID_OPTION`, and the captured Shopify message naming the invalid option/type, while numeric `min` / `max` options reject invalid integer or decimal values before staging. The 2026-04 resource-limit fixture preflighted 54 PRODUCT definitions, excluded two `shopify` standard definitions from the counted merchant bucket, accepted 204 disposable creates, then returned `RESOURCE_TYPE_LIMIT_EXCEEDED` with message `Stores can only have 256 definitions for each store resource.`; a second namespace was also rejected at that boundary. The current 2025-01 conformance app accepts `shopify_standard` and `protected` namespace creates and the recorder preserves those exact live cases in the fixture, but strict parity excludes those two branches because the proxy intentionally keeps the local `RESERVED` guard as runtime-test-backed business logic. The resource-type cap is scoped separately for merchant-owned namespaces and app-reserved `app--<api_client_id>--...` namespaces, matching Shopify's app-owned versus merchant-owned count split.

Update resolves the existing definition by immutable identity (`ownerType`, `namespace`, `key`). It preserves `type`, `ownerType`, `namespace`, and `key`, and locally updates `name`, `description`, `validations`, selected `access`, selected `capabilities`, and selected constraint inputs. Present `name` and `description` inputs longer than 255 characters are rejected before staging with `updatedDefinition: null`, `validationJob: null`, and field-specific `TOO_LONG` userErrors; omitted fields are not revalidated, and 255-character boundary values are accepted and applied. Public Admin 2026-04 exposes `constraintsUpdates`, which can set the constraint key, create/delete values, and clear all constraints with `key: null, values: []`; `key: <string>, values: []` is rejected with field `["definition"]`, code `INVALID_INPUT`, and message `Cannot change the constraint key without providing values.`. The proxy also handles the legacy/internal `constraints` mixed-operation shape and `constraintsSet` replace-all shape for staged update fidelity, applying the same keyed empty-values guard to `constraintsSet` even though public Admin 2026-04 rejects `constraintsSet` at GraphQL validation before resolver execution. When a product-owned validation update changes validations and matching product metafields exist, the update payload returns a synthetic `validationJob` with a Shopify-like `Job` GID, `done: false`, and `query: null`; downstream `job(id:)` and local `node(id:)` resolve the staged job with the same pending shape. Validation-unchanged updates continue to return `validationJob: null`. Live 2026-04 introspection exposes `validationJob` on `MetafieldDefinitionUpdatePayload`, but not on `MetafieldDefinitionCreatePayload` or `MetafieldDefinition`.
...
- `fixtures/conformance/harry-test-heelo.myshopify.com/2026-04/metafields/metafield-definition-resource-type-limit.json`
...
- `corepack pnpm conformance:capture -- --run metafield-definition-update-name-description-length`
...
- Product, product variant, collection, customer, order, and company have fixture-backed definition/value success paths. Location, Page, Article, and Market additionally have mixed-owner CAS, delete, and public-root read-after-write evidence. Other accepted `HasMetafields` mutation owner families use authoritative targeted hydration, but that does not imply full owner-domain lifecycle support beyond their separately documented public read surfaces.
...
- CAS/userError coverage includes product owners and mixed Location, Page, Article, and Market owners. Other accepted owner families do not have equivalent owner-specific CAS evidence and should not be assumed to expose identical validation branches beyond the shared hydrated mutation contract.
