...
- `standardMetafieldDefinitionTemplates` remains registry-only declaration coverage; `standardMetafieldDefinitionEnable` consumes the checked-in captured catalog, but the catalog query root itself should not be treated as locally supported until it has executable read behavior and fixture-backed shape evidence.
