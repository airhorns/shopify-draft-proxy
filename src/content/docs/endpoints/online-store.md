...
The content lifecycle parity evidence captures a Shopify quirk where top-level `articles` omitted a locally updated unpublished article while the same article still appeared through `Blog.articles`, `Article.blog`, `articleAuthors`, and `articleTags`. The proxy now mirrors that boundary: top-level `articles` filters out records with `isPublished: false`, while nested blog/article helper reads continue to expose the effective local graph.
...
The normalized local integration graph covers themes, script tags, web pixels, server pixels, storefront access tokens, and mobile platform applications. These roots are local-only at runtime:
...
- Current content evidence: `fixtures/conformance/harry-test-heelo.myshopify.com/2025-01/online-store/online-store-content-lifecycle.json` captures content catalog/detail/empty reads, blog/page/article lifecycle success paths with downstream reads, and unknown-id comment moderation/delete userErrors. `config/parity-specs/online-store/online-store-content-lifecycle.json` and `config/parity-requests/online-store/online-store-content-*.graphql` replay local create/update/read/delete/comment guardrail requests and strictly compare stable payload/count/null-empty/userErrors fields against the recording. The same parity spec also checks generic `node(id:)` readback for staged blog, page, and article content after local updates.
- Comment moderation not-found-code evidence: `fixtures/conformance/harry-test-heelo.myshopify.com/2025-01/online-store/comment-moderation-not-found-codes.json` captures `commentApprove`, `commentSpam`, `commentNotSpam`, and `commentDelete` with an unknown comment ID and selected `userErrors { field message code }`. Shopify returns `code: "NOT_FOUND"` with `field: ["id"]` and `message: "Comment does not exist"` for all four roots; `config/parity-specs/online-store/comment-moderation-not-found-codes.json` replays that shape through the generic parity runner with no upstream mutations or staged writes.
- `fixtures/conformance/harry-test-heelo.myshopify.com/2025-01/online-store/online-store-content-search-filters.json` plus `config/parity-specs/online-store/online-store-content-search-filters.json` prove article tag/author, blog title, and page published-status/title filters against live Shopify and replay them through `corepack pnpm conformance:parity`. Runtime coverage also covers Shopify-style boolean/fielded content search and comment approval `publishedAt` materialization.
- `fixtures/conformance/harry-test-heelo.myshopify.com/2025-01/online-store/online-store-article-media-navigation-follow-through.json` records live article image/metafield create/update/read behavior, confirms the Page `onlineStoreUrl` and Article `seo` schema boundaries, and captures page-to-menu follow-through behavior for future navigation modeling. Focused runtime coverage covers the supported article image/metafield behavior; menu roots remain unimplemented.
- Focused Rust runtime coverage owns the local-only page default-publication branch plus successful comment approve/spam/not-spam/delete state transitions, downstream nested article/comment reads, and no-upstream-fetch guarantees for locally staged moderation. These local-only branches are no longer represented as Shopify parity evidence.
- `online-store-content-search-filters` uses LiveHybrid Pattern 1 passthrough for cold `articles` / `blogs` / `pages` search reads, backed by a cassette from the captured search response. `online-store-content-lifecycle` keeps staged create/update/read/delete flows local; the runtime hydrates recorded `blogsCount` / `pagesCount` baselines from the capture's narrow count hydrate calls, then adds newly staged local content so downstream count reads match Shopify without forwarding supported mutations.
- `online-store-content-required-fields` captures missing-title schema errors and blank-title model `userErrors` for `pageCreate`, `articleCreate`, and `blogCreate`; the executable parity targets replay the blank-title userError branches, while focused runtime tests cover the proxy's omitted-title normalization and no-staging behavior.
- Article blog-existence evidence: `fixtures/conformance/harry-test-heelo.myshopify.com/2025-01/online-store/article-create-update-blog-not-found.json` captures `articleCreate` and `articleUpdate` with `blogId: "gid://shopify/Blog/999999999999"`. Shopify returns `NOT_FOUND` on `field: ["article"]` with `message: "Must reference an existing blog."` for both branches, and the captured read-after-failed-update proves the original article title and blog remain unchanged. `config/parity-specs/online-store/article-create-update-blog-not-found.json` replays the setup, validation branches, and readback through the generic parity runner.
- Length-validation evidence: `fixtures/conformance/harry-test-heelo.myshopify.com/2025-01/online-store/article-page-blog-length-validations.json` captures article/blog/page title, handle, and body limit branches for create/update mutations, plus public schema evidence that `feedburner` is not accepted on current `BlogCreateInput` / `BlogUpdateInput`. `config/parity-specs/online-store/article-page-blog-length-validations.json` replays the exposed validation branches through the generic parity runner, and focused runtime tests cover no-staging behavior for failed validations.
- Salvaged parity evidence: `fixtures/conformance/harry-test-heelo.myshopify.com/2026-04/online-store/online-store-salvaged-parity-recordings.json` records live Shopify responses for the old online-store parity group by sending the checked-in request documents with exact variables. The fixture includes successful disposable page/blog branches, schema-validation branches, and current app-context/scope blockers for script tags, web/server pixels, mobile platform applications, storefront token writes, and write-themes resolver branches. The restored specs are `captured-fixture` scenarios backed by `tests/graphql_routes/marketing_inventory_online_store.rs`, not strict Shopify parity comparisons for local-only lifecycle behavior.
- Live evidence: `corepack pnpm conformance:probe` succeeds against `harry-test-heelo.myshopify.com`; 2026-04 salvage capture confirmed the current app has content and theme read access while `scriptTags`, `webPixel`, `serverPixel`, `mobilePlatformApplications`, `storefrontAccessTokenCreate`, and theme write roots return `ACCESS_DENIED` / missing-scope responses. Runtime support for blocked-scope families is based on schema introspection plus local executable tests, with the scope blocker recorded in the live salvage fixture rather than synthetic parity fixtures.
- Shopify docs/examples and public Admin GraphQL examples for themes/theme files, script tags, web/server pixels, mobile platform applications, and storefront access tokens confirm the risky paths are externally visible effects: real theme asset writes and publish state, storefront script loading, browser/customer-event pixel activation, EventBridge/Pub/Sub delivery, Storefront API credential issuance, and mobile app verification settings. The proxy deliberately stages inert local records for these effects and does not emulate the external delivery/activation systems.
- Rust integration tests cover token uniqueness/redaction/scope filtering/limits, theme role transitions and pagination, theme-file checksum/body-mode/copy/delete validation, ScriptTag create/update/delete/root-dispatch behavior, supported delete/not-found guardrails, WebPixel duplicate and settings validation, ServerPixel endpoint validation, and MobilePlatformApplication lifecycle, pagination, and create/update model validation for branches that the live salvage fixture records as scope/app-context blocked rather than successful Shopify writes.
- Current presentation/integration schema evidence: `fixtures/conformance/very-big-test-store.myshopify.com/2025-01/admin-platform/admin-graphql-root-operation-introspection.json` proves the root names exist in the captured Admin GraphQL schema. Live 2026-04 introspection confirmed payload, input, union, and theme-file result fields used by the local model.
...
- Parity-spec boundary: do not add planned-only parity specs for these gaps. Online-store specs must not use `fixtures/conformance/local-runtime/...` as capture evidence; strict parity claims still need executable comparison targets, while captured-fixture scenarios must pair live Shopify recordings with runtime tests for the local behavior they enforce.
