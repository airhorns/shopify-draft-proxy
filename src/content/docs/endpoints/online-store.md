...
- `Blog.metafield(namespace:, key:)` and `Blog.metafields(...)` expose hydrated BLOG-owned metafield state so narrow blog updates preserve and read back existing values
...
In live-hybrid mode, article and blog mutations hydrate the authoritative resource before staging whenever the local observation is missing or incomplete. `articleCreate` resolves an unobserved target blog through a read-only upstream query; `articleUpdate` hydrates the current article and any unobserved destination blog; and `blogUpdate` hydrates an unobserved or partial blog. These reads select the scalar, author, image, publication, template, parent-blog, and owner-metafield state needed by the local mutation and follow metafield cursors across pages. The supported mutation document itself is never sent upstream during normal runtime handling.

Page and Blog handles are reserved across the effective shop, while Article handles are reserved within their effective parent Blog. Live-hybrid create and update mutations perform narrow query-only handle lookups when the relevant owner is not already known locally. Generated collisions advance a numeric suffix, including incrementing an existing trailing number; updates exclude their own record while still detecting another owner. Explicit Page and Article collisions return `TAKEN` on `["page" | "article", "handle"]` without applying the rejected write. Shopify treats Blogs differently: an explicit colliding `blogCreate` chooses the next suffix, while a colliding `blogUpdate` returns the unchanged Blog with no user error. Reserved handles are stored on the effective record used by detail roots, connections, handle filters, and generic Node reads.

Hydration preserves the distinction between a field that was not selected and a field Shopify selected as `null`, an empty string, or an empty list. A narrow article update therefore retains its existing body, summary, tags, author, publication state, image, template suffix, blog membership, and ARTICLE-owned metafields; a narrow blog update retains its existing comment policy, template suffix, and BLOG-owned metafields. Live whole-resource evidence for these mutation-first and narrow-update branches is recorded in `fixtures/conformance/harry-test-heelo.myshopify.com/2025-01/online-store/online-store-authoritative-content-hydration.json`.

Article creates and updates advance the effective parent blog's `updatedAt`; moving an article advances the destination blog's `updatedAt`. Effective `Blog.tags` is the sorted union of tags from its known child articles, so locally staged article tag changes are visible through later blog reads.
