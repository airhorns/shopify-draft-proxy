...
- Live evidence: `corepack pnpm conformance:probe` succeeds against `harry-test-heelo.myshopify.com`; a 2026-06-30 probe confirmed the current app has content and theme read access while `scriptTags`, `webPixel`, `serverPixel`, `mobilePlatformApplications`, and `storefrontAccessTokenCreate` return `ACCESS_DENIED` / missing-scope responses. Runtime support for blocked-scope families is based on schema introspection plus local executable tests, with the scope blocker recorded here rather than checked-in local-runtime parity specs.
...
- The retired local-runtime online-store parity specs for storefront tokens, theme publish and file mutations, script tags, web/server pixels, mobile platform applications, integration delete/not-found guards, and integration root dispatch are not Shopify parity evidence. Rust integration tests cover token uniqueness/redaction/scope filtering/limits, theme role transitions and pagination, theme-file checksum/body-mode/copy/delete validation, ScriptTag create/update/delete/root-dispatch behavior, supported delete/not-found guardrails, WebPixel duplicate and settings validation, ServerPixel endpoint validation, and MobilePlatformApplication lifecycle, pagination, and create/update model validation.
...
- Parity-spec boundary: do not add planned-only parity specs for these gaps. Add `config/parity-specs` entries only when backed by captured Shopify interactions and executable comparison; online-store specs must not use `fixtures/conformance/local-runtime/...` as capture evidence.
