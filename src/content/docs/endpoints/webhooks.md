...
- Cold LiveHybrid reads forward authoritative Shopify webhook subscription responses verbatim when there is no staged overlay. Successful upstream detail/list responses hydrate normalized base state; once a local create, update, or delete is staged, subsequent detail/list/count reads recompute from effective base plus staged records and tombstones.
...
- `webhookSubscriptionUpdate` updates an existing staged or hydrated subscription in place, preserving `topic` and `createdAt` while replacing supported mutable fields. In LiveHybrid mode, a cold existing ID is resolved through a read-only `webhookSubscription(id:)` hydration query before local staging; the caller mutation is not sent upstream.
- `webhookSubscriptionDelete` records local deletion state. In LiveHybrid mode, a cold existing ID is resolved through the same read-only hydration path before tombstoning. Downstream detail reads return `null`, and list/count reads omit deleted subscriptions.
