...
Generic `node(id:)` / `nodes(ids:)` resolves staged `PaymentTerms` and `PaymentSchedule` records from the effective payment-terms store. `PaymentTerms` Node reads apply the same clock-sensitive due/overdue recomputation used by mutation payloads and downstream order/draft-order owner reads; `PaymentSchedule` Node reads include the derived parent `paymentTerms` context used by local payment-reminder eligibility checks. Stored `PaymentMandate` records can be projected when they exist under a local customer payment-method mandate connection, but `orderCreateMandatePayment.mandateId` is only an input reference and does not materialize a standalone mandate record; those caller-provided mandate IDs continue to resolve as `null` through generic Node reads unless a real mandate record has been observed.
