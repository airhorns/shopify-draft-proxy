...
- Refill queries select the activity identity and app/event fields needed by the local filter and sort adapters even when the caller omits them; the caller still receives only its requested projection. Snapshot reads remain confined to normalized local state and never run refill queries.
...
- `sourceAndMedium` is derived from tactic and referring-domain inputs rather than from fixed channel/tactic tuples. Captured special tactics keep Shopify labels such as `Abandoned cart email`, `Affiliate link`, `Loyalty program`, `Facebook retargeting ad`, and `Message via Facebook Messenger`; ad/fallback labels use the aliased referring domain when present or `{channel titleized} {tactic humanized}` when absent.
