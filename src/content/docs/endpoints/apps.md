...
- `appRevokeAccessScopes` removes locally granted optional scopes from the current app installation. Unknown-scope and required-scope validation use the stored current installation grant and requested/required scope metadata rather than literal scope handles. Live 2026-04 validation evidence covers unknown handles, required-scope rejection, and mixed unknown-plus-required input: failed validation returns `revoked: null`, and unknown handles take precedence over the required-scope guard. Optional-grant success, non-granted, and implied-scope branches remain runtime-test-backed because recording optional success would revoke a real app grant from the active conformance app.
