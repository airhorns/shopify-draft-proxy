...
- `currentAppInstallation`, `appInstallation(id:)`, `app(id:)`, `appByHandle(handle:)`, and `appByKey(apiKey:)` project the local app model after staged app mutations or LiveHybrid hydration from an upstream app installation read. App ID reads resolve the proxy's default synthetic app identity and request-owned app IDs supplied through `x-shopify-draft-proxy-api-client-id`.
...
- `appRevokeAccessScopes` removes locally granted optional scopes from the current app installation. Granted scopes come from `x-shopify-draft-proxy-access-scopes`; required scopes come from `x-shopify-draft-proxy-required-access-scopes` and default to `read_products` when omitted. Live 2026-04 validation evidence covers unknown handles, required-scope rejection, and mixed unknown-plus-required input: failed validation returns `revoked: null`, and unknown handles take precedence over the required-scope guard. Optional-grant success, non-granted, implied-scope, and missing-source-app branches remain runtime-test-backed; local missing-source-app coverage is modeled through request-owned source-app context rather than the GraphQL operation name.
...
- `delegateAccessTokenCreate` accepts the current `delegateAccessScope` list input, validates requested handles against the granted scopes on the request, returns validated scope handles through payload `accessScopes`, and stores only token hash, redacted preview, owning app ID, and parent-token hash in meta-visible state.
