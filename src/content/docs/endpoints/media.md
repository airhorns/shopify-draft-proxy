...
- `fileSavedSearches` returns an empty Shopify-like connection in snapshot
  ...
  search query before applying local filters. When a captured LiveHybrid read
  observes the real Shopify row for the same staged FILE saved-search flow, the
  connection de-duplicates by resource type, name, and normalized query so the
  real row and synthetic row do not both appear. Unknown saved-search ids match
  no staged files rather than returning the full file set.
