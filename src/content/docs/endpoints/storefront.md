...
Cart lines reference visible variants from the shared normalized product catalog. Prices, compare-at prices, tracked inventory, availability, variant titles, and total costs derive from that shared state. Currency resolves once for the cart from its buyer country/market context, line variant pricing, or observed Storefront/Admin shop state, so line and cart money stay coherent. When none of those sources establishes a currency, money values remain unavailable and the Storefront schema applies its normal null propagation instead of receiving a guessed currency. Identical merchandise, selling-plan, and attribute combinations merge; distinct attributes keep separate lines. Tracked inventory caps quantities across matching variant lines, returns captured not-enough-stock warnings, and retains a zero-quantity line plus out-of-stock warning when a distinct line cannot receive inventory. Zero-quantity line-add input is a no-op, while line update quantity zero removes the line.

Cart and line attributes preserve first-key order with the last duplicate value winning. Mutations enforce the captured 250-element line/attribute limits, the 5,000-character note limit, invalid merchandise and selling-plan branches, missing carts, and stale lines. Cart mutation payloads return captured `CartUserError` and `CartWarning` shapes. Line and cart totals use the shared money helpers and include the selected fixed delivery rate after discounts; duty and tax amounts remain `null` and estimated flags remain true because checkout does not execute locally.

`cartDeliveryAddressesAdd`, `cartDeliveryAddressesUpdate`, `cartDeliveryAddressesRemove`, and `cartDeliveryAddressesReplace` keep ordered cart-owned selectable addresses. Inline addresses trim blank fields and normalize country/province codes; customer-address copies are accepted only when the cart buyer owns the shared customer address. Country is always required. `STRICT` validation derives required last-name, address, zone, postal-code, and city fields from the captured country requirements, preserves Shopify's multi-error order, and normalizes known subdivision codes. Countries for which Shopify does not require a postal code accept an omitted postal code; countries with required subdivisions reject missing or invalid modeled zones, including postal-driven Australian zone normalization. Non-`STRICT` inputs retain the less restrictive validation path. Foreign or missing selectable-address IDs are rejected without partial mutation, the 250-input limit uses Shopify's top-level error shape, `oneTimeUse` and selected state round-trip, and replace allocates a new ordered address set. Changing or removing the selected delivery context invalidates explicit option selections.

Delivery groups are computed from the selected address, shippable cart lines, tracked availability, and effective Admin delivery-profile state. Lines partition by delivery profile and one-time/subscription purchase type. A group appears only when its profile has an active online-fulfilling location, a matching country or rest-of-world zone, and an active fixed rate whose currency and subtotal conditions match the cart. Option titles, descriptions, costs, and opaque handles derive from those profile/rate facts; the lowest-cost eligible option is selected until `cartSelectedDeliveryOptionsUpdate` selects another valid group handle. Invalid or stale handles return `INVALID_DELIVERY_OPTION`, address/profile changes re-evaluate selection, and selected delivery cost participates in cart totals. When a selected-address mutation leaves a shippable line without an eligible option, captured line reconciliation and warning behavior applies instead of inventing a location, carrier, or rate.

Cart delivery addresses, explicit selections, warning state, and synthetic address counters survive dump/restore and clear on reset. Subsequent reads use current shared product, inventory, location, market/pricing, and delivery-profile state. Local Admin delivery-profile rate changes therefore re-evaluate options and totals; immediate propagation of the equivalent live Shopify profile update was not observed during bounded capture, so that particular context-change branch is runtime-regression coverage rather than claimed live parity.

`checkoutUrl` is deterministic for a local cart: it combines the explicitly configured Shopify origin with the cart's opaque synthetic token and key. Producing the URL does not contact Shopify checkout or embed a live cart secret.
...
Storefront customer orders preserve authoritative identity, contact, order-number, money, status, and processed-time values from shared order state. `orderNumber` is stored independently of the caller-visible order `name`, including for custom names containing unrelated digits. Current subtotal and total money sets take precedence over original totals, and later locally staged Admin order changes overlay only the fields they update, leaving the other known Storefront projection values intact. When a required Storefront Order field is absent from a partial Admin record, the projector leaves it unknown so the Storefront schema produces its normal execution error and null propagation; it does not invent zero-valued money, a currency, an order number, a fulfillment state, or an epoch timestamp.
...
`node(id:)` and `nodes(ids:)` parse the Shopify GID resource type and dispatch through the same Storefront-visible serializers as the dedicated roots. The supported local Node types are `Product`, `ProductVariant`, `Collection`, `Article`, `Blog`, `Page`, `Metaobject`, `Location`, and `Menu`. In live-hybrid mode, each ID is resolved independently: an exact staged/base record or tombstone wins, while an unresolved ID reuses one query-only upstream execution of the complete caller operation and then receives the local overlay. Unrelated local records never make another ID authoritative. `nodes(ids:)` preserves input order, duplicates, and null slots without duplicating the upstream request, and Menu-by-ID follows the same effective lookup path. Snapshot mode never hydrates; absent IDs are locally authoritative `null`. Deleted, unpublished, inaccessible, or authoritatively missing IDs also return `null`; malformed global IDs return Shopify's captured coercion error. Aliases, named and inline fragments, the `Node` interface, and concrete type fragments remain under the Storefront executable schema.
...
Snapshot discovery reads use local snapshot plus staged state and never hydrate upstream. Live-hybrid `search` and `predictiveSearch` stay passthrough when no supported discovery state is locally known; once shared supported-domain state exists, those roots resolve locally so staged Admin additions, updates, publication changes, unpublishes, and tombstones are immediately reflected without runtime Shopify writes. Generic `node` and `nodes` use the per-ID hydration and overlay rules above instead of treating that domain-wide discovery state as catalog completeness.
...
Contextual money first resolves a hydrated Storefront market by observed ID or handle, or an active staged market whose region matches the request country, then follows the active catalog to its attached price list. A fixed variant price and compare-at price override the base product price in that context. Currency precedence is hydrated localization, attached price list, matched market base currency, variant-observed currency, then observed Storefront/Admin shop currency. The authenticated catalog capture observes CAD in default context and EUR for a DE market/catalog/price-list context. Without an authoritative currency, money remains null and any selected non-null Storefront money field follows the executable schema's error/null boundary rather than inventing USD or another market conversion. The captured preferred-location context leaves price and empty store availability unchanged. Quantity rules remain the captured default `minimum: 1`, `maximum: null`, `increment: 1` for this market-catalog surface, with empty quantity-price-break and store-availability connections.
...
Carrier-service callbacks and carrier-calculated rate execution remain outside the local delivery model. The proxy does not contact a carrier service to synthesize an option; only explicit fixed-rate profile facts are eligible. Shipping packages, weight-based conditions, and delivery-method types beyond the captured fixed shipping-rate branch are not locally calculated.

Navigating a local cart's `checkoutUrl`, cart preparation/submission, tax/duty finalization, payment authorization/capture, gift-card debit, discount usage consumption, order creation, one-time-address consumption at checkout, and cart-to-order metafield copying remain unsupported. The local cart computes estimates only; it does not claim that checkout navigation or payment succeeded, reserve a discount redemption, debit a gift card, or materialize an order.
...
Generic Storefront Node dispatch does not locally model the remaining Storefront `Node` implementors; unresolved live-hybrid IDs can still use the caller's upstream query, while snapshot mode returns `null`. Search does not return collections because the captured `SearchResultItem` interface contains only products, articles, and pages; collection results are supported through `predictiveSearch`. Query suggestions are a deterministic approximation, not a reconstruction of Shopify's private search corpus or request-scoped ranking session.
