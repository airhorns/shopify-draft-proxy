...
- Generic `node(id:)` and `nodes(ids:)` resolve `BulkOperation` through the same effective base-plus-staged job projection as the dedicated roots. Aliases, fragments, input order, duplicates, null slots, and mixed resource types remain GraphQL-engine behavior; a staged cancellation or terminal transition is therefore visible identically through dedicated and generic reads.
- Snapshot Node reads never call upstream and return `null` for BulkOperation IDs absent from base/staged state. LiveHybrid resolves effective jobs first, then forwards at most one caller Node document for a cold batch; raw upstream BulkOperation values are observed into base state before staged overlays are applied, so later dedicated reads share the same job without hiding local changes.
...
- Supported local JSONL synthesis roots are `products`, `productVariants`, `orders`, and `draftOrders`. Product exports support nested `collections`, `images`, `media`, `metafields`, and `variants`; product-variant exports support nested `media` and `metafields`. Nested rows include `__parentId`. Order and draft-order exports currently project root records, including ordinary nested objects and list fields such as `DraftOrder.warnings`; nested connection rows are rejected until their child lifecycle is modeled.
- Snapshot exports serialize the complete effective local catalog. LiveHybrid exports first hydrate the complete upstream catalog for the selected supported root, selecting identity and requested output fields; product and product-variant hydration also selects parent relations and fields required by the corresponding local search predicate. Top-level catalog pages and selected supported nested connection pages are fetched in 250-row windows until Shopify explicitly returns `hasNextPage: false`.
...
- Query JSONL synthesis is supported for `products`, `productVariants`, `orders`, and `draftOrders`.
- Supported exports require a provably complete baseline in LiveHybrid. Callers receive an `INVALID` userError rather than partial JSONL when that read-side hydration cannot complete.
