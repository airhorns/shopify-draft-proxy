...
- Admission performs validation, concurrency checks, synthetic identity allocation, and CREATED-job staging without issuing an upstream request or materializing JSONL. The CREATED record itself is the staged record; terminal metadata is not stored before the mutation response.
- A subsequent BulkOperation read advances at most one pending query export per GraphQL request. Snapshot completes that job from the effective local catalog without transport. LiveHybrid fetches a fixed slice of at most 16 250-row root catalog pages, records the last cursor, and returns `RUNNING` when more pages remain; a final page within any slice produces the terminal result.
- LiveHybrid page queries select only identity, requested output fields, parent relations, and fields required by the corresponding local search predicate. Selected nested connections are fetched in the same root-page request with a fixed 250-row bound. A nested connection that proves it has another page fails the job instead of issuing per-root-row follow-up requests or publishing partial JSONL.
...
- An upstream transport failure, GraphQL error, malformed connection, incomplete nested page, missing pagination proof, repeated cursor, or page-limit exhaustion transitions the admitted job to `FAILED` with `errorCode: INTERNAL_SERVER_ERROR`, zero counters, and no result metadata. No partial artifact is served.
- Successful execution writes generated JSONL only at completion, exposes a synthetic absolute `https://localhost:<proxy-port>/__meta/bulk-operations/<encoded-id>/result.jsonl` result URL, and never proxies the supported export mutation upstream at runtime.
- Immediate mutation responses return Shopify's created job shape with `status: CREATED`, `completedAt: null`, zero counters, no file/result URL, and the original query. Polling reads expose deterministic `RUNNING` and terminal projections. For query exports, `objectCount` counts every emitted JSONL object, while `rootObjectCount` counts only root-level objects, so product exports with nested variants can report a larger `objectCount` than `rootObjectCount`.
...
- Non-terminal jobs stage a status-only `CANCELING` overlay, preserving the stored job's counters, artifact fields, timestamps, query, and type, then return selected job payloads with empty `userErrors` and append the original raw mutation body plus the staged BulkOperation ID to the mutation log for commit replay and observability. For a pending local query export, cancel also prevents another execution page; the next read exposes `CANCELED` with no artifact.
- LiveHybrid can hydrate a cold known job before applying the same stored-status cancel decision locally.

Meta API behavior:

- `GET /__meta/state` exposes base/staged BulkOperation records, ordering, pending query execution cursors, and generated result artifacts. Dump/restore retains pending work, while reset discards the job, plan, and artifact state together.
...
- Product and product-variant exports require a provably complete baseline in LiveHybrid. Root catalogs execute page by page; selected nested connections must fit within their bounded root-page slice. Execution fails explicitly rather than returning partial JSONL when completeness cannot be proven without per-row pagination.
