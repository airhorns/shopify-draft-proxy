...
- A subsequent BulkOperation read advances at most one pending query export per GraphQL request. Snapshot completes that job from the effective local catalog without transport. LiveHybrid fetches at most one 250-row root catalog page, records its cursor, and returns `RUNNING` while more pages remain; a final page produces the terminal result.
