...
- Missing/null required input object fields are rejected before local staging with Shopify-style top-level GraphQL coercion errors. Captured 2025-01 evidence covers inline omissions for `name`/`query`/`resourceType` on `savedSearchCreate` and `id` on `savedSearchUpdate`, plus variable-supplied `SavedSearchCreateInput` omissions for `resourceType` and `name`. Inline omissions use `missingRequiredInputObjectAttribute`; variable omissions use top-level `INVALID_VARIABLE`. These payloads contain `errors` and no resolver `userErrors`.
...
- `fixtures/conformance/harry-test-heelo.myshopify.com/2025-01/saved-searches/saved-search-blank-name-validation.json` and `config/parity-specs/saved-searches/saved-search-blank-name-validation.json` prove empty-name `savedSearchCreate` returns base `UserError` field/message payloads and aggregates the invalid PRODUCT query-filter error when both validations fail.
- `fixtures/conformance/harry-test-heelo.myshopify.com/2025-01/saved-searches/saved-search-blank-name-update.json` and `config/parity-specs/saved-searches/saved-search-blank-name-update.json` prove explicit empty-name `savedSearchUpdate` returns `userErrors[{ field: ["input", "name"], message: "Name can't be blank" }]` and leaves the existing name visible on downstream `productSavedSearches` reads.
