...
- Empty saved-search roots return a non-null connection with empty `nodes`/`edges` and false/null `pageInfo`, matching the captured no-data shape. Snapshot reads use only base plus staged state and never call Shopify. Current captured conformance evidence only proves built-in defaults for `orderSavedSearches` and `draftOrderSavedSearches`; `productSavedSearches`, `collectionSavedSearches`, `customerSavedSearches`, `fileSavedSearches`, and discount saved-search roots stay empty unless base, upstream, or staged state contains records for those resource types.
- An unaffected cold LiveHybrid read forwards the caller's complete document once and returns Shopify's connection unchanged, including opaque edge cursors and the authoritative partial-page `pageInfo`.
- When a relevant staged create, update, or tombstone exists and the caller supplies `first` or `last`, LiveHybrid forms an effective request window from authoritative upstream rows plus the staged changes. A secondary query asks only for the requested row count plus the number of relevant staged records/tombstones, follows another page only within that bounded budget, and never hydrates rows one at a time. The overlay applies `after`/`before`, `reverse`, staged membership changes, and final `first`/`last` trimming while retaining Shopify cursors on authoritative rows and preserving upstream boundary booleans. A staged local create uses its stable synthetic cursor.
- A LiveHybrid overlay read that omits both `first` and `last` has no bounded completeness proof. It returns the authoritative caller response unchanged and does not project staged connection membership into that argumentless response. Callers that require staged saved-search connection overlays must request an explicit forward or backward window.
- LiveHybrid media reads that select `fileSavedSearches` hydrate upstream FILE saved-search rows before rendering an affected bounded connection. If that upstream row is the real Shopify counterpart of a locally staged FILE saved-search setup with the same resource type, name, and normalized query, the connection returns one row while the staged synthetic ID remains addressable for `files(savedSearchId:)`.
- Once base rows exist for a resource type, those rows replace the synthetic ORDER/DRAFT_ORDER defaults for that resource so captured per-shop IDs and names are preserved. The proxy does not fabricate non-order defaults without captured evidence for the current store/API shape. ORDER and DRAFT_ORDER defaults use stable synthetic local IDs only when no base rows are known, but otherwise behave like persisted Shopify rows for local `savedSearchUpdate` and `savedSearchDelete`: updates stage an override visible through the saved-search connection, and deletes tombstone the synthetic default ID so the static default does not reappear on later reads.
...
URL redirect mutation/import/bulk-delete roots are intentionally registered as unimplemented coverage, not supported local behavior. Cold `urlRedirect`, `urlRedirects`, and `urlRedirectsCount` reads forward upstream in LiveHybrid; cold snapshot reads and mutations fail closed instead of emitting local placeholder data; reject-mode mutations also fail before upstream. Supported metaobject online-store handle changes can stage `UrlRedirect` records, and those staged redirects are locally readable through `urlRedirect`, `urlRedirects`, and `urlRedirectsCount` with `path:` / `target:` filters, staged sort/windowing, and read-after-write behavior. That staged slice is not URL redirect mutation/import/bulk-delete lifecycle support.
