...
- `draftOrdersCount(query:)` applies the same local draft-order query matcher as `draftOrders(query:)` for staged draft orders, including supported name, email, and tag filters. Empty or omitted queries count the effective local draft-order set.
