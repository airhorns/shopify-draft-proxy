...
- `orderCapture` validation for multi-currency capture currency, over-capture, non-positive amounts, missing transactions, and no-longer-capturable authorizations returns local `userErrors` without mutating order financial state or logs. Capture currency is required and matched only when the order presentment currency differs from the shop currency; captured 2026-04 evidence shows single-currency CAD/CAD authorization captures accept omitted `currency` and stage a successful capture in CAD. Zero-amount single-currency captures return the public plain `UserError` shape with `field: null` and message `Amount must be greater than zero for capture transactions`; the local branch does not attach a fabricated `INVALID_AMOUNT` code. Multi-currency currency validation still returns the captured `["currency"]` field path, while parent transaction and capturability guards retain their focused runtime coverage. Live introspection against `harry-test-heelo.myshopify.com` on `2025-01`, `2025-04`, the supported public versions `2025-07`, `2025-10`, `2026-01`, `2026-04`, and `unstable` all returned `OrderCapturePayload` fields `transaction` and `userErrors`; `userErrors` resolves to plain `UserError` with only `field` and `message`, `OrderCaptureUserError` is absent, and selecting `userErrors { code }` fails schema validation. Public parity compares selectable public shape and transaction effects; focused runtime tests cover local staging and final-capture lock behavior until a captured schema exposes code-bearing order-capture user errors.
