...
- `orderUpdate` stages known-order simple edits locally for `note`, `tags`, `customAttributes`, `email`, `phone`, `poNumber`, `shippingAddress`, and order-scoped `metafields`. The staged order keeps its identity and line items, bumps `updatedAt`, updates `Order.email`, and preserves read-after-write through `order(id:)`, `orders`, and `ordersCount` without runtime Shopify writes. Live 2026-04 evidence shows that updating `Order.email` does not rewrite the nested `Order.customer.email`; the proxy preserves that distinction. The original raw mutation body is retained for commit replay.
