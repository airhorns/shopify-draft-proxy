...
- Abandoned checkout empty/no-data reads and local seeded reads are executable.
  `abandonmentUpdateActivitiesDeliveryStatuses` stages scalar
  `AbandonmentDeliveryState` updates for known local abandonments and preserves
  raw mutation order. Unknown abandonment IDs, unknown marketing activities, and
  missing delivery-status membership data return side-effect-free flat
  userErrors.
...
- `abandonmentUpdateActivitiesDeliveryStatuses` is local-only for seeded/snapshot abandonment records and cassette-backed local-runtime abandonment hydration. Public `Abandonment` reads expose `emailState` / `emailSentAt` but not the internal marketing-activity delivery map, so known-record staging requires local delivery activity membership data. The mutation uses scalar arguments (`abandonmentId`, `marketingActivityId`, `deliveryStatus`, optional `deliveredAt`, and optional `deliveryStatusChangeReason`) and the `AbandonmentDeliveryState` values `NOT_SENT`, `SENT`, and `SCHEDULED`. Unknown abandonment IDs return `abandonment: null` plus `userErrors[{ field: ["abandonmentId"], message: "abandonment_not_found", code: "ABANDONMENT_NOT_FOUND" }]`. Unknown marketing activities return `field: ["marketingActivityId"]` and `code: "MARKETING_ACTIVITY_NOT_FOUND"` without staging state. Missing delivery activity membership returns base `field: null` and `code: "DELIVERY_STATUS_INFO_NOT_FOUND"` without staging state. Successful updates surface `emailState` / `emailSentAt` changes on downstream local reads, append the original raw mutation to the meta log, and never send the runtime mutation to Shopify. Future `deliveredAt` values and repeated or reverse state changes are not locally rejected.
