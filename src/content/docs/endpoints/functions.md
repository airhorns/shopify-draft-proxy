...
- `fulfillmentConstraintRuleUpdate` resolves the target from effective staged-plus-base state. In LiveHybrid mode, a missing or partially observed target triggers one read-only `fulfillmentConstraintRules` hydrate before the handler returns `NOT_FOUND`; snapshot mode never invents a rule. A successful update changes `deliveryMethodTypes` while preserving the hydrated Function/app identity, metafields, and omitted fields, then stages the overlay for downstream reads. Empty `deliveryMethodTypes` and IDs absent from the hydrated list return userErrors without staging a replacement.
