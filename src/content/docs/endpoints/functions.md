...
- Before an enabled create or update applies the shop-wide active-validation cap, LiveHybrid scans only `id`, `enabled`, and `shopifyFunction.id` in threshold-width pages of 25. It stops as soon as 25 effective active rows prove the cap or the connection ends, caches those fields as private decision facts, and does not mark or expose a complete validation catalog. If the minimal probe is unavailable, one bounded ordinary connection window can supply the same private facts only when its required fields and terminal `pageInfo` prove the entire unfiltered catalog; partial or malformed windows remain fail-closed. The captured 26th-active payload uses `field: null`, code `MAX_VALIDATIONS_ACTIVATED`, and message `Cannot have more than 25 active validation functions.`.
...
- LiveHybrid resolves the requested Function exactly, reuses cached validation decision facts, and scans further validation decision pages only when a wrong-API Function needs the captured registration-precedence check. A correct cart-transform Function uses `cartTransforms(first: 1) { nodes { id functionId } }` to prove reuse or shop-wide presence without loading transform objects. If that minimal probe is unavailable, an ordinary bounded window proves presence from any valid row and proves absence only when terminal `pageInfo` makes the unfiltered window complete. A registered Function therefore returns `FUNCTION_ALREADY_REGISTERED` even when its lifecycle row had not previously been observed. An unavailable or inconclusive decision read fails closed without staging. With one different cart transform already present, Shopify's captured second-transform payload has null field/code and message `An API client cannot have more than 1 cart transform functions per shop`.
...
- `fulfillmentConstraintRuleUpdate` resolves the target from effective staged-plus-base state. In LiveHybrid mode, a missing or partially observed target triggers an exact read-only `node(id:)` lookup; partial observations merge over the full hydrated state so Function/app identity, metafields, and other omitted fields survive the update. An authoritative miss is scoped to that exact ID across state dump/restore and returns `NOT_FOUND`; an unavailable hydrate fails closed with a distinct null-code error. A compatibility retry may use the argument-free rule list after an exact-read 5xx, and only that successful complete list can prove other IDs absent. Empty `deliveryMethodTypes` still returns its input userError without staging a replacement.
...
- Shopify Admin 2026-04 reports the released `cart.fulfillment-constraints.generate.run` Function as API type `fulfillment_constraints`; the resolver recognizes that value as a fulfillment-constraint Function while retaining the captured public value in output projections.
...
- When a local-cursor continuation has no remaining rows, the connection returns Shopify's captured empty-page shape: false `hasNextPage` / `hasPreviousPage` values and null boundary cursors.
...
- `fulfillmentConstraintRules` has no connection arguments in the captured Admin schema. LiveHybrid forwards that caller read once and overlays staged rules and tombstones on the returned list rather than issuing a fixed-prefix catalog hydrate. If a local overlay exists and the caller omitted `id`, one same-selection list read adds only the identity needed to apply it without paging or fabricating missing rules.
...
- Partial connection observations never prove that validation or cart-transform lifecycle decisions are complete. A no-cursor, unfiltered window with terminal `pageInfo` and all required decision fields can seed private lifecycle facts; it still does not promote partial Function metadata or expose a complete public catalog. Active-validation threshold reads and cart-transform existence probes likewise stay in private decision state.
