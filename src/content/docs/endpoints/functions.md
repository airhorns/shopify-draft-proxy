...
- `fulfillmentConstraintRuleUpdate` resolves the target from effective staged-plus-base state. In LiveHybrid mode, an unresolved ID triggers one read-only `fulfillmentConstraintRules` catalog hydrate before the handler returns `NOT_FOUND`; snapshot mode never invents a rule. A successful update changes `deliveryMethodTypes` while preserving the hydrated Function/app identity, metafields, and omitted fields, then stages the overlay for downstream reads. Empty `deliveryMethodTypes` and IDs absent from an authoritative catalog return userErrors without staging a replacement.
...
- Fulfillment constraint success-path parity uses the released `conformance-fulfillment-constraint` Function and requires `read_fulfillment_constraint_rules` plus `write_fulfillment_constraint_rules`. The registered capture creates a disposable rule with a metafield, records its exact pre-update catalog for the proxy's read-only hydration cassette, updates only `deliveryMethodTypes`, verifies the full downstream rule payload, and deletes the rule.
