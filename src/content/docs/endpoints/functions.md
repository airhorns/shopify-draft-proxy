...
- Before an enabled create or update applies the shop-wide active-validation cap, LiveHybrid hydrates the complete validation catalog. Caller-selected partial validation reads are cached as rows but do not make that lifecycle catalog authoritative. The captured 26th-active payload uses `field: null`, code `MAX_VALIDATIONS_ACTIVATED`, and message `Cannot have more than 25 active validation functions.`.
...
- LiveHybrid hydrates both complete validation and cart-transform catalogs before the `functionId` reuse and shop-wide transform-cap decisions. A registered Function therefore returns `FUNCTION_ALREADY_REGISTERED` even when its lifecycle row had not previously been observed. With one different cart transform already present, Shopify's captured second-transform payload has null field/code and message `An API client cannot have more than 1 cart transform functions per shop`.
...
- `fulfillmentConstraintRuleUpdate` resolves an unknown local target through the read-only LiveHybrid rule catalog, updates effective base or staged `deliveryMethodTypes`, and returns the updated rule. Only an authoritative miss returns `NOT_FOUND`; empty `deliveryMethodTypes` still returns its input userError without staging a replacement.
- `fulfillmentConstraintRuleDelete` likewise hydrates an unobserved target before deciding existence, tombstones effective base or staged rules, and returns `{ success: true, userErrors: [] }`. An authoritative unknown ID returns `{ success: false, userErrors: [...] }`; the payload does not use `deletedId`.
...
- Only the proxy's dedicated complete lifecycle reads mark validation, cart-transform, or fulfillment-rule catalogs authoritative. A forwarded caller query with a bounded/filtered selection can contribute observed rows but never suppress the later complete preflight required for global decisions.
