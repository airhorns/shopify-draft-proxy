...
- `fulfillmentConstraintRuleUpdate` updates staged rule `deliveryMethodTypes` and returns the updated rule. Public create-only arguments such as `functionId` and `functionHandle` are rejected as top-level GraphQL argument errors before the local handler runs. Empty `deliveryMethodTypes` is accepted on update and stores all modeled delivery method types for downstream reads.
- `fulfillmentConstraintRuleUpdate` and `fulfillmentConstraintRuleDelete` check the staged rule's resolved `ShopifyFunction` owner when app id/key metadata is present. A rule whose Function is clearly owned by a different app returns `UNAUTHORIZED_APP_SCOPE` without mutating staged state. Rules without owner metadata remain best-effort because the proxy cannot infer ownership from local state alone.
- `fulfillmentConstraintRuleDelete` removes an authorized staged rule and returns `{ success: true, userErrors: [] }`. Unknown IDs return `{ success: false, userErrors: [...] }`; the payload does not use `deletedId`.
