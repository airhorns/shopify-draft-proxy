...
- Partial connection observations never prove that validation or cart-transform lifecycle decisions are complete. Active-validation threshold reads and cart-transform existence probes stay in private decision state and do not promote a caller window into an authoritative catalog.
