...
before local staging. Unknown publication validation is backed by the effective
publication catalog: staged or base publication rows are accepted, LiveHybrid can
hydrate the catalog before validation, and arbitrary well-formed `Publication`
GIDs are rejected when they remain absent. Missing required `id` arguments are
rejected by the shared Admin GraphQL argument validator before local staging can
fabricate a target. The top-level publishable `id` must resolve to a known
