...
function mergeCustomerRecords(base: CustomerRecord | null, staged: CustomerRecord | null): CustomerRecord | null {
  ...
  return {
    ...
    createdAt: base.createdAt,
    updatedAt:
      base.updatedAt && staged.updatedAt ? ensureUpdatedAtAfterBase(base.updatedAt, staged.updatedAt) : (staged.updatedAt ?? base.updatedAt),
  };
}
...
export class InMemoryStore {
  ...
  updateLogEntry(entryId: string, updates: Partial<Pick<MutationLogEntry, 'status' | 'notes'>>): MutationLogEntry | null {
  ...
  getEffectiveCustomerById(customerId: string): CustomerRecord | null {
    ...
    return mergeCustomerRecords(this.baseState.customers[customerId] ?? null, this.stagedState.customers[customerId] ?? null);
  }
  ...
  listEffectiveCustomers(): CustomerRecord[] {
    ...
    for (const customerId of Array.from(customerIds)) {
      ...
      const customer = mergeCustomerRecords(this.baseState.customers[customerId] ?? null, this.stagedState.customers[customerId] ?? null);
      if (customer) {
  ...
  hasStagedCustomers(): boolean {
    return Object.keys(this.stagedState.customers).length > 0 || Object.keys(this.stagedState.deletedCustomerIds).length > 0;
  }
