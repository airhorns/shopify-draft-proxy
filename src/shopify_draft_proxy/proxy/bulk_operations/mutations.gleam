//// Bulk-operations mutation handling.
...
//// implemented Admin API inner mutations.
...
fn handle_mutation_fields(
        ...
        Field(name: name, ..) -> {
          ...
          case dispatch {
            ...
            Some(#(result, next_store, next_identity)) -> #(
              ...
              suppressed || result.suppress_outer_log,
            )
          }
        }
        _ -> acc
  ...
  let log_drafts = case primary_root, field_drafts {
    ...
    Some("bulkOperationRunMutation"), [] if suppress_outer_log -> []
...
fn continue_run_mutation_with_present_client_identifier(
  ...
  case store.get_staged_upload_content(store, path) {
    ...
    Some(content) ->
      case upload_content_too_large(content, max_input_file_size_bytes) {
        True ->
          return_run_mutation_oversized_upload(
            store,
            identity,
            field,
            fragments,
...
fn return_run_mutation_oversized_upload(
  store: Store,
  identity: SyntheticIdentityRegistry,
  field: Selection,
  fragments: FragmentMap,
...
fn run_bulk_import_line(
  ...
  case bulk_mutation_handler(inner_root) {
    Some(handler) -> {
      ...
      let log_drafts = case outcome.staged_resource_ids {
        [] -> []
        _ -> [
...
fn bulk_import_log_draft(
  ...
  LogDraft(
    ...
    root_fields: root_fields,
    primary_root_field: primary_root_field,
    domain: bulk_import_domain(primary_root_field),
    execution: "stage-locally",
    query: Some(mutation),
    variables: Some(variables),
          ...
          let root_count_errors = case root_fields {
            [single_root] ->
              case root_field_name(single_root) {
                Some(name) ->
                  case is_disallowed_inner_bulk_mutation_root(name) {
                    True -> ["You must use an allowed mutation name."]
                    False -> []
                  }
...
fn is_disallowed_inner_bulk_mutation_root(name: String) -> Bool {
  case name {
    "bulkOperationRunMutation" | "bulkOperationRunQuery" -> True
    _ -> False
  }
}
