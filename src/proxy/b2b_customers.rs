...
fn customer_address_node_json(fields: CustomerAddressNodeFields) -> Value {
    ...
    json!({
        ...
        "province": fields.province.as_ref().map(|province| province.name),
        "provinceCode": fields.province.as_ref().map(|province| province.code),
        "country": fields.country.as_ref().map(|country| country.name),
        "countryCodeV2": fields.country.as_ref().map(|country| country.code),
        "zip": fields.zip,
        "phone": fields.phone,
