...
function getSelectedChildFields(field: FieldNode): FieldNode[] {
  return (field.selectionSet?.selections ?? []).filter((selection): selection is FieldNode => selection.kind === Kind.FIELD);
}
...
function normalizeDraftOrderAddress(raw: unknown): DraftOrderAddressRecord | null {
  ...
  const countryCode = typeof address['countryCodeV2'] === 'string'
...
function normalizeOrderLineItems(raw: unknown, currencyCode: string): OrderLineItemRecord[] {
  ...
  return raw
    .filter((lineItem): lineItem is Record<string, unknown> => typeof lineItem === 'object' && lineItem !== null)
    .map((lineItem) => {
      const originalUnitPriceSet = typeof lineItem['originalUnitPriceSet'] === 'object' && lineItem['originalUnitPriceSet'] !== null
        ? lineItem['originalUnitPriceSet'] as Record<string, unknown>
        : {};
      const shopMoney = typeof originalUnitPriceSet['shopMoney'] === 'object' && originalUnitPriceSet['shopMoney'] !== null
      ...
      return {
        ...
        variantTitle: null,
        originalUnitPriceSet: {
          shopMoney: normalizeMoney(price, typeof shopMoney['currencyCode'] === 'string' ? shopMoney['currencyCode'] : currencyCode),
        },
      };
    });
}
...
function normalizeOrderShippingLines(raw: unknown, currencyCode: string): OrderShippingLineRecord[] {
  ...
  return raw
    .filter((shippingLine): shippingLine is Record<string, unknown> => typeof shippingLine === 'object' && shippingLine !== null)
    .map((shippingLine) => {
      const priceSet = typeof shippingLine['priceSet'] === 'object' && shippingLine['priceSet'] !== null
        ? shippingLine['priceSet'] as Record<string, unknown>
        : {};
      const shopMoney = typeof priceSet['shopMoney'] === 'object' && priceSet['shopMoney'] !== null
      ...
      return {
        ...
        originalPriceSet: {
          shopMoney: normalizeMoney(amount, typeof shopMoney['currencyCode'] === 'string' ? shopMoney['currencyCode'] : currencyCode),
        },
      };
    });
}
...
function buildOrderCustomerFromInput(inputRecord: Record<string, unknown>): OrderCustomerRecord | null {
  ...
  const displayName = [firstName, lastName].filter((value): value is string => Boolean(value && value.length > 0)).join(' ').trim();
...
function buildOrderFromInput(input: unknown): OrderRecord {
  ...
  return {
    ...
    tags: Array.isArray(inputRecord['tags'])
      ? inputRecord['tags'].filter((tag): tag is string => typeof tag['toString'] === 'function' && typeof tag === 'string').sort((left, right) => left.localeCompare(right))
      : [],
...
function buildDraftOrderFromInput(input: unknown, shopifyAdminOrigin: string): DraftOrderRecord {
  ...
  return {
    ...
    note: typeof inputRecord['note'] === 'string' ? inputRecord['note'] : null,
    tags: Array.isArray(inputRecord['tags'])
      ? inputRecord['tags'].filter((tag): tag is string => typeof tag === 'string').sort((left, right) => left.localeCompare(right))
      : [],
    customAttributes: normalizeDraftOrderAttributes(inputRecord['customAttributes']),
    billingAddress: normalizeDraftOrderAddress(inputRecord['billingAddress']),
    shippingAddress: normalizeDraftOrderAddress(inputRecord['shippingAddress']),
...
function serializeDraftOrderAddress(field: FieldNode, address: DraftOrderAddressRecord | null): Record<string, unknown> | null {
  if (!address) {
    return null;
  }
  const result: Record<string, unknown> = {};
...
function serializeDraftOrderAttributes(field: FieldNode, attributes: DraftOrderAttributeRecord[]): Array<Record<string, unknown>> {
...
function serializeDraftOrderShippingLine(field: FieldNode, shippingLine: DraftOrderShippingLineRecord | null): Record<string, unknown> | null {
  if (!shippingLine) {
    return null;
  }
  const result: Record<string, unknown> = {};
...
function serializeDraftOrderLineItemNode(field: FieldNode, lineItem: DraftOrderLineItemRecord): Record<string, unknown> {
...
function serializeDraftOrderLineItemsConnection(field: FieldNode, lineItems: DraftOrderLineItemRecord[]): Record<string, unknown> {
  const result: Record<string, unknown> = {};
  for (const selection of getSelectedChildFields(field)) {
    const key = getFieldResponseKey(selection);
    switch (selection.name.value) {
...
function readNullableIntArgument(field: FieldNode, argumentName: string, variables: Record<string, unknown>): number | null {
...
function readNullableStringArgument(field: FieldNode, argumentName: string, variables: Record<string, unknown>): string | null {
  const argument = field.arguments?.find((candidate) => candidate.name.value === argumentName);
  if (!argument) {
    return null;
  }
...
function serializeDraftOrdersConnection(field: FieldNode, draftOrders: DraftOrderRecord[], variables: Record<string, unknown>): Record<string, unknown> {
        ...
        result[key] = Object.fromEntries(
          getSelectedChildFields(selection).map((pageInfoSelection) => {
            ...
            switch (pageInfoSelection.name.value) {
              ...
              case 'endCursor':
                return [pageInfoKey, visibleRecords.length > 0 ? buildSyntheticCursor(visibleRecords[visibleRecords.length - 1]!.id) : null];
              default:
                return [pageInfoKey, null];
            }
          }),
...
function serializeOrderCustomer(field: FieldNode, customer: OrderCustomerRecord | null): Record<string, unknown> | null {
  if (!customer) {
    return null;
  }
...
function serializeOrderLineItemsConnection(field: FieldNode, lineItems: OrderLineItemRecord[]): Record<string, unknown> {
  const result: Record<string, unknown> = {};
  for (const selection of getSelectedChildFields(field)) {
    const key = getFieldResponseKey(selection);
    switch (selection.name.value) {
...
function serializeOrderShippingLinesConnection(field: FieldNode, shippingLines: OrderShippingLineRecord[]): Record<string, unknown> {
  const result: Record<string, unknown> = {};
  for (const selection of getSelectedChildFields(field)) {
    const key = getFieldResponseKey(selection);
    switch (selection.name.value) {
...
function serializeOrdersConnection(field: FieldNode, orders: OrderRecord[], variables: Record<string, unknown>): Record<string, unknown> {
  const first = readNullableIntArgument(field, 'first', variables);
        ...
        result[key] = Object.fromEntries(
          getSelectedChildFields(selection).map((pageInfoSelection) => {
            ...
            switch (pageInfoSelection.name.value) {
              ...
              case 'endCursor':
                return [pageInfoKey, visibleOrders.length > 0 ? `cursor:${visibleOrders[visibleOrders.length - 1]!.id}` : null];
              default:
                return [pageInfoKey, null];
            }
          }),
...
function serializeSelectedUserErrors(field: FieldNode, userErrors: Array<{ field: string[] | null; message: string }>): Array<Record<string, unknown>> {
...
function buildFulfillmentTrackingInfoUpdateNullInlineIdError(): Record<string, unknown> {
  return {
    message: "Argument 'fulfillmentId' on Field 'fulfillmentTrackingInfoUpdate' has an invalid value (null). Expected type 'ID!'.",
  ...
  for (const field of getRootFields(document)) {
    ...
    if (field.name.value === 'orderUpdate') {
      ...
      if (existingOrder && (readMode === 'snapshot' || readMode === 'live-hybrid')) {
        const updatedOrder = store.updateOrder(recalculateOrderTotals({
          ...existingOrder,
          updatedAt: makeSyntheticTimestamp(),
    ...
    if (field.name.value === 'fulfillmentTrackingInfoUpdate' && (readMode === 'snapshot' || readMode === 'live-hybrid')) {
      handled = true;
    ...
    if (field.name.value === 'orderEditAddVariant' && (readMode === 'snapshot' || readMode === 'live-hybrid')) {
      ...
      handled = true;
      const updatedCalculatedOrder = store.updateCalculatedOrder(recalculateOrderTotals({
        ...calculatedOrder,
    ...
    if (field.name.value === 'orderEditSetQuantity' && (readMode === 'snapshot' || readMode === 'live-hybrid')) {
      ...
      const updatedCalculatedOrder = store.updateCalculatedOrder(recalculateOrderTotals({
        ...calculatedOrder,
        lineItems: calculatedOrder.lineItems.map((lineItem) => lineItem.id === lineItemId ? { ...lineItem, quantity } : lineItem),
      }) as CalculatedOrderRecord);
      const updatedCalculatedLineItem = updatedCalculatedOrder.lineItems.find((lineItem) => lineItem.id === lineItemId) ?? targetLineItem;
      const payload: Record<string, unknown> = {};
      for (const selection of getSelectedChildFields(field)) {
        const selectionKey = getFieldResponseKey(selection);
        switch (selection.name.value) {
    ...
    if (field.name.value === 'orderEditCommit' && (readMode === 'snapshot' || readMode === 'live-hybrid')) {
      ...
      const committedOrder = store.updateOrder(recalculateOrderTotals({
