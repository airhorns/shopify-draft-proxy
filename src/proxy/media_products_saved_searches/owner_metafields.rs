...
impl DraftProxy {
    ...
    fn selected_owner_metafields_connection(
        ...
        // mirrors Shopify's default metafield ordering, with optional reverse
        // applied before cursor slicing). `after` drops everything up to and
        // including the cursor record; `first` truncates and drives hasNextPage so
        // chained `metafields(first:n, after:)` reads page correctly.
        ...
        let start_cursor = records_for_output.first().and_then(metafield_cursor);
        let end_cursor = records_for_output.last().and_then(metafield_cursor);
        selected_typed_connection_with_page_info(
            &records_for_output,
            &selection.selection,
            selected_json,
