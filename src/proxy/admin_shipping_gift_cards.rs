...
impl DraftProxy {
    ...
    pub(in crate::proxy) fn has_delivery_profile_state(&self) -> bool {
    ...
    pub(in crate::proxy) fn shipping_fulfillment_order_mutation_response(
        ...
        match root_field {
            ...
            "fulfillmentOrderSplit" => {
                self.fulfillment_order_split_store_backed(query, variables, request)
            }
            "fulfillmentOrderMerge" => {
                self.fulfillment_order_merge_store_backed(query, variables, request)
            }
    ...
    fn fulfillment_order_split_store_backed(
        &mut self,
        query: &str,
        variables: &BTreeMap<String, ResolvedValue>,
        request: &Request,
        ...
        let response_key =
            root_field_response_key(query).unwrap_or_else(|| "fulfillmentOrderSplit".to_string());
        let payload_selection = root_field_selection(query).unwrap_or_default();
        let arguments = root_field_arguments(query, variables).unwrap_or_default();
        let inputs = resolved_object_list_field(&arguments, "fulfillmentOrderSplits");
        let timestamp = self.next_shipping_fulfillment_timestamp();
        ...
        for input in inputs {
            let id = resolved_string_field(&input, "fulfillmentOrderId").unwrap_or_default();
            if !self.ensure_shipping_fulfillment_order_hydrated(request, &id) {
                user_errors.push(json!({
                    "field": ["fulfillmentOrderSplits", "fulfillmentOrderId"],
                    "message": "The fulfillment order could not be found.",
            ...
            let synthetic_order_ids = vec![self.next_proxy_synthetic_gid("FulfillmentOrder")];
            let mut synthetic_line_item_ids = Vec::new();
            let line_count = requested.len().max(1);
            for _ in 0..line_count {
                synthetic_line_item_ids
            ...
            if let Some(order) = self.store.staged.orders.get_mut(&order_id) {
                if let Some(nodes) = fulfillment_order_nodes_mut(order) {
                    ...
                    let Some(mut remaining_order) = split else {
                        user_errors.push(json!({
                            "field": ["fulfillmentOrderSplits", "fulfillmentOrderLineItems"],
                            "message": "Fulfillment order could not be split for the requested quantities.",
                            "code": "INVALID"
                        }));
                        continue;
                    };
                    fulfillment_order["updatedAt"] = json!(timestamp.clone());
                    fulfillment_order["supportedActions"] =
                        shipping_fulfillment_open_split_merge_actions(true);
                    remaining_order["supportedActions"] =
                        shipping_fulfillment_open_split_merge_actions(false);
                    remaining_order["_draftProxySplitSource"] = json!(id.clone());
                    remaining_order["_draftProxySplitKind"] = json!("split");
                    nodes[index] = fulfillment_order.clone();
                    nodes.insert(index + 1, remaining_order.clone());
                    split_payloads.push(json!({
                        "fulfillmentOrder": fulfillment_order,
                        "remainingFulfillmentOrder": remaining_order,
        ...
        ok_json(json!({
            "data": {
                response_key: fulfillment_order_split_payload_json(
                    split_payloads,
                    &payload_selection,
                    user_errors
                )
    ...
    fn fulfillment_order_merge_store_backed(
        &mut self,
        query: &str,
        variables: &BTreeMap<String, ResolvedValue>,
        request: &Request,
        ...
        let response_key =
            root_field_response_key(query).unwrap_or_else(|| "fulfillmentOrderMerge".to_string());
        let payload_selection = root_field_selection(query).unwrap_or_default();
        let arguments = root_field_arguments(query, variables).unwrap_or_default();
        let inputs = resolved_object_list_field(&arguments, "fulfillmentOrderMergeInputs");
        let timestamp = self.next_shipping_fulfillment_timestamp();
        let mut merge_payloads = Vec::new();
        let mut user_errors = Vec::new();
        let mut staged_ids = Vec::new();

        for input in inputs {
            let ids = resolved_object_list_field(&input, "mergeIntents")
                .into_iter()
            ...
            let Some(first_id) = ids.first() else {
                user_errors.push(json!({
                    "field": ["fulfillmentOrderMergeInputs", "mergeIntents"],
                    "message": "At least one fulfillment order is required.",
                    "code": "INVALID"
                }));
                continue;
            };
            let Some((order_id, _)) = self.shipping_fulfillment_order_location(first_id) else {
                user_errors.push(json!({
                    "field": ["fulfillmentOrderMergeInputs", "mergeIntents"],
                    "message": "The fulfillment orders could not be found.",
                    "code": "FULFILLMENT_ORDERS_NOT_FOUND"
            ...
            if let Some(order) = self.store.staged.orders.get_mut(&order_id) {
                let Some(nodes) = fulfillment_order_nodes_mut(order) else {
                    continue;
                };
                let mut indices = ids
                    .iter()
                ...
                let merge_orders = indices
                    .iter()
                    .filter_map(|index| nodes.get(*index).cloned())
                    .collect::<Vec<_>>();
                let mut merged = nodes[first_index].clone();
                merged["updatedAt"] = json!(timestamp.clone());
                merged["supportedActions"] = shipping_fulfillment_open_actions(true);
                if merged["fulfillBy"].is_null() {
                    if let Some(deadline) = merge_orders
                        .iter()
                        .find_map(|order| order["fulfillBy"].as_str())
                    {
                        merged["fulfillBy"] = json!(deadline);
                    }
                }
                merged["lineItems"] = json!({
                    "nodes": merge_fulfillment_order_line_items(&merge_orders)
                });
                nodes[first_index] = merged.clone();
                for index in indices.into_iter().filter(|index| *index != first_index) {
                    nodes[index]["status"] = json!("CLOSED");
                    nodes[index]["updatedAt"] = json!(timestamp.clone());
                    nodes[index]["supportedActions"] = json!([]);
                    for line in nodes[index]["lineItems"]["nodes"]
                        .as_array_mut()
                        .into_iter()
                        .flatten()
                    {
                ...
                merge_payloads.push(json!({ "fulfillmentOrder": merged }));
        ...
        ok_json(json!({
            "data": {
                response_key: fulfillment_order_merge_payload_json(
                    merge_payloads,
                    &payload_selection,
                    user_errors
                )
    ...
    pub(in crate::proxy) fn observe_shipping_fulfillment_order_passthrough_response(
        &mut self,
        root_field: &str,
        response: &Response,
    ) {
        let payload = &response.body["data"][root_field];
        for field in [
            ...
            if payload[field].is_object() {
                self.stage_shipping_fulfillment_order_record(payload[field].clone());
            }
        }
    }
...
fn merge_fulfillment_order_line_items(orders: &[Value]) -> Vec<Value> {
    let mut line_items_by_id = BTreeMap::new();
    for line in orders.iter().flat_map(fulfillment_order_line_item_nodes) {
        let key = line["lineItem"]["id"]
            .as_str()
            .or_else(|| line["id"].as_str())
            .unwrap_or_default()
    ...
    line_items_by_id.into_values().collect()
}
...
fn fulfillment_order_split_payload_json(
    ...
    selected_payload_json(payload_selection, |selection| {
        match selection.name.as_str() {
            "fulfillmentOrderSplits" => Some(Value::Array(
                splits
                    .iter()
...
fn fulfillment_order_merge_payload_json(
    ...
    selected_payload_json(payload_selection, |selection| {
        match selection.name.as_str() {
            "fulfillmentOrderMerges" => Some(Value::Array(
                merges
                    .iter()
...
fn shipping_fulfillment_open_split_merge_actions(include_split: bool) -> Value {
    let mut actions = vec!["CREATE_FULFILLMENT", "REPORT_PROGRESS", "MOVE", "HOLD"];
    if include_split {
        actions.push("SPLIT");
    }
