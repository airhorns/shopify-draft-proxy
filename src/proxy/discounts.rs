...
impl DraftProxy {
        ...
        root_payload_json(fields, |field| {
            let value = match field.name.as_str() {
                ...
                "discountNodes" => Some(json!({
                    "nodes": self.filtered_discount_records(field).into_iter().map(|record| self.discount_admin_node_for_record(record)).collect::<Vec<_>>()
                })),
    ...
    fn filtered_discount_records(&self, field: &RootFieldSelection) -> Vec<&Value> {
        ...
        self.store
            ...
            .filter(|record| self.discount_matches_query(record, &query))
            .collect()
    }
    ...
    fn discount_admin_node_for_record(&self, record: &Value) -> Value {
        json!({
            "id": discount_id(record),
            "discount": self.discount_body_for_record(record),
...
fn discount_record_from_input(
    ...
    json!({
        ...
        "customerGets": discount_customer_gets_from_input(typename, input, context.shop_currency_code),
