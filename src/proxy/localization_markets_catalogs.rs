...
impl DraftProxy {
    ...
    pub(in crate::proxy) fn discount_timestamps_monotonic_update_data(
        &mut self,
        fields: &[RootFieldSelection],
    ) -> Value {
        let mut data = serde_json::Map::new();
    ...
    pub(in crate::proxy) fn discount_timestamps_monotonic_read_data(
        &self,
        fields: &[RootFieldSelection],
    ) -> Value {
        let mut data = serde_json::Map::new();
    ...
    pub(in crate::proxy) fn localization_register_response(
        ...
        for (index, translation_input) in translations.iter().enumerate() {
            ...
            if resolved_object_string(translation_input, "locale").as_deref() == Some("en") {
                user_errors.push(json!({
