...
impl DraftProxy {
    ...
    pub(in crate::proxy) fn seed_state(&mut self, request: &Request) -> Response {
        ...
        if let Some(gift_cards) = body.get("giftCards").and_then(Value::as_array) {
            for gift_card in gift_cards {
                if let Some(id) = gift_card.get("id").and_then(Value::as_str) {
                    self.store
                        .staged
                        .gift_cards
                        .insert(id.to_string(), gift_card.clone());
                }
            }
        }
