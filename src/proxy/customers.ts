...
function normalizeStringField(raw: Record<string, unknown>, key: string, fallback: string | null = null): string | null {
...
function normalizeStringLikeField(raw: Record<string, unknown>, key: string, fallback: string | null = null): string | null {
  if (!hasOwnField(raw, key)) {
    return fallback;
  }
...
function normalizeCountField(raw: Record<string, unknown>, key: string, fallback: string | number | null = null): string | number | null {
  if (!hasOwnField(raw, key)) {
    return fallback;
  }
...
function normalizeBooleanField(raw: Record<string, unknown>, key: string, fallback: boolean | null = null): boolean | null {
  if (!hasOwnField(raw, key)) {
    return fallback;
  }
...
function buildCustomerDisplayName(firstName: string | null, lastName: string | null, email: string | null): string | null {
...
function normalizeCustomer(raw: unknown): CustomerRecord | null {
  ...
  return {
    ...
    amountSpent: normalizeMoney(hasOwnField(raw, 'amountSpent') ? raw['amountSpent'] : undefined, existing?.amountSpent ?? null),
...
function serializeMoneySelection(field: FieldNode, value: CustomerRecord['amountSpent']): Record<string, unknown> | null {
  if (!value) {
    return null;
  }
...
function resolveCatalogCustomerCursor(customerId: string, catalogConnection: CustomerCatalogConnectionRecord | null): string {
...
function matchesStringValue(candidate: string | null | undefined, rawValue: string, matchMode: 'includes' | 'exact'): boolean {
...
function customerMatchesPositiveQueryTerm(customer: CustomerRecord, term: string): boolean {
  ...
  switch (field) {
    case 'email':
      return matchesStringValue(customer.email, rawValue, 'includes')
...
function compareSortableTuple(left: Array<string | null>, right: Array<string | null>): number {
  for (let index = 0; index < Math.max(left.length, right.length); index += 1) {
    const comparison = normalizeSortableString(left[index] ?? null).localeCompare(normalizeSortableString(right[index] ?? null));
    if (comparison !== 0) {
      return comparison;
    }
  }
...
function sortCustomers(customers: CustomerRecord[], rawSortKey: unknown, rawReverse: unknown): CustomerRecord[] {
  ...
  if (sortKey === 'UPDATED_AT') {
    sorted.sort((left, right) => compareNullableStrings(left.updatedAt, right.updatedAt) || left.id.localeCompare(right.id));
  } else if (sortKey === 'CREATED_AT') {
    sorted.sort((left, right) => compareNullableStrings(left.createdAt, right.createdAt) || left.id.localeCompare(right.id));
  } else if (sortKey === 'NAME') {
    sorted.sort((left, right) => buildCustomerSortName(left).localeCompare(buildCustomerSortName(right)) || left.id.localeCompare(right.id));
  } else if (sortKey === 'ID') {
    sorted.sort((left, right) => compareCustomerIds(left.id, right.id) || left.id.localeCompare(right.id));
  } else if (sortKey === 'LOCATION') {
    sorted.sort(
      (left, right) => compareSortableTuple(
        [left.defaultAddress?.country ?? null, left.defaultAddress?.province ?? null, left.defaultAddress?.city ?? null],
        [right.defaultAddress?.country ?? null, right.defaultAddress?.province ?? null, right.defaultAddress?.city ?? null],
    ...
    switch (pageInfoSelection.name.value) {
      ...
      case 'startCursor':
        pageInfo[pageInfoKey] = visibleCustomers[0]
          ...
          : (options.preserveBaselinePageInfo ? (catalogConnection?.pageInfo.startCursor ?? null) : null);
        break;
      case 'endCursor':
        pageInfo[pageInfoKey] = visibleCustomers.length > 0
          ? resolveCatalogCustomerCursor(visibleCustomers[visibleCustomers.length - 1]!.id, catalogConnection)
          : (options.preserveBaselinePageInfo ? (catalogConnection?.pageInfo.endCursor ?? null) : null);
        break;
    }
  }
...
function serializeCustomersConnection(field: FieldNode, variables: Record<string, unknown>): Record<string, unknown> {
  ...
  const first = typeof args['first'] === 'number' && Number.isFinite(args['first']) ? Math.max(0, Math.floor(args['first'])) : null;
  const last = typeof args['last'] === 'number' && Number.isFinite(args['last']) ? Math.max(0, Math.floor(args['last'])) : null;
  ...
  const allCustomers = sortCustomers(filterCustomersByQuery(listCustomersForConnection(activeConnection), args['query']), args['sortKey'], args['reverse']);
  ...
  const visibleStartIndex = last === null ? startIndex : Math.max(startIndex, startIndex + firstWindow.length - visibleCustomers.length);
  ...
  const hasPreviousPage = calculatedHasPreviousPage || (preserveBaselinePageInfo && (activeConnection?.pageInfo.hasPreviousPage ?? false));
  const hasNextPage = calculatedHasNextPage || (preserveBaselinePageInfo && (activeConnection?.pageInfo.hasNextPage ?? false));
  ...
  for (const selection of getSelectedChildFields(field)) {
    ...
    switch (selection.name.value) {
      ...
      case 'edges':
        ...
        break;
      case 'pageInfo':
        connection[key] = buildCatalogPageInfo(visibleCustomers, selection, hasNextPage, hasPreviousPage, activeConnection, {
...
function collectHydratableCustomers(raw: unknown): CustomerRecord[] {
  ...
  return [directCustomer, ...connectionCustomers].filter((candidate): candidate is CustomerRecord => candidate !== null);
}

function collectCustomerCatalogConnection(raw: unknown, responseKey = 'customers'): CustomerCatalogConnectionRecord | null {
...
function serializeCustomerMutationUserErrors(field: FieldNode, userErrors: CustomerMutationUserError[]): Array<Record<string, unknown>> {
...
function buildCreatedCustomer(input: Record<string, unknown>): CustomerRecord {
  ...
  const firstName = typeof input['firstName'] === 'string' && input['firstName'].trim().length > 0 ? input['firstName'].trim() : null;
  const lastName = typeof input['lastName'] === 'string' && input['lastName'].trim().length > 0 ? input['lastName'].trim() : null;
  const locale = typeof input['locale'] === 'string' && input['locale'].trim().length > 0 ? input['locale'].trim() : null;
...
function buildUpdatedCustomer(existing: CustomerRecord, input: Record<string, unknown>): CustomerRecord {
  ...
  const phone = typeof input['phone'] === 'string' ? (input['phone'].trim() || null) : existing.defaultPhoneNumber?.phoneNumber ?? null;
...
export function handleCustomerMutation(document: string, variables: Record<string, unknown> = {}): { data: Record<string, unknown> } {
