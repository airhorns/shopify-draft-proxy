...
pub(in crate::proxy) fn webhook_subscription_search_decision(
    ...
    search_query_decision(query, |term| {
        webhook_subscription_search_term_decision(record, term)
    })
}

fn webhook_subscription_search_term_decision(
    record: &Value,
    ...
    match webhook_subscription_matches_query_term(record, field, term.value.trim()) {
...
#[cfg(test)]
mod webhook_subscription_search_tests {
    use super::*;

    fn search_webhook_subscription() -> Value {
        json!({
            "id": "gid://shopify/WebhookSubscription/6001",
            "topic": "ORDERS_CREATE",
            "format": "JSON",
            "uri": "https://example.test/webhooks/orders",
            "callbackUrl": "https://example.test/webhooks/orders",
    ...
    #[test]
    fn webhook_subscription_search_composes_boolean_queries() {
        let subscription = search_webhook_subscription();

        assert_eq!(
            webhook_subscription_search_decision(
                &subscription,
                Some("topic:PRODUCTS_CREATE OR topic:ORDERS_CREATE")
            ),
        ...
        assert_eq!(
            webhook_subscription_search_decision(
                &subscription,
                Some("(topic:PRODUCTS_CREATE OR topic:ORDERS_CREATE) format:JSON")
            ),
            StagedSearchDecision::Match
        );
        assert_eq!(
            webhook_subscription_search_decision(&subscription, Some("-topic:ORDERS_CREATE")),
            StagedSearchDecision::NoMatch
        );
        assert_eq!(
            webhook_subscription_search_decision(&subscription, Some("NOT topic:ORDERS_CREATE")),
            StagedSearchDecision::NoMatch
        );
    }
}
