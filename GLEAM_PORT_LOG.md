...
## 2026-04-30 — Pass 37: legacy integration inventory baseline
...
This pass records the original TypeScript integration corpus without requiring
those files to remain present. That is deliberate: Pass 34 already removes the
Events TypeScript integration test after moving that runtime surface to Gleam,
and later cutover passes should not have to update this guard merely because a
legacy TS file disappeared. The retained boundary entries remain intentionally
narrow: JS interop, package launch scripts, the operator HTML meta UI,
normalized snapshot file loading, and the large TypeScript runtime performance
smoke stay tied to their existing TS tests until the corresponding Gleam HTTP
adapter, snapshot loader, and broad resource domains land.
...
TypeScript integration entries from the recorded baseline against 50 Gleam test
