...
## 2026-05-01 - Pass 117: Admin Platform and state-dump merge refresh

Refreshes HAR-498 after `origin/main@2e0fdc79` landed the state dump
completeness proof on top of the latest Segments baseline. The merge keeps
Admin Platform utility/node/taxonomy parity ungated while preserving mainline
strict dump/restore field coverage and capture-shaped parity seeding.

| Module                                                    | Change                                                                                                                    |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `gleam/test/parity/runner.gleam`                          | Keeps mainline capture-shaped seeding and adds Admin Platform generic-node/taxonomy seeding into the helper fold.         |
| `gleam/src/shopify_draft_proxy/state/serialization.gleam` | Preserves strict state-dump field inventories plus Admin Platform serialized buckets.                                     |
| `GLEAM_PORT_LOG.md`                                       | Preserves mainline state-dump and segment history plus the HAR-498 Admin Platform pass history, and records this refresh. |

Validation: `corepack pnpm gleam:port:coverage` is green with 379 specs and 158
expected Gleam failures. `gleam test --target javascript` is green at 726 tests.
Host `gleam test --target erlang` still fails with the known local `undef`
runner issue; the Docker Erlang fallback with `HOME=/tmp` is green at 722 tests.
`corepack pnpm gleam:registry:check`, `corepack pnpm lint`, and `git diff
--check` are green.

### Findings

- Main now expects parity runner preconditions to live in a capture-shaped helper
  fold, so Admin Platform seeding belongs in that fold rather than restoring the
  older scenario-id dispatch table.
- The state-dump strictness pass must retain Admin Platform buckets in the
  serialized field inventory so missing-bucket restore coverage continues to
  include HAR-498 state.
...
## 2026-05-01 - Pass 115: Admin Platform and Segments merge refresh

Refreshes HAR-498 after `origin/main@cfa4635b` landed the Segments baseline and
member parity pass. The merge keeps Admin Platform utility/node/taxonomy parity
ungated while incorporating the latest Segment root payload storage,
customer-segment member query behavior, serialization, runner seeding, and
expected-failure gate updates from main.

| Module                                                    | Change                                                                                                        |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| `gleam/src/shopify_draft_proxy/state/store.gleam`         | Combines Admin Platform generic-node/taxonomy state with Segment root payload and member-query state helpers. |
| `gleam/src/shopify_draft_proxy/state/serialization.gleam` | Preserves both Admin Platform state dump fields and Segment root payload serialization from main.             |
| `gleam/test/parity/runner.gleam`                          | Keeps Admin Platform seeding/substitution while incorporating Segment baseline seeding from main.             |
| `config/gleam-port-ci-gates.json`                         | Preserves Admin Platform and Apps removals while accepting the Segment baseline expected-failure removal.     |
| `GLEAM_PORT_LOG.md`                                       | Preserves both parallel Pass 114 histories and records this merge-refresh pass.                               |

Validation:
`corepack pnpm gleam:port:coverage` is green with 379 specs and 157 expected
Gleam failures. `gleam test --target javascript` is green at 722 tests. Host
`gleam test --target erlang` still fails with the known local `undef` runner
issue; the Docker Erlang fallback with `HOME=/tmp` is green at 718 tests.
`corepack pnpm gleam:registry:check`, `corepack pnpm lint`, and `git diff
--check` are green.

### Findings

- Admin Platform and Segments touched the same state/store import area; the
  correct merge result keeps both resource families rather than treating either
  as superseding the other.
- The expected-failure manifest now needs to keep Admin Platform, Apps, and
  Segment baseline scenarios out of the Gleam gate.
...
## 2026-05-01 - Pass 114: Admin Platform and Apps merge refresh

Refreshes HAR-498 after `origin/main@ec8452e5` landed the Apps billing/access
parity cutover. The merge keeps Admin Platform utility/node/taxonomy parity
ungated while also preserving the new Apps node serializers, app lifecycle
state behavior, and target-to-target parity variable substitution from main.

| Module                                                     | Change                                                                                                              |
| ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `gleam/src/shopify_draft_proxy/proxy/admin_platform.gleam` | Combines HAR-498 Product/Collection/Customer/Location/taxonomy/generic Node dispatch with mainline App-owned nodes. |
| `config/gleam-port-ci-gates.json`                          | Leaves both Admin Platform and Apps billing/access scenarios out of the expected-failure gate.                      |
| `GLEAM_PORT_LOG.md`                                        | Preserves parallel Admin Platform and Apps pass history and records this merge-refresh pass.                        |

Validation:
`corepack pnpm gleam:port:coverage` is green with 379 specs and 158 expected
Gleam failures. `gleam test --target javascript` is green at 720 tests. Host
`gleam test --target erlang` still fails with the known local `undef` runner
issue; the Docker Erlang fallback with `HOME=/tmp` is green at 716 tests.
`corepack pnpm lint`, `corepack pnpm gleam:registry:check`, and `git diff
--check` are green.

### Findings

- Admin Platform and Apps both extend generic Relay Node dispatch in
  `admin_platform.gleam`; the correct merge result keeps both families and
  still lets owning resource serializers handle real resource nodes.
- The expected-failure manifest must remove both the Admin Platform entries
  completed by HAR-498 and the Apps billing/access entry completed on main.
...
## 2026-05-01 - Pass 113: Admin Platform mainline refresh

Refreshes HAR-498 after `origin/main` landed the HAR-505 Marketing and
localization seeding pass. The conflict resolution keeps the Admin Platform
node/taxonomy seeding and file-backed `fromCapturePath` variable substitution
while preserving mainline Marketing/localization parity work. It also restores
the finance/risk no-data connection roots that were exposed by the Admin
Platform pass and are now ungated in the Gleam parity manifest.

| Module                                                     | Change                                                                                                                    |
| ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `gleam/test/parity/runner.gleam`                           | Combines Admin Platform generic node/taxonomy capture seeding with mainline localization source-marker seeding.           |
| `gleam/src/shopify_draft_proxy/proxy/admin_platform.gleam` | Returns Shopify-like empty connections for finance/risk no-data roots instead of `null` after the mainline merge refresh. |
| `GLEAM_PORT_LOG.md`                                        | Preserves both parallel Pass 112 entries and records this merge-refresh pass.                                             |

Validation:
Full JavaScript is green at 718 tests. Host Erlang still fails with the known
local `undef` runner issue under the workspace OTP, and the Docker Erlang
fallback with `HOME=/tmp` is green at 714 tests. `corepack pnpm
gleam:port:coverage` is green with 379 specs and 163 expected failures.
`corepack pnpm lint`, `corepack pnpm gleam:registry:check`, and `git diff
--check` are green.

### Findings

- The mainline refresh changed the aggregate parity environment enough to expose
  `finance-risk-no-data-read` as a live gate again. The Admin Platform handler
  already returned `null` for singular finance/risk roots; plural roots needed
  explicit empty connection serialization to match the captured no-data payload.
- The two previous Pass 112 log entries were produced on parallel branches, so
  this pass leaves both intact and records the merge-specific reconciliation at
  the top.
