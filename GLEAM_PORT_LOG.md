...
## 2026-05-01 - Pass 112: bulk operations query and import parity
...
Validation:
Host `gleam test --target erlang` still compiles but fails under local
Erlang/OTP 25 because `gleam_json` requires OTP 27+. The established container
fallback is green at 717 tests with OTP 28:
...
Host `gleam test --target javascript` is green at 721 tests.
`corepack pnpm gleam:port:coverage` is green with 379 parity specs and 176
