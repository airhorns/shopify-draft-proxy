...
## 2026-05-05 - Pass 203: HAR-573 fulfillment order cancel parity guardrails
...
- `docker run --rm --user "$(id -u):$(id -g)" -e HOME=/tmp -v "$PWD":/repo -w /repo/gleam ghcr.io/gleam-lang/gleam:v1.16.0-erlang-alpine sh -lc 'erl -eval "io:format(\"OTP=~s~n\", [erlang:system_info(otp_release)]), halt()." -noshell && gleam clean && gleam test --target erlang'`
  (OTP 28, 862 passed after merging `origin/main@4e6d19c0`)
...
- `corepack pnpm build`
- `corepack pnpm test` (123 files passed; 2311 passed after merging
  `origin/main@4e6d19c0`)
- `git diff --check`
