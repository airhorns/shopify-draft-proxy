...
import { buildOperationCoverageMatrix, renderOperationCoverageMatrixMarkdown } from './operation-coverage-matrix-lib.mjs';
...
const operationMatrixJsonPath = path.join(repoRoot, 'docs', 'generated', 'operation-coverage-matrix.json');
const operationMatrixMarkdownPath = path.join(repoRoot, 'docs', 'generated', 'operation-coverage-matrix.md');
...
const allowedDomain = new Set(['products', 'customers', 'orders', 'unknown']);
...
const implementedEntries = allEntries.filter((entry) => entry.implemented);
...
const statusJson = {
  ...
  operations: allEntries.map((entry) => ({
    name: entry.name,
    type: entry.type,
...
const worklistStatusReport = [
  ...
  '',
  ...gapEntries.map((entry) => `- [-] \`${entry.name}\` — ${entry.conformance.reason}`),
  '',
];

const operationCoverageMatrix = buildOperationCoverageMatrix({
  repoRoot,
  operationRegistry,
  scenarioRegistry,
});
const operationCoverageMatrixMarkdown = renderOperationCoverageMatrixMarkdown(operationCoverageMatrix);
...
writeFileSync(operationMatrixJsonPath, JSON.stringify(operationCoverageMatrix, null, 2) + '\n');
writeFileSync(operationMatrixMarkdownPath, operationCoverageMatrixMarkdown + '\n');

console.log(`conformance coverage ok (${coveredEntries.length} covered / ${gapEntries.length} declared gaps)`);
console.log(
  `reports written to ${path.relative(repoRoot, reportPath)}, ${path.relative(repoRoot, statusJsonPath)}, ${path.relative(repoRoot, statusMarkdownPath)}, ${path.relative(repoRoot, operationMatrixJsonPath)}, and ${path.relative(repoRoot, operationMatrixMarkdownPath)}`,
);
