...
function updateEnvAccessToken(envSource, accessToken) {
  const [nextEnvSource, replacements] = envSource.replace(
    /^SHOPIFY_CONFORMANCE_ADMIN_ACCESS_TOKEN=.*$/m,
    `SHOPIFY_CONFORMANCE_ADMIN_ACCESS_TOKEN=${accessToken}`,
  ) === envSource
    ? [envSource, 0]
    : [envSource.replace(/^SHOPIFY_CONFORMANCE_ADMIN_ACCESS_TOKEN=.*$/m, `SHOPIFY_CONFORMANCE_ADMIN_ACCESS_TOKEN=${accessToken}`), 1];
...
const clientIdResolution = resolveRefreshClientId(
    ...
    JSON.stringify(
      {
        ...
        refreshFailure,
        recommendedNextStep:
          refreshFailure?.recommendedNextStep ?? 'retry-refresh-or-investigate-auth-state',
