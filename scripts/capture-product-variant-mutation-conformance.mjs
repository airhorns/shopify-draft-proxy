...
async function writeScopeBlocker(blocker) {
  ...
  const note = renderWriteScopeBlockerNote({
    ...
    whatFailed:
      'Attempted to capture live conformance for the staged bulk product variant mutation family.',
...
try {
  ...
  expectNoUserErrors('productOptionsCreate (bulk seed setup)', setupOptionsResponse.data?.productOptionsCreate?.userErrors);
