...
async function loadPersistedPublicationDeployOutcome() {
  try {
    ...
    return {
      ...
      versionLabel: typeof details?.shopifyAppDeployVersion === 'string' && details.shopifyAppDeployVersion.length > 0
...
function probeShopifyAppCliAuth(appScopeDrift) {
  ...
  const workdir = typeof appScopeDrift?.directory === 'string' && appScopeDrift.directory.length > 0
    ? appScopeDrift.directory
    : null;
...
async function loadManualStoreAuthSummary() {
  try {
    ...
    return {
      ...
      liveScopeHandles: [],
      status: summary.scopeHandles.includes('read_product_listings')
...
function summarizeManualStoreAuthSummary(manualStoreAuthSummary) {
  ...
  const liveScopeLines = (manualStoreAuthSummary.liveScopeHandles ?? manualStoreAuthSummary.scopeHandles ?? []).map((scope) => `- \`${scope}\``);
  const associatedUserScopeLines = (manualStoreAuthSummary.associatedUserScopeHandles ?? []).map((scope) => `- \`${scope}\``);
  const statusLines = manualStoreAuthSummary.status === 'scope-blocked'
    ? [
        '',
  ...
  return [
    ...
    ...(manualStoreAuthSummary.associatedUserEmail ? [`- associated user: \`${manualStoreAuthSummary.associatedUserEmail}\``] : []),
...
function buildPublicationAggregateFieldBlockerNote({
  ...
  const blockerLines = [
    publishAggregateBlocker
      ? `- publish mutation aggregate slice → ${publishAggregateBlocker.message}`
      : null,
    publishReadBlocker
      ? `- post-publish downstream read → ${publishReadBlocker.message}`
      : null,
    unpublishAggregateBlocker
      ? `- unpublish mutation aggregate slice → ${unpublishAggregateBlocker.message}`
      : null,
    unpublishReadBlocker
      ? `- post-unpublish downstream read → ${unpublishReadBlocker.message}`
      : null,
  ].filter(Boolean);
  const channelConfigExtension = Array.isArray(appScopeDrift?.channelConfigExtensions)
    ? appScopeDrift.channelConfigExtensions[0] ?? null
    : null;
  const deployObservationLines = appDeployOutcome?.status === 'deployed-but-still-scope-blocked'
    ? [
...
async function collectPublicationMutationScopeProbe(client, runId) {
  ...
  try {
    const createResponse = await client.runGraphql(createMutation, buildCreateVariables(`${runId}-mutation-scope-probe`));
...
async function collectPublicationProbe(client, runId) {
  ...
  const publicationId = listProbe.payload?.data?.publications?.edges?.[0]?.node?.id
...
async function refreshCliIdentity(identity) {
  ...
  return {
    ...
    identity: {
      ...
      expiresAt: new Date(Date.now() + (typeof payload.expires_in === 'number' ? payload.expires_in : 0) * 1000).toISOString(),
    },
  };
}
...
async function tryShopifyCliPublicationFallback() {
  ...
  try {
    ...
    return {
      ...
      credentialSummary: 'a refreshed Shopify CLI account bearer token temporarily provided the required publication probe access',
      probe,
    };
  } catch (error) {
    return {
