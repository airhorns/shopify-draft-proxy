...
try {
  ...
  if (!productId || !inventoryItemId || !variantId) {
    throw new Error(`Inventory item update capture failed to create a usable temporary product: ${JSON.stringify(createResult, null, 2)}`);
  }
  ...
  await rm(path.join('pending', 'inventory-item-update-conformance-scope-blocker.md'), { force: true }).catch(() => null);
}
