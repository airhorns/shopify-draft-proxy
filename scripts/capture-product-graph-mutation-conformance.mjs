...
try {
  ...
  if (!locationId) {
    throw new Error('Product graph capture could not resolve a writable location id for productSet inventoryQuantities.');
  }
  ...
  expectNoUserErrors('productDuplicate handle collision', duplicateCollisionResponse.data?.productDuplicate?.userErrors);
