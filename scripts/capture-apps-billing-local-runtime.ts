...
const appRevokeAccessScopesErrorCodesFixturePath = path.join(
  repoRoot,
  'fixtures',
  'conformance',
  'local-runtime',
...
async function readExistingFixturePath(filePath: string): Promise<JsonRecord> {
  return readObject(JSON.parse(await readFile(filePath, 'utf8')) as unknown, filePath);
}
...
async function writeFixturePath(filePath: string, fixture: JsonRecord): Promise<void> {
...
async function captureRevokeAccessScopesErrorCodes(): Promise<void> {
  const scenario = 'app-revoke-access-scopes-error-codes';
  const existing = await readExistingFixturePath(appRevokeAccessScopesErrorCodesFixturePath);
  const proxy = await createProxy();
  try {
    const missingSourceApp = assertOk(
      await proxy.processGraphQLRequest(
        { query: await readGraphql('appRevokeAccessScopes-error-codes.graphql') },
        {
          apiVersion,
          headers: {
            'x-shopify-draft-proxy-source-app-missing': 'true',
          },
        },
      ),
      'missing source app',
    ...
    await writeFixturePath(appRevokeAccessScopesErrorCodesFixturePath, {
      capturedAt: existing['capturedAt'],
      ...
      scenario,
      notes: [
        'Local-runtime fixture for appRevokeAccessScopes error-code parity. Live conformance probe on harry-test-heelo.myshopify.com succeeded for 2025-01, and safe live probes confirmed UNKNOWN_SCOPES and CANNOT_REVOKE_REQUIRED_SCOPES wording.',
        'Public Admin requests with a valid access token provide source app context, while unauthenticated requests fail before the resolver, so the missing-source-app payload is Core-source-derived rather than real-Shopify-recorded.',
        'Core source contract: AppRevokeScopeError maps non-scope errors, including MISSING_SOURCE_APP, to field id; the missing_source_app i18n message is No app found on the access token.',
        `Regenerated by scripts/capture-apps-billing-local-runtime.ts for ${scenario}.`,
      ],
      operationNames: ['appRevokeAccessScopes'],
      scopeCatalog: existing['scopeCatalog'],
      evidence: {
        parity: {
          expected: {
            missingSourceApp,
