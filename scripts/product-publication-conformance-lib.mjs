...
export function extractCliIdentityFromConfig(config) {
  ...
  const sessionId = activeSessionId && sessionIds.includes(activeSessionId)
    ? activeSessionId
    : sessionIds.length === 1
      ? sessionIds[0]
      : null;
...
export function extractManualStoreAuthTokenSummary(payload) {
  ...
  const scopeHandles = typeof payload.scope === 'string'
    ? payload.scope.split(',').map((scope) => scope.trim()).filter(Boolean)
    : [];
  const associatedUserScopeHandles = typeof payload.associated_user_scope === 'string'
    ? payload.associated_user_scope.split(',').map((scope) => scope.trim()).filter(Boolean)
    : [];
