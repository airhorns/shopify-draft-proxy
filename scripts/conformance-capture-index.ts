...
export const conformanceCaptureIndex = defineCaptureIndex([
  ...
  },
  {
    ...
    purpose:
      'Web presence create/update/delete lifecycle, downstream top-level webPresences reads, multi-locale rootUrls, duplicate and non-letter subfolder-suffix validation, and primary-domain delete guard.',
    requiredAuthScopes: ['read_markets', 'write_markets'],
    fixtureOutputs: [
    ...
    cleanupBehavior:
      'Records the primary-domain webPresenceDelete guard without cleanup because it must fail, creates one disposable subfolder web presence, updates it, deletes it, records one multi-locale disposable web presence with subfolder suffix intl, records one partial alternate-locale-only update, records non-letter subfolder suffix validation and valid-us control cases, deletes all disposable web presences, and verifies the baseline read after cleanup.',
    expectedStatusChecks: DEFAULT_STATUS_CHECKS,
  },
  {
    domain: 'markets',
