...
export const conformanceCaptureIndex = defineCaptureIndex([
  ...
  },
  {
    ...
    purpose:
      'LiveHybrid products/productsCount parity for a complete upstream catalog overlaid with staged product create, update, and delete effects.',
    requiredAuthScopes: ['read_products', 'write_products'],
    fixtureOutputs: [
    ...
    cleanupBehavior:
      'Creates three disposable base products, one staged-create comparison product, and one PRODUCT saved search; records pre-mutation hydrate cassettes, captures create/update/delete reads, then removes all disposable resources.',
    expectedStatusChecks: DEFAULT_STATUS_CHECKS,
    notes:
      'The generated parity spec replays every mutation and read through public GraphQL requests while exact pre-mutation Shopify hydrates provide the authoritative base catalog.',
  },
  {
    domain: 'products',
