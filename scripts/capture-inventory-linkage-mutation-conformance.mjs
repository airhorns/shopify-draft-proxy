...
async function createTemporaryProduct(title) {
  ...
  if (!product?.id || !variant?.id || !inventoryItem?.id || !inventoryLevel?.id) {
    throw new Error(`Unexpected productCreate payload for inventory linkage capture: ${JSON.stringify(created, null, 2)}`);
  }
...
`;
  ...
  try {
    ...
    const bulkDeactivateOnlyLocationError = await runGraphql(inventoryBulkToggleMutation, bulkDeactivateOnlyLocationVariables);
    ...
    const fixture = {
      ...
      downstreamReadAfterInventoryDeactivateWithAlternateLocation:
        downstreamReadAfterDeactivateWithAlternateLocation,
      ...
      downstreamReadAfterInventoryBulkToggleActivateSecondLocation:
        downstreamReadAfterBulkActivateSecondLocation,
      ...
      downstreamReadAfterInventoryBulkToggleDeactivateSecondLocation:
        downstreamReadAfterBulkDeactivateSecondLocation,
    };
