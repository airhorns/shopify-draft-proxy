...
const fulfillmentTrackingInfoUpdateInlineMissingIdFixturePath = path.join(fixtureDir, 'fulfillment-tracking-info-update-inline-missing-id.json');
const fulfillmentTrackingInfoUpdateInlineNullIdFixturePath = path.join(fixtureDir, 'fulfillment-tracking-info-update-inline-null-id.json');
const fulfillmentTrackingInfoUpdateMissingIdFixturePath = path.join(fixtureDir, 'fulfillment-tracking-info-update-missing-id.json');
...
const draftOrderCreateInlineMissingInputFixturePath = path.join(fixtureDir, 'draft-order-create-inline-missing-input.json');
...
const draftOrderCompleteInlineMissingIdFixturePath = path.join(fixtureDir, 'draft-order-complete-inline-missing-id.json');
...
function summarizeCredential(token) {
  ...
  return {
    ...
    summary: "the active conformance credential is being sent through the repo's bearer-token fallback path on this host",
  };
}
...
async function readManualStoreAuthSummary() {
  try {
    ...
    const status = tokenFamily === 'shpca'
      ? 'present-shpca-user-token-not-offline-capable'
      : 'present-non-shpca-token';
...
function renderManualStoreAuthSection(manualStoreAuthSummary) {
  const cachedScopes = manualStoreAuthSummary.cachedScopes.length > 0
    ? manualStoreAuthSummary.cachedScopes.map((scope) => `\`${scope}\``).join(', ')
    : 'none recorded';
  const associatedUserScopes = manualStoreAuthSummary.associatedUserScopes.length > 0
    ? manualStoreAuthSummary.associatedUserScopes.map((scope) => `\`${scope}\``).join(', ')
    : 'none recorded';
  const interpretation = manualStoreAuthSummary.status === 'present-shpca-user-token-not-offline-capable'
...
async function readOrderCreationParityMetadata(credential, manualStoreAuthSummary) {
  const [
    ...
    refreshOrderDomainBlockerSpec(path.join('config', 'parity-specs', 'draftOrderComplete-parity-plan.json'), credential, manualStoreAuthSummary),
    refreshOrderDomainBlockerSpec(path.join('config', 'parity-specs', 'draftOrders-read-parity-plan.json'), credential, manualStoreAuthSummary),
    refreshOrderDomainBlockerSpec(path.join('config', 'parity-specs', 'draftOrdersCount-read-parity-plan.json'), credential, manualStoreAuthSummary),
    refreshOrderDomainBlockerSpec(path.join('config', 'parity-specs', 'orderEditBegin-parity-plan.json'), credential, manualStoreAuthSummary),
    refreshOrderDomainBlockerSpec(path.join('config', 'parity-specs', 'orderEditAddVariant-parity-plan.json'), credential, manualStoreAuthSummary),
    refreshOrderDomainBlockerSpec(path.join('config', 'parity-specs', 'orderEditSetQuantity-parity-plan.json'), credential, manualStoreAuthSummary),
    refreshOrderDomainBlockerSpec(path.join('config', 'parity-specs', 'orderEditCommit-parity-plan.json'), credential, manualStoreAuthSummary),
    refreshOrderDomainBlockerSpec(path.join('config', 'parity-specs', 'fulfillmentTrackingInfoUpdate-parity-plan.json'), credential, manualStoreAuthSummary),
    refreshOrderDomainBlockerSpec(path.join('config', 'parity-specs', 'fulfillmentCancel-parity-plan.json'), credential, manualStoreAuthSummary),
  ]);
...
function getAuthFailureMessage(result) {
  ...
  if (result.status === 401) {
    return result.payload?.errors?.[0]?.message
      ?? result.payload?.errors
      ?? '401 Unauthorized';
  }

  const errors = result.payload?.errors;
  if (typeof errors === 'string' && /Invalid API key or access token|Service is not valid for authentication/i.test(errors)) {
    return errors;
  }

  if (Array.isArray(errors)) {
    const authError = errors.find((error) => /Invalid API key or access token|Service is not valid for authentication/i.test(error?.message ?? ''));
...
function formatRequiredAccessSummary(blockerDetails, liveError, defaultMessage = 'missing requiredScopes blocker metadata') {
  ...
  const liveRequiredAccess = typeof liveError?.extensions?.requiredAccess === 'string'
...
async function main() {
  ...
  const fulfillmentCreateInvalidIdResult = await runGraphql(fulfillmentCreateInvalidIdProbe, fulfillmentCreateInvalidIdVariables);
  ...
  const draftOrderCompleteMissingIdResult = await runGraphql(draftOrderCompleteMissingIdProbe, draftOrderCompleteMissingIdVariables);
  const draftOrderCompleteResult = await runGraphql(draftOrderCompleteProbe, draftOrderCompleteVariables);
  const orderEditBeginMissingIdResult = await runGraphql(orderEditBeginMissingIdProbe, orderEditBeginMissingIdVariables);
  const orderEditBeginResult = await runGraphql(orderEditBeginProbe, orderEditBeginVariables);
  const orderEditAddVariantMissingIdResult = await runGraphql(orderEditAddVariantMissingIdProbe, orderEditAddVariantMissingIdVariables);
  const orderEditAddVariantResult = await runGraphql(orderEditAddVariantProbe, orderEditAddVariantVariables);
  const orderEditSetQuantityMissingIdResult = await runGraphql(orderEditSetQuantityMissingIdProbe, orderEditSetQuantityMissingIdVariables);
  const orderEditSetQuantityResult = await runGraphql(orderEditSetQuantityProbe, orderEditSetQuantityVariables);
  const orderEditCommitMissingIdResult = await runGraphql(orderEditCommitMissingIdProbe, orderEditCommitMissingIdVariables);
  ...
  const orderReadAfterCreateResult = createdOrderId ? await runGraphql(orderReadAfterCreate, { id: createdOrderId }) : null;
  ...
  const orderUpdateLiveResult = orderUpdateLiveVariablesForRun
    ...
    : null;
  const orderReadAfterUpdateResult = createdOrderId ? await runGraphql(orderReadAfterUpdate, { id: createdOrderId }) : null;
  const draftOrderDetailResult = createdDraftOrderId ? await runGraphql(draftOrderDetailRead, { id: createdDraftOrderId }) : null;
  ...
  const draftOrdersInvalidEmailQueryResult = await runGraphql(draftOrdersInvalidEmailQueryRead, draftOrdersInvalidEmailQueryVariables);

  const authFailureMessage = getAuthFailureMessage(orderEmptyStateResult)
...
`;
  ...
  if (authRegressed) {
    const draftOrdersCatalogLastVerified = 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/draft-orders-catalog.json';
    const draftOrdersCountLastVerified = 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/draft-orders-count.json';
