...
function findForbiddenShippingFulfillmentEvidence(): string[] {
  ...
  for (const specPath of collectJsonFiles('config/parity-specs/shipping-fulfillments')) {
  ...
  for (const fixturePath of collectJsonFiles('fixtures/conformance').filter((filePath) =>
