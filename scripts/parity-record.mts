...
// upstream, plays the parity spec's primary + target requests through it, and
// rewrites the capture file with an `upstreamCalls` cassette: every upstream
// GraphQL call the proxy makes while serving the spec, in order, with its
// (operationName, variables, response) tuple.
//
...
// be replayed later by the Rust parity runner.
//
// Usage:
...
async function startRecordingUpstream(opts: {
    ...
    request.on('end', () => {
      void (async () => {
        try {
          ...
          const upstreamResponse = await fetch(`${opts.adminOrigin}/admin/api/${opts.apiVersion}/graphql.json`, {
            method: 'POST',
            headers: {
          ...
          const responsePayload = responseText.length > 0 ? JSON.parse(responseText) : {};
          opts.calls.push({
  ...
  if (address === null || typeof address === 'string') {
    throw new Error('Failed to start recording upstream server.');
  }
  return {
...
async function recordSpec(opts: RecordOptions): Promise<void> {
  ...
  let recordingUpstream: { close: () => Promise<void> } | null = null;
  try {
