...
try {
  ...
  if (isProtectedCustomerDataError(catalogResult)) {
    const failingMessage = extractGraphqlError(catalogResult)?.message || 'Protected customer data approval is missing.';
    ...
    console.error(JSON.stringify({ ok: false, blockerPath, message: failingMessage, docsUrl: protectedCustomerDataDocsUrl }, null, 2));
    process.exit(1);
  }
  ...
  await writeFile(path.join(outputDir, 'customers-advanced-search.json'), `${JSON.stringify(advancedSearch, null, 2)}\n`, 'utf8');
  await writeFile(path.join(outputDir, 'customers-sort-keys.json'), `${JSON.stringify(sortKeys, null, 2)}\n`, 'utf8');
  await writeFile(path.join(outputDir, 'customers-relevance-search.json'), `${JSON.stringify(relevanceSearch, null, 2)}\n`, 'utf8');
