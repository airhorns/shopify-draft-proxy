...
fn captured_json_value_decoder() -> Decoder(types.CapturedJsonValue) {
  decode.recursive(fn() {
    decode.one_of(decode.bool |> decode.map(types.CapturedBool), or: [
      decode.int |> decode.map(types.CapturedInt),
      decode.float |> decode.map(types.CapturedFloat),
      decode.string |> decode.map(types.CapturedString),
      decode.list(of: captured_json_value_decoder())
        |> decode.map(types.CapturedArray),
      decode.dict(decode.string, captured_json_value_decoder())
        |> decode.map(fn(fields) { types.CapturedObject(dict.to_list(fields)) }),
      decode.success(types.CapturedNull),
    ])
  })
}
