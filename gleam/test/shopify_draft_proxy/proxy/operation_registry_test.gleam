...
pub fn default_registry_marks_only_ported_roots_as_locally_dispatched_test() {
  ...
  let assert Some(fulfillment_orders) =
    operation_registry.find_entry(entries, Query, [Some("fulfillmentOrders")])
  assert !draft_proxy.registry_entry_has_local_dispatch(fulfillment_orders)
