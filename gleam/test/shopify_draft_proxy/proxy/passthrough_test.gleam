...
fn unported_registry_request() -> Request {
  // `bulkOperationRunMutation` is an implemented TypeScript registry root,
  // but mutation-import execution is not yet ported to Gleam.
  ...
  Request(
    ...
    path: "/admin/api/2025-01/graphql.json",
    headers: dict.new(),
    body: "{\"query\":\"mutation { bulkOperationRunMutation(mutation: \\\"\\\", stagedUploadPath: \\\"\\\") { bulkOperation { id } userErrors { field message } } }\"}",
  )
}
