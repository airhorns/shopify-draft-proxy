...
import shopify_draft_proxy/state/types.{
  ...
  type DiscountRecord, type GiftCardConfigurationRecord,
  ...
  DiscountRecord, GiftCardConfigurationRecord, GiftCardRecipientAttributesRecord,
  GiftCardRecord, GiftCardTransactionRecord, InventoryItemRecord,
  InventoryLevelRecord, InventoryLocationRecord, InventoryMeasurementRecord,
  InventoryQuantityRecord, InventoryWeightFloat, InventoryWeightInt,
  InventoryWeightRecord, LocationRecord, MetafieldDefinitionCapabilitiesRecord,
...
fn seed_capture_preconditions(
  ...
  let proxy = seed_discount_preconditions(parsed.scenario_id, capture, proxy)
  ...
  case parsed.scenario_id {
    ...
    "discount-app-bulk-local-runtime" ->
      seed_shopify_function_records(capture, proxy)
    "shop-baseline-read"
...
fn seed_discount_preconditions(
  ...
  let discount_sources = case jsonpath.lookup(capture, "$.seedDiscounts") {
    ...
    _ ->
      case string.contains(scenario_id, "lifecycle") {
        True -> []
        False -> collect_objects(capture)
      }
  }
  let discounts =
    discount_sources
...
fn seed_shopify_function_preconditions(
  ...
  let proxy = seed_shopify_function_records(capture, proxy)
