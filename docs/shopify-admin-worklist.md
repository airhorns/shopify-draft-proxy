...
5. Every implemented root operation must stay in sync with `config/operation-registry.json`; `pnpm conformance:check` is the structural gate for runtime-test coverage plus convention-discovered parity specs.
6. Do not hand-maintain central scenario manifests. The scenario-to-operation mapping lives in each `config/parity-specs/*.json` file.
...
- [x] `fileCreate` (staged locally for store-level Files API inputs, separate from product media attachments)
- [ ] broader media create family
- [ ] media update family
- [ ] media delete family
