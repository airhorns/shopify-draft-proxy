...
- Staged app-managed automatic discounts use the same Admin GraphQL automatic lifecycle roots as native automatic discounts. `discountAutomaticDeactivate` expires the local app discount, `discountAutomaticActivate` reactivates it, and `discountAutomaticDelete` removes it from downstream automatic and aggregate catalog reads while preserving the staged deletion marker for meta state and commit replay.
