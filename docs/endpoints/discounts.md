...
- Native discount creates and updates validate product, product-variant, and collection item references before staging. Product and variant IDs under `productsToAdd`, plus collection IDs under `collections.add`, reject `/0` sentinels and unknown IDs once the local product/collection state is authoritative through staged state or LiveHybrid hydration. BXGY applies the same reference checks to `customerBuys.items`. Captured 2026-04 behavior accepts non-entitled `productsToRemove`, `productVariantsToRemove`, and `collections.remove` references as no-op updates, including `Collection/0`, so local update handling preserves existing item entitlements for remove-only inputs rather than returning reference userErrors.
...
- `scripts/capture-discount-items-refs-validation-conformance.ts` creates two temporary products and one collection, captures discount item reference validation for unknown product, variant, and collection IDs, collection `/0` sentinels, BXGY `customerBuys` invalid product IDs, and successful product/variant/collection references, then deletes the temporary resources and created discounts after capture.
