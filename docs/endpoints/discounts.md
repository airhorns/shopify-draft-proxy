...
- HAR-366 app/bulk local parity lives in `config/parity-specs/discount-app-bulk-local-runtime.json` with the local-runtime fixture `fixtures/conformance/local-runtime/2026-04/discount-app-bulk-local-runtime.json`. It compares app-discount create/update payloads, completed bulk job payloads, downstream app discount reads, downstream count/status/absence reads, and Function metadata-only ownership evidence through the generic `conformance:parity` runner.
