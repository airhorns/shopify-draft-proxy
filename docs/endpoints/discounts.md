...
- Staged app-managed discounts use the same Admin GraphQL activate/deactivate/delete lifecycle roots as native discounts. Activation first verifies that the discount's captured `appDiscountType.functionId` still resolves to local `ShopifyFunction` metadata; missing Function ownership returns `Discount could not be activated.` with `field: ["base"]` and code `INTERNAL_ERROR`, and leaves local discount state unchanged.
- `discountCodeActivate` and `discountAutomaticActivate` are no-ops for stored already-active records: they return the existing payload unchanged, do not synthesize timestamps, and do not stage replayable state. `discountCodeDeactivate` and `discountAutomaticDeactivate` are the same kind of no-op for stored already-expired records, preserving existing `updatedAt`, `startsAt`, and `endsAt`. Non-no-op deactivations expire local discounts by setting `endsAt` to the current synthetic timestamp and moving missing/future `startsAt` to that same timestamp; non-no-op activations clear stale elapsed `endsAt` values and move missing/future `startsAt` to the current synthetic timestamp. Unknown activate/deactivate IDs return an `INVALID` userError code.
- `discountCodeDelete` and `discountAutomaticDelete` first resolve the effective local discount record. Unknown delete IDs do not stage a deletion: the deleted ID field is `null` and Shopify 2026-04 returns type-specific `INVALID` userErrors (`Code discount does not exist.` / `Automatic discount does not exist.`). Once a local record resolves, delete is deterministic and always succeeds in the proxy; Shopify's underlying destroy-failure `INTERNAL_ERROR` branch is not modelled because local in-memory deletion has no backend failure mode to simulate.
- Native code and automatic discount creates/updates derive `status` from the effective `startsAt` / `endsAt` window against the proxy's deterministic synthetic clock: future starts are `SCHEDULED`, elapsed ends are `EXPIRED`, and currently visible windows are `ACTIVE`. The derived value is stored in normalized `DiscountRecord` state, so downstream `discountNode`, `codeDiscountNode`, `automaticDiscountNode`, catalog reads, and `status:` filters observe the same status. Explicit activate/deactivate and bulk status overrides continue to update the stored status, so subsequent reads preserve those lifecycle transitions.
- `discountRedeemCodeBulkAdd`, `discountRedeemCodeBulkCreation(id:)`, and the introspected `discountCodeRedeemCodeBulkDelete` root stage locally for code-basic redeem-code changes. Bulk add rejects unknown discount IDs, empty code arrays, and more than 250 code inputs before staging. For schema-shaped `DiscountRedeemCodeInput` arrays, Shopify accepts per-code empty/newline/too-long/duplicate failures into an async `DiscountRedeemCodeBulkCreation`; the proxy mirrors that by returning a pending mutation payload and exposing the final per-code `errors` through `discountRedeemCodeBulkCreation(id:)`. Legacy local-runtime string-code arrays keep the completed bulk payload shape used by earlier tests. Accepted codes append stable local `DiscountRedeemCode` rows, record durable `discountBulkOperations` state, and become visible through detail `codes`, `codesCount`, `codeDiscountNodeByCode`, and catalog reads. Redeem-code bulk delete validates its mutually exclusive `ids` / `search` / `savedSearchId` selector flow before staging, rejects unknown discounts after Pattern 2 hydration, removes matching local redeem-code rows, and returns a completed `Job` shape for selected fields such as `id`, `done`, and `query`.
...
- Code-basic updates include update-only guardrails: unknown update IDs return `field: ["id"]`, message `Discount does not exist`, and code `INVALID`; any `code` field on discounts that have become redeem-code bulk rules is rejected locally before staging. The HAR-605 live capture records that the public 2026-04 conformance store still returned `code: null` for these two error branches, while the local proxy follows the internal Shopify source path and ticketed `INVALID` contract.
- Discount update roots coerce the normalized discount type to the target root's type. For example, `discountCodeBasicUpdate` applied to a staged `DiscountCodeBxgy` record rewrites it to `DiscountCodeBasic`, matching the HAR-605 live `__typename` capture.
...
- Basic code and automatic creates/updates validate `customerGets.value` bounds before staging. Captured 2026-04 behavior rejects `percentage` values below `0.0` or above `1.0` with `VALUE_OUTSIDE_RANGE`, rejects negative fixed `discountAmount.amount` values with `LESS_THAN_OR_EQUAL_TO`, accepts zero percentage and zero fixed amounts, and rejects non-numeric Decimal variables as top-level `INVALID_VARIABLE` errors before resolver execution.
- Staged basic discounts persist `allocationMethod`: percentage discounts inferred as `PRODUCT` class expose `EACH`, while all other staged basic value/class combinations expose `ACROSS`.
...
- Subscription-only discount fields are gated by the effective shop's `features.sellsSubscriptions` capability. Missing synthetic shop state defaults to non-subscription. Tests and parity fixtures that model a subscription shop can stage or configure a shop with `features.sellsSubscriptions: true` (the shared store helper `set_shop_sells_subscriptions` is the narrow test path). In LiveHybrid, discount creates that supply gated fields and have no local shop state issue a cassette-backed `shop { features { sellsSubscriptions } }` read before local validation. On non-subscription shops, code/automatic basic `customerGets.appliesOnSubscription`, `customerGets.appliesOnOneTimePurchase`, and top-level `recurringCycleLimit` return `INVALID` `DiscountUserError`s; code free-shipping top-level `appliesOnSubscription`, `appliesOnOneTimePurchase`, and `recurringCycleLimit` do the same. Explicit null subscription booleans are still `_not_permitted` on non-subscription shops and return the Shopify blank form (for example, `applies_on_subscription can't be blank`) when subscriptions are enabled. `freeShippingAutomaticDiscount` intentionally skips this validation carve-out.
- Discount `context.markets` inputs are gated by the effective shop's `features.discountsByMarketEnabled` capability. Missing synthetic shop state defaults to disabled, while LiveHybrid requests that include `context.markets` attempt a cassette-backed `shop { features { sellsSubscriptions discountsByMarketEnabled } }` read before local validation. The combined capability query keeps markets and subscription validation from defaulting each other to disabled when a single input uses both gated fields. Disabled shops return `field: [<input>, "context", "markets"]`, `code: "INVALID"`, and `Discounts by market is not supported for this shop`; create roots also reject non-null `context.markets.remove` with `field: [<input>, "context", "markets", "remove"]`. Market IDs in `add` and `remove` must be structurally valid positive `gid://shopify/Market/<id>` values, and when local market state exists they must resolve to the effective staged/base market catalog. The live conformance store's public 2025-01 and 2026-04 Admin schemas did not expose `DiscountContextInput.markets` or `ShopFeatures.discountsByMarketEnabled` when this behavior was implemented, so no synthetic parity fixture was checked in for this private/internal schema behavior.
...
- Captured validation branches split into top-level GraphQL errors and mutation-scoped `DiscountUserError` payloads:
  ...
  - `customerGets.value` inputs with more than one of `percentage`, `discountAmount`, or `discountOnQuantity` return top-level `BAD_REQUEST` errors with the affected mutation root set to `null`
  - code-discount `customerSelection.all: true` with `customers` returns a top-level `BAD_REQUEST` error using Shopify's public 2026-04 message `A discount cannot have customerSelection set to all, when customers or customerSavedSearches is specified.`; `customerSelection.all: true` with `customerSegments` returns `A discount cannot have customerSelection set to all, when customerSegments is specified.`. Public Admin GraphQL does not expose `customerSavedSearches` on `DiscountCustomerSelectionInput`, so variable-bound inputs using that field fail earlier with `INVALID_VARIABLE`.
  - non-numeric Decimal variables such as `discountAmount.amount: "abc"` return top-level `INVALID_VARIABLE` errors before mutation execution
  - basic code and automatic create/update roots reject a single `customerGets.value.discountOnQuantity` branch with `field: [<input>, "customerGets", "value", "discountOnQuantity"]`, `code: "INVALID"`, and the captured 2026-04 message `discountOnQuantity field is only permitted with bxgy discounts.`
...
- Code-discount create roots reject unusable `code` values before staging. Captured 2026-04 behavior returns `BLANK` / `Code can't be blank` for omitted `code`, `TOO_SHORT` / `Code is too short (minimum is 1 character)` for an empty string, `TOO_LONG` for values longer than 255 characters, and `INVALID` / `Code cannot contain newline characters.` for CR/LF values. The local proxy also treats whitespace-only codes as blank to avoid staging non-usable redeem codes.
...
- Code-basic lifecycle tests cover create-read-update-read-activate/deactivate-delete flows, customer/segment buyer-context links, discount-class inference, value-bounds validation, meta log/state inspection, and commit replay mapping from synthetic IDs to authoritative Shopify IDs for later staged mutations. Redeem-code bulk tests cover add/delete read-after-write behavior, completed local job payloads, broad destructive local refusal, safe id-scoped bulk staging, unsupported passthrough observability, and commit replay order. The live 2026-04 capture fixture `discount-code-basic-lifecycle.json` now runs through executable `captured-vs-proxy-request` parity for create, successful update with Shopify's current `discountAmount` input field, deactivate, activate, delete, and downstream reads. The live 2026-04 fixtures `discount-buyer-context-lifecycle.json`, `discount-class-inference.json`, and `discount-value-bounds.json` add executable code-basic parity for customer buyer selection, segment buyer selection, all/product/collection discount class inference, value-bounds validation, downstream `discountNode` / `codeDiscountNodeByCode` reads, cleanup userErrors, and `discount_class:product` count behavior. These parity contracts compare deterministic selected fields while runtime tests continue to cover synthetic IDs/timestamps, generated summaries, singular local `discountClass`, `allocationMethod`, and commit replay. Do not treat the remaining validation guardrails as full support for BXGY, app-discount, or broad bulk job happy paths.
...
- HAR-783 app-discount Function validation parity lives in `config/parity-specs/discounts/discount-app-function-validation.json` with the live fixture `fixtures/conformance/harry-test-heelo.myshopify.com/2026-04/discounts/discount-app-function-validation.json`. It captures missing identifier, multiple identifiers, unknown `functionId`, wrong Function API through the released cart-transform Function handle, missing automatic identifier, and unknown automatic `functionHandle` branches. The active conformance app does not expose a discount Function, so the fixture is validation-only and creates no app discounts.
- HAR-438 redeem-code live parity lives in `config/parity-specs/discounts/discount-redeem-code-bulk.json` with the fixture `fixtures/conformance/harry-test-heelo.myshopify.com/2026-04/discounts/discount-redeem-code-bulk.json`. It covers redeem-code bulk add/delete payloads, exact and lowercase `codeDiscountNodeByCode` lookup, code-count read-after-write behavior, and removed-code null lookup after id-scoped bulk delete. HAR-784 validation parity lives in `config/parity-specs/discounts/discount-redeem-code-bulk-add-validation.json` with the fixture `fixtures/conformance/harry-test-heelo.myshopify.com/2026-04/discounts/discount-redeem-code-bulk-add-validation.json`; it covers unknown discount IDs, empty code arrays, the 250-code GraphQL limit, schema-shaped async per-code validation, final bulk-creation reads, and accepted-code downstream visibility.
- HAR-785 selector-validation parity lives in `config/parity-specs/discounts/discount-redeem-code-bulk-delete-validation.json` with the fixture `fixtures/conformance/harry-test-heelo.myshopify.com/2026-04/discounts/discount-redeem-code-bulk-delete-validation.json`. It covers missing selector, too many selectors, unknown `discountId`, empty `ids`, blank `search`, unknown `savedSearchId`, and an id-scoped happy-path delete baseline. The captured 2026-04 Admin response serializes the two base selector errors and the generic empty-ids error with `field: null`; empty ids also returns `code: null`.
- HAR-605 update-edge live parity lives in `config/parity-specs/discounts/discount-update-edge-cases.json` with the fixture `fixtures/conformance/harry-test-heelo.myshopify.com/2026-04/discounts/discount-update-edge-cases.json`. It covers the redeem-code bulk-rule code-change rejection after lookup-visible bulk processing, BXGY-to-basic update coercion, and unknown-id update errors.
...
- Future discount work should expand live app-discount success fixtures once a discount Function exists and capture more complex redeem-code search/saved-search deletion semantics beyond the current local code matcher.
...
- `scripts/capture-discount-validation-conformance.ts` creates a temporary native `DiscountCodeBasic` only to settle the duplicate-code branch, captures representative validation failures plus code-required guardrails for code-basic/BXGY/free-shipping creates, and deletes the seed discount immediately.
- `scripts/capture-discount-customer-gets-value-multiple-types-conformance.ts` captures the top-level `BAD_REQUEST` response for multi-branch `customerGets.value` inputs on basic create and update; it creates one disposable code-basic discount only to exercise the update path, then deletes it immediately.
- `scripts/capture-discount-customer-selection-internal-conflicts-conformance.ts` captures code-basic `customerSelection.all` conflicts with customers, public-schema saved-search input coercion, customer segments, and a customers-only happy path against Admin GraphQL 2026-04; it deletes the disposable customer, segment, and happy-path discount during cleanup.
- `scripts/capture-discount-basic-disallowed-discount-on-quantity-conformance.ts` creates one disposable basic code discount and one disposable basic automatic discount, captures rejected `customerGets.value.discountOnQuantity` create/update branches for both families, and deletes the setup discounts immediately.
...
- `scripts/capture-discount-update-edge-cases-conformance.ts` creates temporary products plus native code-basic and code-BXGY discounts, captures update-only edge cases, waits for redeem-code bulk additions to be query-visible before probing the bulk-rule code-change rejection, and deletes all created resources during cleanup.
- `scripts/capture-discount-status-time-window-derivation-conformance.ts` creates scheduled, expired, and active native `DiscountCodeBasic` records with stable time windows, captures create payload status plus downstream read/filter status behavior, and deletes the temporary discounts immediately after capture.
