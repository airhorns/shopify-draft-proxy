...
- Price-list validation is intentionally conservative until broader live write fixtures exist. Local branches cover blank/duplicate names, invalid currencies, unknown price-list/catalog/product/variant IDs, duplicate fixed-price additions, deleting/updating missing fixed prices, product-scoped variant mismatches, and the captured quantity pricing/rule unknown ID branches. The HAR-391 lifecycle test verifies that `priceListFixedPricesByProductUpdate` rejects a variant from a different product and leaves the target product's staged fixed price intact. Permission/access-scope failures and richer B2B catalog/contextual product-variant pricing payloads remain documented gaps.
