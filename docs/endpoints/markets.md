...
- Captured validation parity currently covers safe no-side-effect branches for blank `marketCreate` names, schema-current `status`/`enabled` mismatches, and unknown IDs for `marketUpdate`/`marketDelete`. Admin GraphQL 2026-04 accepts `MarketStatus` values `ACTIVE` and `DRAFT`; `INACTIVE` is rejected by enum validation before the resolver-level status/enabled guard. Local `marketCreate` also models plan-limit, base-currency, and duplicate country-region guardrails before staging. Additional success-path conformance should use disposable market setup/cleanup before touching shared buyer-facing market configuration.
