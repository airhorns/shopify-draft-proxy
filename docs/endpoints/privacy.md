...
- `dataSaleOptOut` sanitizes the email before validation by stripping Shopify Core's customer email whitespace-removal set anywhere in the string, narrowed to the public Admin API behavior captured for this project: SP, NBSP, U+2000 through U+200A spaces, NNBSP, MMSP, IDEOGRAPHIC SPACE, ZWSP/ZWNJ/ZWJ, WORD JOINER, BOM, MVS, BRAILLE PATTERN BLANK, HANGUL FILLER, the visible space symbols U+2420/U+2422/U+2423, NUL, LF, VT, and CR. For example, `her mes@example.com` and `her mes@example.com` are validated and staged as `hermes@example.com`; if stripping leaves an empty value or the remaining value does not match the email validator approximation, the mutation returns the captured `FAILED` userError and stages nothing. Live 2025-01 capture rejected tab characters and U+FFFD replacement characters with the same `FAILED` shape, so the proxy deliberately preserves those boundaries instead of treating them as removable whitespace.
...
`dataSaleOptOut` is captured through the aggregate runner with `corepack pnpm conformance:capture -- --run data-sale-opt-out`, backed by `scripts/capture-data-sale-opt-out-conformance.ts`. It creates disposable customers, captures existing-email opt-out, repeat idempotency, invalid-email userErrors, unknown-email customer creation, ASCII and Unicode-whitespaced email sanitization, downstream `Customer.dataSaleOptOut` reads, and cleanup.
...
- Fixture-backed Unicode whitespace sanitization scenario: `config/parity-specs/privacy/data-sale-opt-out-unicode-whitespace-email.json`
