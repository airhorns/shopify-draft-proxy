...
- `draftOrderInvoicePreview` returns deterministic local preview subject/html for staged draft orders and never sends email or writes upstream. Locally selected `invoiceErrors` now defaults to `[]` and returns `CUSTOMER_NO_EMAIL` when the staged draft has no draft/customer recipient email. When callers pass `presentmentCurrencyCode`, the proxy records the chosen code in the generated preview HTML so tests can verify that the argument was read. Public Admin GraphQL 2025-01 and 2026-04 on the current conformance store reject both `presentmentCurrencyCode` on this mutation and `invoiceErrors` on `DraftOrderInvoicePreviewPayload`, so the new compatibility behavior is covered by local-runtime parity rather than claimed as public-schema live parity.
