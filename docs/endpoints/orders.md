...
- Supported local order mutations append mutation-log entries only when the handler stages a successful local effect. Validation branches with `userErrors` or top-level GraphQL `errors`, including access-denied branches such as `orderCreateManualPayment` and `taxSummaryCreate`, leave the mutation log unchanged.
