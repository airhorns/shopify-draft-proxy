...
- `orderUpdate` rejects locally known orders before staging when the input has no mutable attributes beyond `id`, when `phone` fails the same local E.164-shaped guard used by customer mutations, when `shippingAddress` fails the shared country/province resolution used for mailing-address-style inputs, or when `staffMemberId` is malformed/unknown. Unknown well-formed staff member IDs return `userErrors[{ field: ["input", "staffMemberId"], code: "NOT_FOUND" }]`; known local `StaffMember` nodes stage through both `Order.user` and `Order.staffMember`. These branches return mutation-scoped `userErrors` with `order: null` and do not append mutation-log entries. Shopify's phone/email churn throttle is not modeled locally; treat throttle parity as captured-only fidelity until a session-scoped `Throttle::Counter` equivalent is deliberately added.
- `orderUpdate.localizedFields` and deprecated `orderUpdate.localizationExtensions` stage localized order records locally and are mirrored through both `Order.localizedFields` and `Order.localizationExtensions`, matching the captured 2026-04 behavior where the deprecated connection aliases the same stored records. Repeated updates merge by localization key and preserve existing keys. The public 2026-04 conformance schema on the current store rejects `OrderInput.staffMemberId` before resolver execution and staff reads are access-gated, so staff-member success/NOT_FOUND behavior is backed by local runtime tests while localization read-after-write is backed by the live parity fixture.
...
- Direct `orderCreate` line items preserve expanded staged field data for `properties`/`customAttributes`, shipping/taxable flags, gift-card/internal fulfillment/weight fields, vendor/product linkage, canonical `priceSet`, and line-level applied discounts. The live 2025-01 `order-create-line-item-fields` parity slice strictly covers the publicly selectable subset (`customAttributes`, `requiresShipping`, `taxable`, `vendor`, `product`, `originalUnitPriceSet`, empty `discountAllocations`) through the mutation payload and immediate downstream `order(id:)` read; focused runtime tests cover internal/offline-import fields that are not selectable in the current public Admin schema.
...
- `orderCapture` validation for multi-currency capture currency, over-capture, non-positive amounts, missing transactions, and no-longer-capturable authorizations returns local `userErrors` without mutating order financial state or logs. Local capture validation follows the internal `OrderCaptureUserError` code contract for these branches: `CURRENCY_REQUIRED` / `CURRENCY_MISMATCH` on `["currency"]`, `TRANSACTION_NOT_FOUND` / `INVALID_TRANSACTION_STATE` on `["parent_transaction_id"]`, and `INVALID_AMOUNT` / `OVER_CAPTURE` on `["amount"]`. This code-bearing shape is an internal draft-proxy contract derived from Shopify application behavior, not a currently selectable public Admin GraphQL payload field. Live introspection against `harry-test-heelo.myshopify.com` on `2025-01`, `2025-04`, the supported public versions `2025-07`, `2025-10`, `2026-01`, `2026-04`, and `unstable` all returned `OrderCapturePayload` fields `transaction` and `userErrors`; `userErrors` resolves to plain `UserError` with only `field` and `message`, `OrderCaptureUserError` is absent, and selecting `userErrors { code }` fails schema validation. Public live validation messages for manual test authorizations also differ from the internal code contract, so public parity must compare selectable public shape and transaction effects only; focused runtime tests remain the executable evidence for the local/internal code projection and final-capture lock behavior until a captured schema exposes code-bearing order-capture user errors.
