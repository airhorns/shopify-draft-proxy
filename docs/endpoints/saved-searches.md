...
- `fixtures/conformance/harry-test-heelo.myshopify.com/2025-01/saved-searches/saved-search-customer-deprecation-early-return.json` and `config/parity-specs/saved-searches/saved-search-customer-deprecation-early-return.json` prove `savedSearchCreate(resourceType: CUSTOMER)` returns only the deprecation `userErrors` entry before name-length or query-grammar validation runs, even when the submitted name is over 40 characters and the query contains otherwise invalid filters.
