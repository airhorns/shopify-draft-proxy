...
- `gleam/test/parity_test.gleam` additionally verifies resource-specific read-after-write routing for every locally supported saved-search create `resourceType`, including the explicit current `PRICE_RULE` behavior shared by code and automatic discount saved-search roots.
