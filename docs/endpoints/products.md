...
- Local `publishablePublish` and `publishableUnpublish` currently stage Product and Collection publishables. Product-scoped generic publishable payloads also serialize the selected `shop.publicationCount` from the effective local publication catalog, so store-property counts move with staged product publication targets. Broader publishable implementers remain unsupported in their own groups.
