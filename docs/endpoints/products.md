...
- `productSet(input:, synchronous: false)` records a local `ProductSetOperation` and `productOperation(id:)` can read it back with `status`, `product`, and `userErrors`. Unknown operation IDs return `null`, matching the captured helper-root no-data behavior.
...
- Product duplicate lifecycle: `tests/integration/product-draft-flow.test.ts`, `config/parity-specs/productDuplicate-parity-plan.json`, `config/parity-specs/productDuplicate-async-success.json`, and `config/parity-specs/productDuplicate-async-missing.json`
