...
- Product feed reads support Shopify-like no-data behavior in snapshot mode and read-after-write behavior for locally staged feeds. Captured 2025-01 `harry-test-heelo` evidence returns `productFeed(id:)` as `null` for an absent feed id and `productFeeds(first:)` as an empty connection with empty `nodes`/`edges`, `hasNextPage: false`, `hasPreviousPage: false`, and null cursors. Locally staged `productFeedCreate` returns an `ACTIVE` `ProductFeed`, `productFullSync` acknowledges known local feeds, and `productFeedDelete` removes staged feeds; supported feed mutations do not proxy upstream at runtime. The same live store still returns a top-level `NOT_FOUND`, `Unable to find channel for product feed`, for Shopify-side `productFeedCreate(country: US, language: EN)`, so success conformance is documented as channel-configuration-blocked for that shop.
...
- `bulkProductResourceFeedbackCreate` stages `ProductResourceFeedback` records locally for effective products and returns `Product does not exist` userErrors for missing products. `shopResourceFeedbackCreate` stages shop-level `AppFeedback` in meta state and returns the mutation payload locally. Live HAR-362 probes against `harry-test-heelo` returned top-level `ACCESS_DENIED` for both feedback mutations because the app lacks `write_resource_feedbacks` and Storefront API / Sales Channel configuration, so live success parity is blocked by app configuration rather than local runtime behavior.
