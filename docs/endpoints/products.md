...
- Versioned request-contract note: HAR-408 captured Admin GraphQL 2026-04 inventory quantity drift in `fixtures/conformance/harry-test-heelo.myshopify.com/2026-04/inventory-quantity-contracts-2026-04.json`. The parity runner exercises the missing `changeFromQuantity` set branch on a 2026-04 proxy request, and `tests/integration/inventory-quantity-roots.test.ts` replays the remaining route-specific validation and success branches through `/admin/api/2026-04/graphql.json`.
