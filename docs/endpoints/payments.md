...
- `paymentReminderSend` records a local reminder intent only when the
  ...
  same order inside the synthetic 24-hour window. Invalid schedule ID formats
  return `INVALID_PAYMENT_SCHEDULE_ID`; unknown or ineligible schedules return
  `success: null` with `PAYMENT_REMINDER_SEND_UNSUCCESSFUL` and do not stage
  reminder intent. It does not send customer email at runtime.
