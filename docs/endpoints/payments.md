...
- `paymentCustomizationCreate(paymentCustomization:)` creates a synthetic local `PaymentCustomization` record with selected metafields when required `title`, `enabled`, `metafields`, and exactly one Function identifier are present. Empty `metafields: []` is accepted, but the key must be present. The current Admin API uses `functionHandle`; deprecated `functionId` remains accepted for captured 2025-01 parity branches. Enabled creates are capped at five effective active payment customizations in the local store.
