...
- Captured Admin GraphQL 2025-01 evidence for `CustomerInput.emailMarketingConsent` and `CustomerInput.smsMarketingConsent` now covers the inline consent gap found during the HAR-435 review. `customerCreate(input: ...)` accepts inline email and SMS marketing consent values and immediately reflects them through `Customer.defaultEmailAddress`, `Customer.defaultPhoneNumber`, `emailMarketingConsent`, and `smsMarketingConsent`. `customerUpdate(input: ...)` rejects inline consent fields with payload `userErrors` that instruct callers to use `customerEmailMarketingConsentUpdate` or `customerSmsMarketingConsentUpdate`; the rejected update leaves downstream customer reads unchanged. The dedicated consent mutations remain the supported local path for post-create consent changes. The current fixture covers Shopify's default `OTHER` SMS consent collection source; source-location-specific attribution remains deferred until a capture records a non-default branch.
...
- HAR-436 reviewed the current Shopify Admin docs/examples and public example search results for customer merge, store credit, and customer payment method flows against the checked-in evidence. No new passive parity files were added: the high-risk customer-side flows already have executable coverage through `customer-merge-live-parity`, `customer-merge-attached-resources-live-parity`, `store-credit-account-local-staging`, `customer-outbound-side-effect-validation-parity`, and their integration tests. Keep future additions executable by the generic parity runner or by a runtime-test-backed fixture mode; do not add reserved merge/store-credit/payment-method scenario placeholders.
