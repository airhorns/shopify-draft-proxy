...
- There is no intentional passthrough posture for the Bulk Operations root operations themselves in the checked-in registry slice. Every known Bulk Operations root is intercepted locally, but support is bounded by the behavior below. Unsupported query export shapes fail locally with explicit `userErrors`; they must not be normalized as permanent passthrough support or described as broad Shopify export support.
...
- `bulkOperationRunQuery(query: String!, groupObjects: Boolean! = false)` creates an async query export. Locally supported product exports complete immediately against effective in-memory state, write JSONL result records, accept explicit `groupObjects: true`/`false` as well as the omitted default path, and never proxy supported export requests upstream at runtime. The local handler defensively supports `clientIdentifier` when it reaches the resolver, but the captured public 2026-04 schema rejects `clientIdentifier` as an argument on `bulkOperationRunQuery`. Blank `clientIdentifier` values are treated as absent; nonblank values must be 10..255 characters. Before staging, the proxy refuses a new query run when a matching effective `QUERY` BulkOperation is non-terminal (`CREATED`, `RUNNING`, or `CANCELING`), returning `bulkOperation: null` and `userErrors[{ field: null, code: "OPERATION_IN_PROGRESS" }]`.
