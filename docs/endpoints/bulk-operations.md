...
- For `bulkOperationRunMutation`, Shopify accepts any single inner mutation root except `bulkOperationRunMutation` and `bulkOperationRunQuery` themselves. The proxy mirrors that validator. If an accepted inner root has the same local mutation handler used by normal Admin GraphQL dispatch, each JSONL line stages locally through that handler. If the accepted inner root has no local handler yet, the proxy creates a failed local import job and records a result row explaining the unsupported local executor boundary; it does not send those bulk import lines upstream at runtime.
...
Accepted roots without a local mutation handler still produce the same outer CREATED response shape instead of a false `INVALID_MUTATION` rejection. For those roots, each JSONL line writes a local result row with an explanatory error, the local job is marked `FAILED`, and the job remains observable through `bulkOperation(id:)`, `bulkOperations`, and `currentBulkOperation(type: MUTATION)`. No unsupported bulk import line is sent upstream; downstream read-after-write effects remain absent until the corresponding domain is implemented locally.
...
- Unsupported local-executor lines do not create staged commit entries, because no local domain mutation was applied and no runtime Shopify write occurred.
