...
- `bulkOperationRunMutation(mutation: String!, stagedUploadPath: String!, clientIdentifier: String, groupObjects: Boolean = true)` creates an async mutation import from uploaded JSONL variables. The `groupObjects` argument is deprecated. After inner-mutation validation, any locally available staged upload content is byte-counted before same-type in-progress throttling or job staging. Oversized uploads use the configurable runtime limit `bulkOperationRunMutationMaxInputFileSizeBytes`, defaulting to Shopify's 100 MiB ceiling, and return `INVALID_STAGED_UPLOAD_FILE` with `bulkOperation: null`. Non-oversized submissions then refuse a new mutation run when any effective `MUTATION` BulkOperation is non-terminal, using the same `OPERATION_IN_PROGRESS` userError shape.
