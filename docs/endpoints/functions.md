...
Local validation guardrails currently cover missing Function references, unknown validation/cart-transform update or delete IDs, `validationUpdate` attempts to pass non-input Function rebinding fields, and re-enabling validations beyond the 10-active-validation cap. These branches return GraphQL errors or `userErrors` locally and still avoid runtime Shopify writes.
