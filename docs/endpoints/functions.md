...
`validationCreate` resolves the referenced Function from local state or cassette-backed hydration before staging. It rejects missing or multiple identifiers, unresolved references, and known non-validation API types without staging a `Validation` or inventing a `ShopifyFunction` row. Omitted `enable` / `enabled` input defaults to `false`, activation is capped at 25 active validations across effective state, and validation-owned metafields are persisted for downstream reads.
