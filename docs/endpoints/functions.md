...
`validationCreate` resolves the referenced Function from local state or cassette-backed hydration before staging. It rejects missing or multiple identifiers, unresolved references, and known non-validation API types without staging a `Validation` or inventing a `ShopifyFunction` row. Omitted `enable` / `enabled` input defaults to `false`, activation is capped at 10 active validations across effective state, and validation-owned metafields are persisted for downstream reads.

`cartTransformCreate` still preserves the older local metadata behavior for cart-transform draft reads: when no local Function metadata exists, it can create a deterministic metadata row from the submitted `functionId` or `functionHandle`.
...
Local validation guardrails currently cover missing/multiple Function identifiers, unknown or API-mismatched `validationCreate` Function references, unknown validation/cart-transform update or delete IDs, `validationUpdate` attempts to pass non-input Function rebinding fields, and activation beyond the 10-active-validation cap. These branches return GraphQL errors or `userErrors` locally and still avoid runtime Shopify writes.

`validationCreate` and `validationUpdate` persist validation-owned `metafields` inputs into the staged `Validation` record. Subsequent `validation`, `validations`, and mutation payload reads expose those rows through `metafields(...)` connection selections, including namespace, key, type, value, timestamps, owner type, and compare digest when selected. `validation.metafield(namespace:, key:)` is still a null placeholder until singular lookup behavior is captured.

### Boundaries

- Cart-transform metafield inputs are not expanded into full owner-scoped metafield records yet; selected cart-transform `metafield` returns `null` and selected `metafields` returns an empty connection.
...
- HAR-698 validation create evidence is recorded in `fixtures/conformance/local-runtime/2026-04/functions/functions-validation-create-validation.json` and enforced by `config/parity-specs/functions/functions-validation-create-validation.json`; the scenario verifies omitted `enable` defaults to `false`, the 11th active validation returns `MAX_VALIDATIONS_ACTIVATED`, non-validation Function API references return `FUNCTION_DOES_NOT_IMPLEMENT`, validation metafields persist, and a downstream `validations` connection observes the staged rows.
- HAR-703 validation update shape evidence is recorded in `fixtures/conformance/local-runtime/2026-04/functions/functions-validation-update-shape.json` and enforced by `config/parity-specs/functions/functions-validation-update-shape.json`; the scenario verifies `validationUpdate` rejects Function rebinding input shape, enforces the active-validation cap, persists validation metafields, and returns those metafields through a downstream `validations` connection.
