...
`validationCreate` persists validation-owned `metafields` inputs into the staged `Validation` record. `validationUpdate` treats omitted or explicit empty `metafields` input as a no-op for existing rows, and treats non-empty input as an upsert by `(namespace, key)`: matching rows keep their original position while value/type/`updatedAt` are updated, unrelated rows are retained, and new rows append after existing rows. Subsequent `validation`, `validations`, and mutation payload reads expose those rows through `metafields(...)` connection selections, including namespace, key, type, value, timestamps, owner type, and compare digest when selected. `validation.metafield(namespace:, key:)` is still a null placeholder until singular lookup behavior is captured.
