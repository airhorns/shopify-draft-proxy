...
The shipping TypeScript Koa/runtime dispatcher now routes these Functions roots through the Gleam JavaScript Functions implementation via `src/proxy/functions-gleam-bridge.ts`. The bridge preserves the existing TypeScript public API and instance-owned runtime state shape while delegating Functions-owned query/mutation behavior to Gleam. It also keeps the admin-platform `Validation` Node resolver and `bulkOperationRunMutation` inner Functions mutation executor on the same cutover path.
...
- HAR-517 cutover evidence in `tests/integration/functions-flow.test.ts` covers direct Functions lifecycle staging, admin-platform `node(id:)` / `nodes(ids:)` reads for `Validation`, and `bulkOperationRunMutation` replay of Functions-owned inner mutations through the Gleam bridge.
