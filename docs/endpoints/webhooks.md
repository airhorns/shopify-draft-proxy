...
- HTTP endpoint URI validation is shared by create and update, including the deprecated `callbackUrl` input alias. Captured Shopify behavior rejects plain `http://` URLs with `Address protocol http:// is not supported` and rejects Shopify/internal hosts (`shopify.com`, `myshopify.com`, `shopifypreview.com`, `myshopify.dev`, `localhost`, `127.x`, `192.168.x`, `172.16-31.x`, `10.x`, and `0.x`) with `Address cannot be a Shopify or an internal domain`. Rejected create/update mutations return `webhookSubscription: null` and do not stage webhook subscription records.
...
- `webhook-subscription-uri-validation` compares captured create/update URI validation branches for plain HTTP URLs, localhost/internal destinations, and Shopify-owned domains. The update branch creates a valid baseline subscription in the capture, then uses the proxy-created baseline ID during replay so the rejected update can prove the URI validation path runs before local staging.
...
- HTTP destination scheme and host validation is modeled for the captured Shopify/internal-domain branches. Pub/Sub and EventBridge URI projections are modeled for local state and reads, but cloud destination existence, app configuration, provider credentials, and denied shop-owned custom domains are out of scope for runtime staging until captured separately.
