...
`webhookSubscriptionCreate`, `webhookSubscriptionUpdate`, `webhookSubscriptionDelete`, `pubSubWebhookSubscriptionCreate`, `pubSubWebhookSubscriptionUpdate`, `eventBridgeWebhookSubscriptionCreate`, and `eventBridgeWebhookSubscriptionUpdate` are registered under the `webhooks` domain as implemented `stage-locally` operations for the captured Admin API-created subscription subset.
...
- Local records preserve captured fields: `id`, `topic`, `uri`, `name`, `format`, `includeFields`, `metafieldNamespaces`, `filter`, `apiVersion`, `createdAt`, `updatedAt`, and deprecated endpoint-specific fields for `WebhookHttpEndpoint`, `WebhookEventBridgeEndpoint`, and `WebhookPubSubEndpoint`.
- `WebhookSubscription.apiVersion` is projected as a non-null `ApiVersion` object. The normalized record stores the version handle, then the serializer projects `handle`, `displayName`, and `supported`; older records without a stored handle fall back to the deterministic request/app-effective version instead of returning `null`.
...
- `webhookSubscriptionCreate(topic:, webhookSubscription:)` rejects unknown or non-public `WebhookSubscriptionTopic` values before resolver side effects, including hidden enum values such as `CUSTOMER_INTERACTIONS`. These failures return Shopify-like top-level GraphQL `errors`, do not include a create payload, and do not stage records or append mutation-log entries. Accepted public enum values stage a new normalized webhook subscription with a proxy synthetic `WebhookSubscription` GID, stable synthetic timestamps, the requested `topic`, `uri`, `name`, `format`, `includeFields`, `metafieldNamespaces`, `filter`, and app-effective `apiVersion` handle after local address validation passes. `$app:<suffix>` `metafieldNamespaces` entries resolve through the request-owned `x-shopify-draft-proxy-api-client-id` identity before storage and downstream reads, producing the canonical `app--<api_client_id>--<suffix>` form while leaving non-app-prefixed values such as `custom` and `app--<other_app>--...` unchanged. Captured Shopify 2026-04 webhook behavior preserves the suffix casing in this field. If the request does not expose an API client ID, `$app:` namespace inputs are preserved unchanged rather than resolved with a fabricated caller ID. The deprecated `endpoint` field is derived from `WebhookSubscriptionInput.uri`: HTTPS strings become `WebhookHttpEndpoint.callbackUrl`, structurally valid `pubsub://{project-id}:{topic-id}` strings become `WebhookPubSubEndpoint`, and structurally valid `arn:aws:events:...` strings become `WebhookEventBridgeEndpoint`.
- `webhookSubscriptionUpdate(id:, webhookSubscription:)` updates an existing staged or hydrated webhook subscription in place, preserving `topic`, `createdAt`, and any existing `apiVersion` handle while replacing the captured mutable fields, `name`, and endpoint URI when present. If an older or partially hydrated record has no stored `apiVersion`, update stamps the same app-effective version handle used by create. Update applies the same request-owned `$app:` `metafieldNamespaces` canonicalization used by create when a caller API client ID is available, and preserves the input unchanged when that ID is not knowable. Update reuses the create-path address checks when `uri` or deprecated `callbackUrl` is present, rejects blank addresses, non-HTTPS HTTP callback URLs, malformed Pub/Sub/project/topic values, malformed EventBridge ARNs, wrong EventBridge API client IDs when known, public Kafka URIs, and Shopify/internal callback hosts (`shopify.com`, `myshopify.com`, `shopifypreview.com`, `myshopify.dev`, `localhost`, `127.x`, `192.168.x`, `172.16-31.x`, `10.x`, and `0.x`) with `Address cannot be a Shopify or an internal domain`, and returns `webhookSubscription: null` without staging on validation failure. Current Admin GraphQL docs describe `uri` as the unified field; `endpoint` and `callbackUrl` are kept only as deprecated compatibility projections.
- `pubSubWebhookSubscriptionCreate(topic:, webhookSubscription:)` and `pubSubWebhookSubscriptionUpdate(id:, webhookSubscription:)` normalize `PubSubWebhookSubscriptionInput.pubSubProject` plus `pubSubTopic` into `pubsub://{pubSubProject}:{pubSubTopic}` before applying the shared subscription staging, duplicate, filter, format, and downstream read paths. Captured dedicated Pub/Sub validation returns `field: ["webhookSubscription", "pubSubProject"]` with `Google Cloud Pub/Sub project ID is not valid` or `field: ["webhookSubscription", "pubSubTopic"]` with `Google Cloud Pub/Sub topic ID is not valid`, rather than the unified input's `callbackUrl` field path.
- `eventBridgeWebhookSubscriptionCreate(topic:, webhookSubscription:)` and `eventBridgeWebhookSubscriptionUpdate(id:, webhookSubscription:)` normalize `EventBridgeWebhookSubscriptionInput.arn` into the stored URI/address before applying the shared subscription staging, duplicate, filter, format, and downstream read paths. Captured dedicated EventBridge validation returns `field: ["webhookSubscription", "arn"]` for malformed ARN and wrong API-client-ID failures, including the generic `Address is invalid` companion message and the current `Address is an AWS ARN and includes api_client_id '...' instead of '...'` detail when the requesting API client ID is known.
...
- Callback address byte-size validation uses Shopify's MySQL text-column maximum of 65,535 bytes. HTTPS addresses below or at that byte-size limit continue through normal protocol/host validation and can stage locally; addresses above the limit return `userErrors: [{ field: ["webhookSubscription", "callbackUrl"], message: "Address is too big (maximum is 64 KB)" }]` without staging.
- Shop-owned callback host validation uses the effective shop record when one is already captured in state, or a LiveHybrid upstream shop baseline read when the handler is allowed to use cassette-backed upstream reads. The current public Admin GraphQL shop baseline exposes `myshopifyDomain` and `primaryDomain`; it does not expose the full internal `shop.domains` list or the `external?` / `custom_storefront_environment?` flags used by Shopify's Rails validator. The proxy therefore rejects the effective non-static `primaryDomain.host` as a shop-owned denied domain and documents the remaining plural-domain visibility gap instead of treating missing shop domain data as proof that no denied domains exist. Matching is exact-host only: `https://cool-shop.com/...` is rejected for denied domain `cool-shop.com`, while `https://www.cool-shop.com/...` is not rejected by this shop-owned-domain check.
...
- `webhook-subscription-dedicated-cloud-destinations` compares captured dedicated Pub/Sub and EventBridge create/update payloads, downstream detail reads, and dedicated input validation field paths. It records `PubSubWebhookSubscriptionInput.pubSubProject` / `pubSubTopic` synthesis into `pubsub://...` and `EventBridgeWebhookSubscriptionInput.arn` synthesis into the stored ARN URI.
- `webhook-subscription-api-version-projection` compares captured `WebhookSubscription.apiVersion` object projection on the create payload, a downstream detail read, and a URI-filtered connection node. The capture shows Shopify returns the creating app's effective Admin API version, not necessarily the request route version.
