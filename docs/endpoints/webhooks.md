...
- Destination-specific validation beyond captured structural URI branches is not generalized. HTTP endpoint reachability, cloud destination existence, app configuration, provider credentials, and successful cloud subscription lifecycles are out of scope for runtime staging until backed by separate executable evidence.
