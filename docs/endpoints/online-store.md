...
- `commentApprove`, `commentSpam`, `commentNotSpam`, and `commentDelete` stage moderation for comments that already exist in hydrated/snapshot local state, or hydrate the existing comment through a narrow upstream read before staging when needed. Approval sets `status: "PUBLISHED"`, spam sets `status: "SPAM"`, and not-spam sets `status: "UNAPPROVED"`.
- `commentDelete` records a local hard deletion marker and returns `deletedCommentId`, matching Shopify's destroy path for the public mutation. Deleted comments disappear from `comment(id:)`, root `comments`, `Article.comments`, and `Article.commentsCount`, while repeated local `commentDelete` remains idempotent for the same ID.
- Removed comments supplied by legacy snapshots or cascades still return an `INVALID` `userErrors` entry from `commentApprove` and `commentSpam`. Approval also sets `isPublished: true` and a local `publishedAt` timestamp when the comment did not already have one.
...
- ServerPixel endpoint validation evidence: `config/parity-specs/online-store/event_bridge_server_pixel_update_arn_format.json`, `pub_sub_server_pixel_update_blank_required.json`, and `server_pixel_endpoint_update_no_pixel.json` replay malformed/blank/missing endpoint arguments plus absent-current-ServerPixel updates through the generic Gleam parity runner. These scenarios are local-runtime-backed because the current conformance credential lacks server-pixel write scopes, while the implementation pass captured live schema/coercion probes for the invalid EventBridge ARN and blank Pub/Sub input branches.
