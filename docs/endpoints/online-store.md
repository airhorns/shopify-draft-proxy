...
- script tag mutations stage `src`, normalized lowercase `displayScope`, and `cache` values, but never load or execute storefront JavaScript. `scriptTagCreate` rejects missing/blank, over-255-character, and non-HTTP(S) `src` values before staging; `scriptTagUpdate` applies the same checks only to changed fields. GraphQL responses expose display scope as the Admin enum form (`ONLINE_STORE`, `ALL`, or `ORDER_STATUS`).
