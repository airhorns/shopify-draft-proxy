...
- `storefrontAccessTokenDelete` validates `input.id` as a `StorefrontAccessToken` gid before local lookup. Wrong gid types produce a schema-level coercion-style GraphQL error; missing or out-of-scope tokens return a `NOT_FOUND` user error on `["input", "id"]` with `deletedStorefrontAccessTokenId: null`.
