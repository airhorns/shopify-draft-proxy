...
Create and update now normalize definition types through Shopify's app-reserved namespace rules before storage and duplicate checks. `$app:<rest>` resolves to the captured installed app namespace (`app--347082227713--<rest>` in the conformance shop) and the resolved type is lowercased before downstream reads and uniqueness checks. Definition create/update validation returns Shopify-like `TOO_SHORT`, `TOO_LONG`, and `INVALID` userErrors for type length/format guardrails, and create/update field-definition keys are locally guarded to lowercase alphanumeric/underscore keys.
