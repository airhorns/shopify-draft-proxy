...
`metaobjectUpdate` and the update branch of `metaobjectUpsert` run a local `DISPLAY_NAME_CONFLICT` guard when the effective display name changes. The guard checks other effective metaobjects of the same type and only rejects a duplicate display name when the competing row is referenced by a product option through a `productOptionsCreate.linkedMetafield` association recorded on the metaobject definition. Display-field changes return `DISPLAY_NAME_CONFLICT` at `field: ["fields", "<index>"]`; handle-derived display-name changes return it at `field: ["handle"]`. Rejected rows are not staged. `metaobjectCreate` keeps Shopify's create behavior and continues to auto-suffix duplicate handles without running this update/upsert validator. Runtime coverage exercises this local guard, but no checked-in parity capture has been promoted for this branch because the 2026-04 conformance shop probe accepted the tested linked-product-option display-name collision shape; recordings must not be fabricated.
