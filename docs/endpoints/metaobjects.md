...
HAR-351 promotes the HAR-241 definition-read subset into `config/parity-specs/metaobjects/metaobject-definitions-read.json` as a strict generic proxy-vs-recording parity scenario. The parity runner seeds normalized definition state from the captured 2026-04 fixture, then replays definition catalog, ID lookup, type lookup, and missing ID/type requests against the local proxy harness. Opaque Shopify catalog cursors remain explicit expected differences because the snapshot harness emits stable local cursors. `tests/integration/metaobject-definition-query-shapes.test.ts` continues to cover aliases/fragments, live-hybrid overlay, field order, and no-runtime-live-access behavior.
