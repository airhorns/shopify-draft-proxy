...
- `appRevokeAccessScopes` validates requested handles against the modeled Shopify access-scope catalog, rejects unknown handles with a single `UNKNOWN_SCOPES` userError, rejects required app scopes with `CANNOT_REVOKE_REQUIRED_SCOPES`, and stages revocation only when the request has no userErrors. Unknown app IDs return `APPLICATION_CANNOT_BE_FOUND`; a missing current installation returns `APP_NOT_INSTALLED`.
