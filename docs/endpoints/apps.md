...
- `delegateAccessTokenDestroy` matches the raw token against the stored hash, marks it destroyed locally, and returns `ACCESS_TOKEN_NOT_FOUND` when repeated or unknown. Its non-null `shop` payload uses the same Apps current-shop helper on both success and user-error branches, matching Shopify's resolver contract that still returns the merchant shop with validation errors.
