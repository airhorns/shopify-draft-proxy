...
- `appSubscriptionLineItemUpdate` validates the subscription line-item GID shape before lookup, only mutates usage-pricing line items, rejects capped-amount currency mismatches, and requires the requested capped amount to be greater than the existing usage cap before staging the new amount. Recurring-pricing line items, malformed IDs, unknown local IDs, currency mismatches, and non-increasing caps return userErrors without mutating local state. `appSubscriptionTrialExtend` mutates staged subscription state and returns userErrors for unknown local IDs.
