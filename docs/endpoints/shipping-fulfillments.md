...
- `fulfillmentOrderReschedule` accepts local `SCHEDULED` fulfillment orders, updates `fulfillAt` and `updatedAt`, and leaves status/actions otherwise unchanged. Non-scheduled fulfillment orders return the captured `Fulfillment order must be scheduled.` user error without partial staging.
- `fulfillmentOrderClose` accepts local fulfillment orders assigned to an API fulfillment-service location (`assignedLocation.location.isFulfillmentService: true`), moves the order to `CLOSED`, clears `supportedActions`, and exposes the staged state in downstream fulfillment-order reads. Merchant-managed or missing assigned-service records return the captured API-service guardrail without partial staging.
...
HAR-234/HAR-367 captured guardrails for the narrower fulfillment-order lifecycle roots: the disposable merchant-managed setup returns `Fulfillment order must be scheduled.` for reschedule, `The fulfillment order's assigned fulfillment service must be of api type` for close, a Shopify internal error for the attempted included-location reroute success branch, and `FULFILLMENT_ORDER_INVALID` for prepared-for-pickup against non-pickup fulfillment orders. The proxy now models the local scheduled reschedule branch and API-service close branch described above, while `fulfillmentOrdersReroute` and `fulfillmentOrderLineItemsPreparedForPickup` remain registry-unimplemented until pickup/reroute success setup and downstream read behavior are captured.
...
- `fulfillmentOrdersReroute` and `fulfillmentOrderLineItemsPreparedForPickup` remain registry-only/full-support gaps except for captured guardrail branches already called out above.
