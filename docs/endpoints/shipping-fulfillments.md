...
- `fulfillmentOrderMove` stages full or partial line-item moves by assigning selected work to a replacement fulfillment order at the requested location and leaving remaining quantities on the original order for partial moves. Move rejects locally closed fulfillment orders, request-status-blocked fulfillment orders, manually progress-reported fulfillment orders, and unknown/inactive destination locations using the captured Shopify user-error shapes. Full moves return `remainingFulfillmentOrder: null`; partial residual moves continue to expose the original order as the remaining order until true 3-way split behavior is modeled.
