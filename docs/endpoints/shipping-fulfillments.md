...
- `fulfillmentOrderReportProgress` changes local status to `IN_PROGRESS` and marks the order as manually progress-reported; `fulfillmentOrderOpen` changes it back to `OPEN` and clears that local manual-progress guard.
