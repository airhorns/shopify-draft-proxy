...
Those roots are implemented in the Gleam runtime. Most shipping/fulfillment lifecycle evidence runs through the parity runner and `config/parity-specs/shipping-fulfillments/fulfillment*.json`; `fulfillmentOrderClose` and `fulfillmentOrderReschedule` additionally have focused local runtime coverage for their newly modeled success branches because the checked-in HAR-234 Shopify capture only proves their merchant-managed guardrails. HAR-122/HAR-187 provide the evidence-backed fulfillment lifecycle slices; HAR-233 adds request/cancellation lifecycle evidence backed by `fixtures/conformance/harry-test-heelo.myshopify.com/2026-04/shipping-fulfillments/fulfillment-order-request-lifecycle.json`.
...
- `fulfillmentOrderClose` stages `IN_PROGRESS` fulfillment orders, plus `OPEN` fulfillment orders whose `assignedLocation` is a known local `THIRD_PARTY` fulfillment-service location, to `status: CLOSED` / `requestStatus: CLOSED` and clears `supportedActions`. Merchant-managed `OPEN` records without local API-service evidence keep Shopify's captured `FULFILLMENT_SERVICE_NOT_API_TYPE` guardrail; other non-closeable statuses return `INVALID_FULFILLMENT_ORDER_STATUS`; unknown IDs return `FULFILLMENT_ORDER_NOT_FOUND`. Failed close attempts leave the local record unchanged.
...
HAR-234/HAR-367 captured guardrail branches for `fulfillmentOrderReschedule` and `fulfillmentOrderClose`, but did not capture scheduled/API-service success paths; those success branches are currently covered by focused local runtime tests rather than fabricated cassette data. `fulfillmentOrdersReroute` and `fulfillmentOrderLineItemsPreparedForPickup` remain registry-unimplemented: the current disposable setup returned a Shopify internal error for the attempted included-location reroute success branch and `FULFILLMENT_ORDER_INVALID` for prepared-for-pickup against non-pickup fulfillment orders.
...
- `fulfillmentOrdersReroute` and `fulfillmentOrderLineItemsPreparedForPickup` remain registry-only/full-support gaps except for captured guardrail branches already called out above. `fulfillmentOrderReschedule` and `fulfillmentOrderClose` now have local success-path models, but still need live scheduled/API-service success cassettes before their happy paths can be described as captured Shopify parity.
