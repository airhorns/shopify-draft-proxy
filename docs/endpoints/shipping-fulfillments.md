...
- `fulfillmentOrderReportProgress` requires an `IN_PROGRESS` fulfillment order, then restamps local progress and marks the order as manually progress-reported. Other statuses are rejected with `INVALID_FULFILLMENT_ORDER_STATUS` on `field: ["id"]` without changing `status`, `supportedActions`, `updatedAt`, or `__draftProxyManuallyReportedProgress`.
- `fulfillmentOrderOpen` requires a `SCHEDULED` fulfillment order, then changes it to `OPEN` and clears the local manual-progress guard. Other statuses are rejected with `INVALID_FULFILLMENT_ORDER_STATUS` on `field: ["id"]` without partial staging. The Shopify flag path that permits `IN_PROGRESS -> OPEN` is not modeled.
