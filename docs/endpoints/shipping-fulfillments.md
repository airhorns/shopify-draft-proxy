...
- `deliveryProfileCreate(profile:)` validates create input before staging. Blank names return `PROFILE_CREATE_REQUIRES_NAME`, names of 128 characters or more return `TOO_LONG`, unknown or non-merchant-active location IDs are rejected before synthetic profile creation, and empty or overlapping `zonesToCreate[*].countries` sets return `CANNOT_UPDATE_ZONES`. Indexed location and zone field paths use the local dotted-index format such as `locationGroupsToCreate.0.locations.0` when callers select `userErrors.field`; callers that select the proxy's local `code` convenience field receive the corresponding create validation code even though the captured public 2026-04 payload exposes generic `UserError` field/message only.
