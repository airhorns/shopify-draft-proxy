...
- current 2026-04 conversion metric fields `primaryConversions` and `allConversions` are staged and returned as decimal strings when supplied, matching the existing decimal handling for orders/customer conversion metrics; this branch is runtime-test-backed because the checked-in HAR-214 live fixture was captured against Admin GraphQL 2025-01 before those input fields were present
...
- HAR-453 did not find a stronger live evidence path for native/deprecated activity success or recognized channel-handle engagement success. Those gaps remain intentionally documented: native success depends on a deprecated marketing activity app extension in the conformance app, and channel-level engagement success depends on discovering a recognized `channelHandle` in a disposable shop.
