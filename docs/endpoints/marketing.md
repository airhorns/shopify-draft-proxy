...
- `marketingActivityUpdateExternal` also accepts a matching `remoteId` + `utm` selector pair; the focused multi-selector parity spec replays Shopify's captured successful payload for that request shape
...
- `marketing-activity-update-external-multi-selector` reuses the live immutable-fields capture to prove that a successful update can supply both the external `remoteId` and matching `utm` selector while still staging locally in replay.
