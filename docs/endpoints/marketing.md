...
External activity `remoteUrl` and `remotePreviewImageUrl` inputs accept only `http` and `https` schemes on create, update, and upsert. Captured 2026-04 behavior rejects URL-scalar-valid non-HTTP schemes such as `ftp`, `file`, and `mailto` as top-level `INVALID_FIELD_ARGUMENTS` errors with `data.<root>: null`; URL scalar failures such as `data:` and `javascript:` remain top-level `INVALID_VARIABLE` coercion errors before mutation handling. Rejected inputs do not stage activities or alter downstream `marketingActivities` reads.
...
- `marketingEngagementCreate` validates selector count before currency, channel, or activity lookup work. On the channel-handle path, unknown channel handles return `INVALID_CHANNEL_HANDLE` before adSpend/sales currency validation. On activity-ID and remote-ID paths, adSpend/sales currency validation runs before activity lookup, so a missing remote ID with mismatched input currencies returns `CURRENCY_CODE_MISMATCH_INPUT` rather than `MARKETING_ACTIVITY_DOES_NOT_EXIST`.
- if an activity-level engagement selector resolves an activity whose nested/effective marketing event has been deleted or is otherwise absent, `marketingEngagementCreate` returns `MARKETING_EVENT_DOES_NOT_EXIST` instead of treating the activity as unknown. Unknown activities still return `MARKETING_ACTIVITY_DOES_NOT_EXIST`.
...
- Admin GraphQL 2026-04 response-shape evidence for `marketingEngagementCreate` shows the immediate `marketingEngagement` payload echoes supplied fields without synthesizing omitted defaults. Full input returns the supplied `occurredOn`, `utcOffset`, `isCumulative`, and metric fields; inline input missing `isCumulative`/`utcOffset` or `occurredOn` returns top-level `missingRequiredInputObjectAttribute` errors before resolver side effects. The proxy keeps `__typename` selection via normal projection and returns `null` for omitted selected fields only in direct local handler paths that bypass dispatcher schema validation.
...
- `marketing-engagement-create-validation-order` captures the observable `marketingEngagementCreate` validation-order branches against Admin GraphQL 2026-04: multiple selectors beat currency mismatch, unknown channel handles beat currency mismatch on the channel path, input currency mismatch beats missing activity lookup on the remote-ID path, and valid-currency unknown remote IDs still return `MARKETING_ACTIVITY_DOES_NOT_EXIST`. The soft-deleted `MarketingEvent` branch is not directly recordable through the public Admin API because the disposable setup cannot delete an event while preserving its activity; focused runtime coverage models that deleted-event state and returns `MARKETING_EVENT_DOES_NOT_EXIST`.
