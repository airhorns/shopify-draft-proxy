...
- local `orderCreate` staging now carries the richer creation slice used by the promoted parity fixture: `options` are accepted without upstream side effects, variant-backed line items preserve variant references when supplied, MoneyBag presentment money is serialized when requested, tax lines contribute to `totalTaxSet`, fixed discount codes contribute to `discountCodes` / `totalDiscountsSet`, and downstream `order(id:)` / `orders` reads expose those staged fields
