...
- `node(id:)` and `nodes(ids:)` use one type-to-loader inventory in `src/node_resolver_inventory.rs`; inventory entries represent locally modeled types and carry their executable loader rather than a second loader enum/switch. Loaders return explicit `Found`, `KnownMissing`, `NeedsHydration`, or `UnsupportedType` states. Live-hybrid sends one upstream request for a mixed cold batch, including well-formed GIDs outside the local inventory, merges staged values and tombstones over the response before caching it, and preserves input ordering/null placeholders. Successful generic Node nulls and successful nulls from the finance/POS singular roots establish exact, API-version-scoped authoritative misses for later generic Node reads; `/__meta/reset` clears that instance-owned cache. Snapshot mode never hydrates upstream.
