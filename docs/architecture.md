...
Long-running local jobs keep their execution plans in that same instance-owned Store. `bulkOperationRunQuery` admission stages one CREATED record and a replay-ready raw mutation without hydrating the catalog. Later BulkOperation reads advance a fixed slice of at most 16 root catalog pages per GraphQL request, so LiveHybrid execution cost stays bounded while small jobs can complete on their first poll and Snapshot execution remains transport-free. Completion replaces the CREATED/RUNNING record with terminal metadata and an instance-owned JSONL artifact; failure and cancellation remove pending execution without publishing a partial artifact. Dump/restore persists pending plans, and reset discards plans, jobs, and results together.
