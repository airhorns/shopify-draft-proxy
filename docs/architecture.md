...
Parity specs that declare `user-errors-parity` must include at least one comparison target that can observe the mutation validation surface: GraphQL `errors`, the mutation payload root, or a selected `*UserErrors` field. Resource-only targets such as `productCreate.product` are rejected because they can pass while validation/user-error behavior has drifted.
