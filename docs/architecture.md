...
- `GET /__meta/log`
- `GET /__meta/state`
- `GET /__meta/webhooks/outbox`
- `POST /__meta/webhooks/outbox/reset`
- `GET /__meta/config`
- `GET /__meta/health`
...
- `GET /__meta/webhooks/outbox` returns the ordered in-memory synthetic webhook delivery records generated by explicitly supported topic mappers; `POST /__meta/webhooks/outbox/reset` clears only that outbox
- `POST /__meta/reset` clears the webhook outbox together with staged state, hydrated/cache state, mutation logs, and synthetic identity counters
