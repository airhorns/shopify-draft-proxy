...
- A 2026-07-20 mutation-first capture for cold Order-owned and DraftOrder-owned terms confirms one `node(id:)` query can recover the PaymentTerms, its single owner, and its schedule before a local delete decision. After Shopify deletes the terms, the owner returns `paymentTerms: null` and generic Node reads return `null` for both the PaymentTerms and PaymentSchedule ids. A confirmed-missing PaymentTerms node is also `null`, followed by the standard `PAYMENT_TERMS_DELETE_UNSUCCESSFUL` payload.
