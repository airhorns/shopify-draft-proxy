...
- `fulfillmentOrderClose` and `fulfillmentOrderReschedule` now have local success-path models and read-after-write runtime tests, but the checked-in Shopify capture still only proves the merchant-managed close guardrail and non-scheduled reschedule guardrail. Do not present those local success branches as live-captured parity until a scheduled/API-service success cassette exists.
...
- keep `fulfillmentOrdersReroute` unimplemented for full support until success-path fixtures exist, even if local guardrails are mirrored
