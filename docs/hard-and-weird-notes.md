...
- A 2026-07 live mandate-capability probe confirmed the active conformance shop reports `shop.plan.shopifyPlus: false` and the installed app has neither `read_payment_mandate` nor `write_payment_mandate`. Reading `Order.paymentCollectionDetails.vaultedPaymentMethods` returns top-level `ACCESS_DENIED` requiring `read_payment_mandate`. Calling `orderCreateMandatePayment` with otherwise well-formed Order/PaymentMandate IDs returns top-level `ACCESS_DENIED` requiring `write_payment_mandate`, `pay_orders_by_vaulted_card`, and Shopify Plus access for the amount field. Wrong-resource Order or mandate IDs are rejected earlier as `Invalid id: <gid>` with `RESOURCE_NOT_FOUND`. This environment cannot capture successful, missing-correct-type, unrelated-mandate, or schedule-eligibility resolver branches; local guards for those branches remain runtime-test evidence rather than Shopify parity.
