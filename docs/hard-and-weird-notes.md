...
- A 2026-07-20 live capability probe confirmed that `read_cash_tracking`, `write_cash_tracking`, and `read_shopify_payments_disputes` are granted, while `cashTrackingSessions(first: 1)` and `disputes(first: 1)` are both empty. QueryRoot has only singular `pointOfSaleDevice(id:)`, with no catalog/create path for discovering or provisioning a device, and cash sessions require real POS Pro activity. Disputes require a real external chargeback/inquiry rather than an Admin mutation. The same probe returned `ACCESS_DENIED` for `shopifyPaymentsAccount` because the install lacks `read_shopify_payments` / `read_shopify_payments_accounts`. Until those capabilities produce scrubbed real records, transport tests—not fabricated fixtures—must guard non-empty generic Node routing.
