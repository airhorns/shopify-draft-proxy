...
- `customer-payment-method-no-auto-seed` is local-runtime fixture evidence, not live Shopify payment-method fidelity evidence. It exists only to lock the blank snapshot contract that unhydrated reads for sentinel IDs such as `gid://shopify/CustomerPaymentMethod/base-card` and Customer/8801 payment-method lists stay null/empty instead of reintroducing fabricated fixture records.
