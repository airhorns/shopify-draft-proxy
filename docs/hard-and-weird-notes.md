...
- The older lifecycle capture records `fulfillmentOrderReportProgress` changing an open merchant-managed fulfillment order to `IN_PROGRESS` and adding `MARK_AS_OPEN`, followed by `fulfillmentOrderOpen` changing it back to `OPEN`; current local support treats that open-order report-progress branch as stale for status preconditions and requires `IN_PROGRESS` for report-progress / `SCHEDULED` for open unless a newer capture proves a version- or flag-specific exception
