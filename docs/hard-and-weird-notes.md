...
## How to read current vs. historical notes

- Headings without a `Historical:` or `Superseded:` prefix record current fidelity traps, serializer/modeling rules, or still-useful Shopify behavior observations.
- `Historical:` headings preserve old access, credential, or sequencing lessons that explain how earlier evidence changed. They are regression-diagnosis notes, not current implementation boundaries.
- `Superseded:` headings preserve a once-useful implementation warning that has been replaced by endpoint documentation, executable parity, or runtime tests. Use the linked endpoint docs as the current contract.
- Endpoint docs remain authoritative for supported roots, current local behavior, validation anchors, and declared gaps. When this field notebook and an endpoint doc appear to disagree, update this file or the endpoint doc instead of treating the disagreement as a new support claim.

## Current: GraphQL operation names are not reliable routing keys
...
## Current: Query overlay fidelity is much harder than mutation staging
...
## Current: Handle generation is already weird
...
## Current: Product update semantics are mode-sensitive
...
### Current: BulkOperation reads expose terminal job history, not just active jobs
...
Historical shared-credential finding:
...
- additional historical host trap: even when the saved payload still has `client_id` and `refresh_token`, the repo-local refresh can fail with `invalid_request` / `This request requires an active refresh_token`; treat that as a dead saved grant, not a retryable transient error
...
### Historical: order-domain auth regression should not clobber last verified blocker evidence
...
### Historical: merchant-realistic order-creation scaffolds needed richer surfaces before happy-path capture existed
...
### Historical: bearer-token fallback unlocked the first real draft-order happy path on this host
...
### Historical: `orderCreate` had one safe GraphQL-validation slice before offline-token write access existed
...
### Historical: `draftOrderCreate` validation slices preceded draft-order write access
...
## Current: `orderUpdate` remains a narrow evidence-backed order-edit increment
...
## Current: Fulfillment validation and lifecycle support arrived in layers
...
- `customersCount(query:)` is **not** safely assumed to share the richer live `customers(query:)` grammar: in the live `customers-count.json` fixture on this host, both `email:grace@example.com` and `state:DISABLED` returned exact total counts plus Shopify `invalid_field` warnings instead of filtered customer counts
- practical rule: keep live/captured `customersCount` conservative and evidence-driven. For locally staged overlay reads, use the same narrow staged predicate as `customers(query:)` so read-after-write count and list semantics do not diverge; do not expand that into broader Shopify count-query grammar without new conformance evidence
...
- successful multi-location `inventoryBulkToggleActivation(... activate: false ...)` keeps the surviving level on default downstream reads and returns an empty top-level `inventoryLevels` payload array in the mutation response rather than echoing the deactivated level. The 2026-04 inactive-level read fixture currently covers the direct `inventoryDeactivate` / `inventoryActivate` lifecycle; bulk inactive-read parity should be refreshed separately before claiming identical bulk include-inactive behavior.
...
- payment customization reads now have a narrow HAR-223 snapshot/local model for `paymentCustomization(id:)` and `paymentCustomizations(...)`, seeded only from normalized state; writes are staged locally for `paymentCustomizationCreate`, `paymentCustomizationUpdate`, `paymentCustomizationDelete`, and `paymentCustomizationActivation` without invoking Shopify Functions or checkout payment behavior at runtime
...
- HAR-223 also captured payment customization validation branches with the write scope granted:
  - the current test store returned no `shopifyFunctions(first: 20)` nodes; HAR-223 added and deployed a repo-local `conformance-payment-customization` Function extension, but the existing store install still returned an empty Function catalog afterward, so a true live happy-path create/update/delete sequence likely needs a refreshed app install/grant that can see the released Function
  - create with missing `title`, `enabled`, or `functionId` returns `PaymentCustomizationError` code `REQUIRED_INPUT_FIELD` under `['paymentCustomization', <field>]`
...
- 2026-04 payment customization metafield validation returns one `INVALID_METAFIELDS` userError per invalid field, not one generic metafield error. The captured public Admin behavior reports missing `key`/`value` as `"may not be empty"`, blank `type` as `"can't be blank"`, and a non-empty two-character namespace as `"is too short (minimum is 3 characters)"`; for the present-but-invalid update capture, `type` is reported before `namespace`.
...
- keep scaffold-only payment roots out of capability routing until captured evidence and local modeling exist for each family; the current exceptions are the payment customization slice backed by empty/null evidence plus normalized snapshot rows, the payment terms template catalog backed by captured list/filter evidence, and the HAR-222 local payment-terms lifecycle model backed by executable parity plus runtime read-after-write tests
...
- captured no-data reads for `cashTrackingSession(s)`, `pointOfSaleDevice`, `dispute(s)`, and Shop Pay payment request receipt roots are safe Snapshot-only empty/null behavior; cold LiveHybrid reads remain authoritative upstream and are enforced by `finance-risk-no-data-read`. A successful singular upstream null is an exact, API-version-scoped authoritative miss for later generic Node reads, not a global invariant for the type.
- the active conformance installation grants `read_cash_tracking`, `write_cash_tracking`, and `read_shopify_payments_disputes`, but 2026-07 discovery returns no cash drawers, cash-tracking sessions, POS payment sessions, POS-origin orders, or disputes. Admin GraphQL exposes no POS device-create or dispute-create mutation. A device requires an external Shopify POS installation, and a dispute requires an external chargeback or inquiry. `shopifyPaymentsAccount` is separately `ACCESS_DENIED` without `read_shopify_payments` / `read_shopify_payments_accounts`.
- keep `CashTrackingSession`, `PointOfSaleDevice`, and `ShopifyPaymentsDispute` out of the local Node resolver inventory until real non-empty payloads and local lifecycle models exist. LiveHybrid can still forward their well-formed GIDs through the generic unsupported-type path; transport tests prove that routing mechanic but are not Shopify payload evidence.
...
## 116. Draft-order shipping-rate handles bind calculation to configured delivery state
...
## 119. Discount prerequisite transport history is not validation authority
