...
- HAR-222 locally stages `paymentTermsCreate`, `paymentTermsUpdate`, and `paymentTermsDelete` against the order/draft-order graph using the captured template catalog and Shopify-documented standalone mutation input shapes. Successful paths are runtime/local evidence only for now; live happy-path parity still needs an isolated order or draft order because Shopify writes alter payment schedules and payment status. Rejected local guardrails cover unknown targets, missing/unknown template ids, NET schedules without `issuedAt`, FIXED schedules without `dueAt`, missing update ids, and duplicate deletes without appending staged-write log entries.
...
- payment terms template reads are captured and locally modeled as a catalog; payment terms lifecycle writes are locally staged for existing order/draft-order graph records, while live parity capture still requires order or draft-order access and an isolated order/payment schedule because the real mutations change due dates and payment status
