...
- LiveHybrid connection overlays use `observed_connection_rows(...)` to retain authoritative edge cursors and `complete_upstream_connection(...)` to exhaust a forward upstream baseline before local filtering/windowing. Reuse `upstream_page_is_complete_baseline(...)` when an already-returned first page may avoid the dedicated paginator; do not infer completeness from a bounded node vector or reconstruct Shopify cursors from resource IDs.
