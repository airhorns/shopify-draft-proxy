...
describe('product draft flow', () => {
  ...
  it('returns Shopify-like argument validation errors when productDelete inlines a missing required input id', async () => {
    ...
    const response = await request(app)
      ...
      .send({
        query:
          'mutation { productDelete(input: {}) { deletedProductId userErrors { field message } } }',
      });
  ...
  it('returns Shopify-like argument validation errors when productDelete inlines a null input id', async () => {
    ...
    const response = await request(app)
      ...
      .send({
        query:
          'mutation { productDelete(input: { id: null }) { deletedProductId userErrors { field message } } }',
      });
  ...
  it('rejects explicit colliding handles for productCreate with Shopify-like userErrors', async () => {
    ...
    expect(collisionResponse.body.data.productCreate).toEqual({
      product: null,
      userErrors: [{ field: ['input', 'handle'], message: "Handle 'handle-owner' already in use. Please provide a new handle." }],
    });
  });
  ...
  it('preserves the current handle when productUpdate tries to claim a different products handle', async () => {
    ...
    expect(updateResponse.body.data.productUpdate).toEqual({
      ...
      userErrors: [{ field: ['input', 'handle'], message: `Handle '${firstHandle}' already in use. Please provide a new handle.` }],
    });
  ...
  it('accepts Shopify-like bulk variant input shapes with optionValues, inventoryQuantities, and inventoryItem.sku', async () => {
    ...
    const createProductResponse = await request(app)
      .post('/admin/api/2025-01/graphql.json')
      .send({
        query: 'mutation { productCreate(product: { title: "Live Shape Bulk Hat" }) { product { id } userErrors { field message } } }',
      });
      ...
      data: {
        inventoryDeactivate: {
          userErrors: [
            {
              field: null,
              message: "The product couldn't be unstocked from 103 ossington because products need to be stocked at a minimum of 1 location.",
            },
          ],
        },
      },
      ...
      data: {
        inventoryBulkToggleActivation: {
          ...
          userErrors: [
            {
              field: ['inventoryItemUpdates', '0', 'locationId'],
              message: "The variant couldn't be unstocked from 103 ossington because products need to be stocked at a minimum of 1 location.",
