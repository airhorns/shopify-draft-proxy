...
describe('customer query shapes', () => {
  ...
  it('replays live Shopify customer cursors and string count fields from hydrated catalog state', async () => {
    const fetchSpy = vi
      ...
      .mockImplementationOnce(async () =>
        new Response(JSON.stringify({ data: { customers: { edges: [], pageInfo: { hasNextPage: false, hasPreviousPage: false, startCursor: null, endCursor: null } } } }), {
          status: 200,
          headers: { 'content-type': 'application/json' },
        }),
      )
  ...
  it('filters customers by query terms and sorts them by updatedAt in live replay mode', async () => {
    ...
    store.setBaseCustomerCatalogConnection({
      orderedCustomerIds: [
        'gid://shopify/Customer/401',
        'gid://shopify/Customer/402',
        'gid://shopify/Customer/403',
      ],
      cursorByCustomerId: {
