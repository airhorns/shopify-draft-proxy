...
describe('media draft flow', () => {
  ...
  it('stages fileCreate locally without attaching product media or proxying upstream', async () => {
    ...
    const productResponse = await request(app)
      ...
      .send({
        query: 'mutation { productCreate(product: { title: "Media file owner" }) { product { id } userErrors { field message } } }',
      });
    ...
    const productMediaResponse = await request(app)
      .post('/admin/api/2026-04/graphql.json')
      .send({
        query: 'query ProductMedia($id: ID!) { product(id: $id) { id media(first: 10) { nodes { id alt mediaContentType } pageInfo { hasNextPage hasPreviousPage } } } }',
  ...
  it('returns Shopify-like fileCreate user errors without staging invalid file inputs', async () => {
    ...
    const createResponse = await request(app)
      .post('/admin/api/2026-04/graphql.json')
      .send({
        query: 'mutation { fileCreate(files: [{ contentType: IMAGE, originalSource: "not-a-valid-url", alt: "Bad" }]) { files { id } userErrors { field message code } } }',
      });
