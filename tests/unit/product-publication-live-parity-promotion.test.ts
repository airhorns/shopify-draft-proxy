...
describe('product publication live parity promotion state', () => {
  ...
  it('upgrades the publication parity specs to captured mode without keeping the minimal mutation slice blocked', () => {
    for (const expected of expectedCoveredFamilies) {
  ...
  it('records the current publication aggregate-field blocker note alongside the now-captured mutation family', () => {
    ...
    expect(blockerNote).toContain('minimal `productPublish` / `productUnpublish` mutation payloads now capture successfully');
