...
const expectedCapturedScenarios: CapturedScenario[] = [
  ...
  },
  {
    ...
    captureFile: 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/order-edit-add-variant-missing-id.json',
  ...
  },
  {
    ...
    captureFile: 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/order-edit-set-quantity-missing-id.json',
...
describe('order editing scaffolding', () => {
  it('registers the first order-editing roots with explicit blocked status instead of leaving them invisible', () => {
    ...
    for (const expected of expectedOrderEditRegistryEntries) {
      const conformanceExpectation = expected.conformanceStatus === 'covered'
      ...
      expect(registry).toContainEqual(
        expect.objectContaining({
          ...
          conformance: conformanceExpectation,
        }),
      );
    }
  });
  ...
  it('refreshes a dedicated blocker note with the current write_order_edits access-denied evidence', () => {
    ...
    const blockerNote = readFileSync(
      resolve(repoRoot, 'pending/order-editing-conformance-scope-blocker.md'),
      'utf8',
    );

    expect(captureScript).toContain("const orderEditingBlockerNotePath = path.join('pending', 'order-editing-conformance-scope-blocker.md');");
    expect(captureScript).toContain('first local calculated-order edit flow');
    expect(captureScript).not.toContain('only after live write evidence exists should the proxy start staging calculated-order state');
    ...
    expect(blockerNote).not.toContain('only after live write evidence exists should the proxy start staging calculated-order state');
  });
});
