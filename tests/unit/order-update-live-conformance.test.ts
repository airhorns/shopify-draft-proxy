...
describe('orderUpdate live conformance', () => {
    ...
    expect(registry).toContainEqual(
      expect.objectContaining({
        ...
        conformance: expect.objectContaining({
          status: 'covered',
          scenarioIds: [
            'order-update-inline-missing-id-argument-error',
            'order-update-inline-null-id-argument-error',
            'order-update-missing-id-invalid-variable',
            'order-update-unknown-id-parity',
            'order-update-live-parity',
          ],
        }),
      }),
    );
  ...
  it('registers captured parity scenarios and executable request scaffolds for the safe orderUpdate validation slices', () => {
    ...
    const document = readFileSync(
      resolve(repoRoot, 'config/parity-requests/orderUpdate-parity-plan.graphql'),
      'utf8',
    );
    ...
    const inlineMissingIdVariables = JSON.parse(
      readFileSync(resolve(repoRoot, 'config/parity-requests/orderUpdate-inline-missing-id-parity.variables.json'), 'utf8'),
    ) as Record<string, unknown>;
    const inlineNullIdVariables = JSON.parse(
      readFileSync(resolve(repoRoot, 'config/parity-requests/orderUpdate-inline-null-id-parity.variables.json'), 'utf8'),
    ) as Record<string, unknown>;
    const unknownIdFixture = JSON.parse(
      readFileSync(
        resolve(repoRoot, 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/order-update-unknown-id.json'),
        'utf8',
      ),
    ) as Record<string, unknown>;
    const missingIdFixture = JSON.parse(
      readFileSync(
        resolve(repoRoot, 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/order-update-missing-id.json'),
        'utf8',
      ),
    ) as Record<string, unknown>;
    const inlineMissingIdFixture = JSON.parse(
      readFileSync(
        resolve(repoRoot, 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/order-update-inline-missing-id.json'),
        'utf8',
      ),
    ) as Record<string, unknown>;
    const inlineNullIdFixture = JSON.parse(
      readFileSync(
        resolve(repoRoot, 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/order-update-inline-null-id.json'),
        'utf8',
      ),
    ) as Record<string, unknown>;
    ...
    expect(scenarios).toContainEqual(
      expect.objectContaining({
        ...
        status: 'captured',
        captureFiles: ['fixtures/conformance/very-big-test-store.myshopify.com/2025-01/order-update-inline-missing-id.json'],
    ...
    expect(scenarios).toContainEqual(
      expect.objectContaining({
        ...
        operationNames: ['orderUpdate'],
        status: 'captured',
        captureFiles: ['fixtures/conformance/very-big-test-store.myshopify.com/2025-01/order-update-inline-null-id.json'],
    ...
    expect(inlineMissingIdSpec).toEqual(
      expect.objectContaining({
        ...
        liveCaptureFiles: ['fixtures/conformance/very-big-test-store.myshopify.com/2025-01/order-update-inline-missing-id.json'],
    ...
    expect(inlineNullIdSpec).toEqual(
      expect.objectContaining({
        ...
        scenarioStatus: 'captured',
        liveCaptureFiles: ['fixtures/conformance/very-big-test-store.myshopify.com/2025-01/order-update-inline-null-id.json'],
        comparisonMode: 'captured-vs-proxy-request',
        proxyRequest: {
    ...
    expect(unknownIdSpec).toEqual(
      expect.objectContaining({
        ...
        scenarioStatus: 'captured',
        liveCaptureFiles: ['fixtures/conformance/very-big-test-store.myshopify.com/2025-01/order-update-unknown-id.json'],
        comparisonMode: 'captured-vs-proxy-request',
        proxyRequest: {
    ...
    expect(missingIdSpec).toEqual(
      expect.objectContaining({
        ...
        scenarioStatus: 'captured',
        liveCaptureFiles: ['fixtures/conformance/very-big-test-store.myshopify.com/2025-01/order-update-missing-id.json'],
        comparisonMode: 'captured-vs-proxy-request',
        proxyRequest: {
          documentPath: 'config/parity-requests/orderUpdate-parity-plan.graphql',
    ...
    expect(inlineMissingIdDocument).toContain('orderUpdate(input: { note: "order update inline missing-id parity plan"');
    ...
    expect(worklist).toContain('- [x] `orderUpdate`');
    expect(worklist).toContain('inline-literal missing/null id validation');
    expect(worklist).toContain('first happy-path local `orderUpdate` slice for already-known orders, not just freshly synthetic ones');
  ...
  it('captures the first live happy-path orderUpdate slice and keeps the parity/reporting artifacts aligned with the narrow local runtime', () => {
    ...
    const document = readFileSync(
      resolve(repoRoot, 'config/parity-requests/orderUpdate-live-parity.graphql'),
      'utf8',
    );
    ...
    expect(registry).toContainEqual(
      expect.objectContaining({
        ...
        conformance: expect.objectContaining({
          status: 'covered',
          scenarioIds: expect.arrayContaining(['order-update-live-parity']),
        }),
      }),
    );
    expect(scenarios).toContainEqual(
    ...
    expect(worklist).toContain('first live happy-path `orderUpdate` slice');
    expect(weirdNotes).toContain('live `orderUpdate` happy path');
