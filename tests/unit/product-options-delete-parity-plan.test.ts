import { readFileSync, existsSync } from 'node:fs';
import { resolve } from 'node:path';

import { describe, expect, it } from 'vitest';

describe('productOptionsDelete parity plan scaffold', () => {
  it('declares a concrete proxy request scaffold for the staged option-delete slice', () => {
    const repoRoot = resolve(import.meta.dirname, '../..');
    const specPath = resolve(repoRoot, 'config/parity-specs/productOptionsDelete-parity-plan.json');
    const spec = JSON.parse(readFileSync(specPath, 'utf8')) as {
      proxyRequest?: { documentPath?: string | null; variablesPath?: string | null };
    };

    expect(spec.proxyRequest?.documentPath).toBe('config/parity-requests/productOptionsDelete-parity-plan.graphql');
    expect(spec.proxyRequest?.variablesPath).toBe(
      'config/parity-requests/productOptionsDelete-parity-plan.variables.json',
    );

    const documentPath = resolve(repoRoot, spec.proxyRequest!.documentPath!);
    const variablesPath = resolve(repoRoot, spec.proxyRequest!.variablesPath!);

    expect(existsSync(documentPath)).toBe(true);
    expect(existsSync(variablesPath)).toBe(true);

    const document = readFileSync(documentPath, 'utf8');
    const variables = JSON.parse(readFileSync(variablesPath, 'utf8')) as {
      productId?: string;
      options?: string[];
    };

    expect(document).toContain('mutation ProductOptionsDeleteParityPlan($productId: ID!, $options: [ID!]!)');
    expect(document).toContain('productOptionsDelete(productId: $productId, options: $options)');
    expect(document).toContain('deletedOptionsIds');
    expect(document).toContain('product {');
    expect(document).toContain('options {');
    expect(document).toContain('optionValues {');
    expect(document).toContain('userErrors {');

    expect(variables).toEqual({
      productId: 'gid://shopify/Product/100',
      options: ['gid://shopify/ProductOption/200'],
    });
  });
});
