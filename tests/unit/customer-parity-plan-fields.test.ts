...
describe('customer parity request scaffolds', () => {
  it('keep the richer customer detail and catalog field slices aligned with the supported overlay serializer', () => {
    ...
    const advancedSearchDocumentPath = resolve(repoRoot, 'config/parity-requests/customers-advanced-search-read.graphql');
    const advancedSearchVariablesPath = resolve(repoRoot, 'config/parity-requests/customers-advanced-search-read.variables.json');
    ...
    const relevanceVariablesPath = resolve(repoRoot, 'config/parity-requests/customers-relevance-search-read.variables.json');
    ...
    const advancedSearchVariables = JSON.parse(readFileSync(advancedSearchVariablesPath, 'utf8')) as Record<string, unknown>;
    ...
    expect(advancedSearchDocument).toContain('query CustomersAdvancedSearchRead($prefixQuery: String!, $orQuery: String!, $groupedQuery: String!)');
    expect(advancedSearchDocument).toContain('prefix: customers(first: 2, query: $prefixQuery, sortKey: UPDATED_AT, reverse: true)');
    expect(advancedSearchDocument).toContain('orMatches: customers(first: 5, query: $orQuery, sortKey: UPDATED_AT, reverse: true)');
    expect(advancedSearchDocument).toContain('groupedExclusion: customers(first: 5, query: $groupedQuery, sortKey: UPDATED_AT, reverse: true)');
