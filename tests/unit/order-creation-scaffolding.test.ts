...
const expectedCapturedScenarios = [
  {
    ...
    captureFile: 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/order-create-inline-missing-order.json',
  ...
  },
  {
    ...
    captureFile: 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/draft-order-create-inline-missing-input.json',
  ...
  },
  {
    ...
    operationName: 'draftOrderCreate',
    captureFile: 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/draft-order-create-inline-null-input.json',
  ...
  },
  {
    ...
    captureFile: 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/draft-order-complete-inline-missing-id.json',
  ...
  },
  {
    ...
    operationName: 'draftOrderComplete',
    captureFile: 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/draft-order-complete-inline-null-id.json',
...
describe('order creation scaffolding', () => {
  it('adds a dedicated capture script for order-domain conformance discovery', () => {
    ...
    expect(packageJson.scripts?.['conformance:capture-orders']).toBe(
      'node ./scripts/capture-order-conformance.mjs',
    );
  });
      ...
      expect(registry).toContainEqual(
        expect.objectContaining({
          ...
          conformance: expect.objectContaining({
            status: expected.conformanceStatus,
          }),
        }),
      );
    }
  ...
  it('tracks the first captured-vs-blocked order-creation scenarios explicitly instead of leaving them ad hoc', () => {
    const repoRoot = resolve(import.meta.dirname, '../..');
    const scenarios = JSON.parse(readFileSync(resolve(repoRoot, 'config/conformance-scenarios.json'), 'utf8')) as ConformanceScenario[];
  ...
  it('keeps the captured orderCreate missing-order parity artifacts executable', () => {
    ...
    const variables = JSON.parse(readFileSync(resolve(repoRoot, expected.variablesPath), 'utf8')) as Record<string, unknown>;
  ...
  it('keeps the captured orderCreate happy-path parity artifacts executable', () => {
    ...
    expect(variables).toMatchObject({
      order: expect.objectContaining({
        ...
        shippingLines: expect.arrayContaining([
          expect.objectContaining({ title: 'Standard', code: 'STANDARD' }),
        ]),
      }),
    });
  });
  ...
  it('keeps the captured draftOrderCreate inline-argument validation parity artifacts executable', () => {
    ...
    const inlineNullSpec = JSON.parse(readFileSync(resolve(repoRoot, inlineNullExpected.paritySpecPath), 'utf8')) as ParitySpec;
    const inlineNullDocument = readFileSync(resolve(repoRoot, inlineNullExpected.documentPath), 'utf8');
    const inlineNullVariables = JSON.parse(
      readFileSync(resolve(repoRoot, inlineNullExpected.variablesPath), 'utf8'),
    ) as Record<string, unknown>;
  ...
  it('keeps the captured draftOrderComplete validation parity artifacts executable', () => {
    const repoRoot = resolve(import.meta.dirname, '../..');
    const expectedScenarios = expectedCapturedScenarios.filter((scenario) =>
      scenario.operationName === 'draftOrderComplete');
    ...
    for (const expected of expectedScenarios) {
      ...
      if (expected.id === 'draft-order-complete-missing-id-invalid-variable') {
        expect(document).toContain('mutation DraftOrderCompleteMissingIdParity($id: ID!, $paymentGatewayId: ID, $sourceName: String)');
        expect(document).toContain('draftOrderComplete(id: $id, paymentGatewayId: $paymentGatewayId, sourceName: $sourceName)');
        expect(variables).toEqual({
  ...
  it('keeps the live order capture harness aligned with the richer merchant-realistic order-creation parity scaffolds and validation slices', () => {
    ...
    const captureScript = readFileSync(
      resolve(repoRoot, 'scripts/capture-order-conformance.mjs'),
      'utf8',
    );
  ...
  it('refreshes a shared blocker note with the current healthy creation state while preserving the remaining draft-order completion blocker', () => {
    ...
    expect(blockerNote).not.toContain('still lacks any writable happy-path local runtime support for the whole creation family');
  });
  ...
  it('documents the first safe orderCreate validation slice alongside the still-blocked happy path', () => {
    ...
    expect(worklist).toContain('orderCreate');
    expect(worklist).toContain('order-create-inline-missing-order.json');
    expect(worklist).toContain('order-create-inline-null-order.json');
    expect(worklist).toContain('missingRequiredArguments');
    expect(worklist).toContain('argumentLiteralsIncompatible');
    expect(worklist).toContain('required `$order` variable');
    expect(worklist).toContain('order-create-missing-order.json');
    ...
    expect(worklist).toContain('draft-order-create-missing-input.json');
    expect(worklist).toContain('required `$input` variable');
    expect(worklist).toContain('draft-order-complete-missing-id.json');
    expect(worklist).toContain('required `$id` variable');
    expect(worklist).toContain("Field 'order' doesn't exist on type 'DraftOrderCompletePayload'");
