...
describe('productUnpublish parity plan scaffold', () => {
  ...
  it('tracks the aggregate publication-field blocker in a separate captured scenario', () => {
    ...
    expect(spec.proxyRequest?.documentPath).toBe(
      'config/parity-requests/productUnpublish-aggregate-parity-blocker.graphql',
    );
    expect(spec.proxyRequest?.variablesPath).toBe(
      'config/parity-requests/productUnpublish-aggregate-parity-blocker.variables.json',
    );
