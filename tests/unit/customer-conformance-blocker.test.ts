...
describe('customer conformance coverage state', () => {
  it('promotes customer reads to covered with captured live fixtures and reusable proxy requests', () => {
    for (const expected of expectedScenarios) {
      ...
      expect(operation?.conformance).toMatchObject({
        status: 'covered',
      });
      expect(operation?.conformance?.scenarioIds).toEqual(expect.arrayContaining([expected.scenarioId]));
  ...
  it('keeps the dedicated live customer capture script while removing the stale protected-data blocker note after successful capture', () => {
    expect(packageJson.scripts?.['conformance:capture-customers']).toBe('node ./scripts/capture-customer-conformance.mjs');
