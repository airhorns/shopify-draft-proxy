...
describe('order read scaffolding', () => {
  it('tracks the current direct-order empty-state slice plus captured draft-order detail/catalog/count reads explicitly', () => {
    ...
    expect(registry).toContainEqual(
      expect.objectContaining({
        ...
        conformance: expect.objectContaining({
          status: 'covered',
          scenarioIds: ['order-empty-state-read'],
        }),
      }),
    );
    expect(registry).toContainEqual(
    ...
    expect(registry).toContainEqual(
      expect.objectContaining({
        ...
        conformance: expect.objectContaining({
          status: 'covered',
          scenarioIds: ['draft-order-detail-read'],
        }),
      }),
    );
    expect(registry).toContainEqual(
      expect.objectContaining({
        ...
        conformance: expect.objectContaining({
          status: 'covered',
          scenarioIds: ['draft-orders-catalog-read', 'draft-orders-invalid-email-query-read'],
        }),
      }),
    );
    expect(registry).toContainEqual(
      expect.objectContaining({
        ...
        conformance: expect.objectContaining({
          status: 'covered',
          scenarioIds: ['draft-orders-count-read', 'draft-orders-invalid-email-query-read'],
        }),
      }),
    );
  });
  ...
  it('adds conformance scenarios and parity specs for the safe order empty-state slice plus captured draft-order detail/catalog/count reads', () => {
    ...
    expect(scenarios).toContainEqual(
      expect.objectContaining({
        ...
        status: 'captured',
        captureFiles: ['fixtures/conformance/very-big-test-store.myshopify.com/2025-01/draft-orders-invalid-email-query.json'],
  ...
  it('keeps order-read parity artifacts executable after promoting draft-order catalog/count reads to captured parity', () => {
    ...
    const detailDocument = readFileSync(resolve(repoRoot, 'config/parity-requests/draftOrder-read-parity-plan.graphql'), 'utf8');
    ...
    const capturedSpecs = [
      ...
      },
      {
        ...
        liveCaptureFiles: ['fixtures/conformance/very-big-test-store.myshopify.com/2025-01/draft-orders-invalid-email-query.json'],
    ...
    for (const captured of capturedSpecs) {
      ...
      const variables = JSON.parse(readFileSync(resolve(repoRoot, captured.variablesPath), 'utf8')) as Record<string, unknown>;
    ...
    if (existsSync(blockerPath)) {
      ...
      expect(blockerNote).toContain('current run is auth-regressed before the draft-order read roots could be reprobed');
    ...
    expect(captureScript).toContain("const draftOrdersCatalogFixturePath = path.join(fixtureDir, 'draft-orders-catalog.json');");
    expect(captureScript).toContain("const draftOrdersCountFixturePath = path.join(fixtureDir, 'draft-orders-count.json');");
    expect(captureScript).toContain("const draftOrdersInvalidEmailQueryFixturePath = path.join(fixtureDir, 'draft-orders-invalid-email-query.json');");
