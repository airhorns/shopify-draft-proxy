...
describe('inventoryAdjustQuantities live conformance wiring', () => {
  ...
  it('keeps richer live evidence for location names, full app identity, and the staffMember scope blocker', () => {
    ...
    expect(
      fixture.mutation?.response?.data?.inventoryAdjustQuantities?.inventoryAdjustmentGroup?.changes?.[0]?.location,
    ).toMatchObject({
        ...
        data: {
          ...
          firstInventoryItem: {
            inventoryLevels: {
              nodes: [
                {
                  quantities: expect.arrayContaining([expect.objectContaining({ name: 'incoming', quantity: 2 })]),
                },
              ],
            },
          },
