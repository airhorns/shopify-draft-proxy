...
import { extractCliIdentityFromConfig, extractManualStoreAuthTokenSummary, extractScopesFromShopifyAppToml, extractShopifyAppDeployVersion, findConfiguredShopifyApp, findShopifyChannelConfigExtensions, isInvalidGrantRefreshResponse, parsePublicationTargetBlocker, shouldAttemptShopifyAppDeploy, shouldProbeManualStoreAuthFallback } from '../../scripts/product-publication-conformance-lib.mjs';
...
describe('extractManualStoreAuthTokenSummary', () => {
  it('summarizes a saved manual store-auth token with its scope families', () => {
    expect(
      ...
      associatedUserScopeHandles: ['write_products', 'write_inventory', 'write_files', 'write_orders', 'write_customers'],
