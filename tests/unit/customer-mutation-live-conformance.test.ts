...
describe('customer mutation live conformance wiring', () => {
  ...
  it('preserves live validation userErrors alongside the happy-path customer mutation captures', () => {
    const createCapture = JSON.parse(
      readFileSync(resolve(repoRoot, 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/customer-create-parity.json'), 'utf8'),
    ) as CustomerMutationCapture;
    const updateCapture = JSON.parse(
      readFileSync(resolve(repoRoot, 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/customer-update-parity.json'), 'utf8'),
    ) as CustomerMutationCapture;
    const deleteCapture = JSON.parse(
      readFileSync(resolve(repoRoot, 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/customer-delete-parity.json'), 'utf8'),
    ) as CustomerMutationCapture;
