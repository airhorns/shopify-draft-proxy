...
type ProductSortKeysCapture = {
  ...
  response?: {
    data?: {
      ...
      vendorOrder?: {
        edges?: Array<{ node?: { id?: string; title?: string; vendor?: string; productType?: string } }>;
      };
      productTypeOrder?: {
        edges?: Array<{ node?: { id?: string; title?: string; vendor?: string; productType?: string } }>;
      };
...
describe('product sort-key live conformance wiring', () => {
  ...
  it('keeps the proxy request aligned with the live ProductSortKeys subset and preserves the captured aliases', () => {
    const repoRoot = resolve(import.meta.dirname, '../..');
    const document = readFileSync(resolve(repoRoot, 'config/parity-requests/products-sort-keys-read.graphql'), 'utf8');
    const capture = JSON.parse(
      readFileSync(
        resolve(repoRoot, 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/products-sort-keys.json'),
        'utf8',
      ),
    ...
    expect(document).toContain(
      'publishedAtOrder: products(first: $first, query: $query, sortKey: PUBLISHED_AT, reverse: true)',
    );
