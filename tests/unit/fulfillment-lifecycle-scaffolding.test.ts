...
const expectedCapturedScenarios = [
  {
    ...
    requiredText: 'mutation FulfillmentTrackingInfoUpdateInlineMissingId($trackingInfoInput: FulfillmentTrackingInput!, $notifyCustomer: Boolean)',
    expectedVariables: {
      trackingInfoInput: {
    ...
    fixturePath: 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/fulfillment-tracking-info-update-inline-missing-id.json',
  ...
  },
  {
    ...
    requiredText: 'mutation FulfillmentTrackingInfoUpdateInlineNullId($trackingInfoInput: FulfillmentTrackingInput!, $notifyCustomer: Boolean)',
    expectedVariables: {
      trackingInfoInput: {
        number: 'HERMES-TRACK-UPDATE',
        url: 'https://example.com/track/HERMES-TRACK-UPDATE',
    ...
    fixturePath: 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/fulfillment-tracking-info-update-inline-null-id.json',
  ...
  },
  {
    ...
    requiredText: 'mutation FulfillmentTrackingInfoUpdateMissingId($fulfillmentId: ID!, $trackingInfoInput: FulfillmentTrackingInput!, $notifyCustomer: Boolean)',
    expectedVariables: {
      trackingInfoInput: {
        number: 'HERMES-TRACK-UPDATE',
        url: 'https://example.com/track/HERMES-TRACK-UPDATE',
    ...
    fixturePath: 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/fulfillment-tracking-info-update-missing-id.json',
  ...
  },
  {
    ...
    expectedVariables: {},
    fixturePath: 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/fulfillment-cancel-inline-missing-id.json',
  ...
  },
  {
    ...
    expectedVariables: {},
    fixturePath: 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/fulfillment-cancel-inline-null-id.json',
    fixtureMessageIncludes: "invalid value (null). Expected type 'ID!'",
  },
  {
...
const expectedBlockedScenarios = [
  {
    ...
    requiredText: 'mutation FulfillmentTrackingInfoUpdateParityPlan($fulfillmentId: ID!, $trackingInfoInput: FulfillmentTrackingInput!, $notifyCustomer: Boolean)',
...
describe('fulfillment lifecycle scaffolding', () => {
      ...
      expect(registry).toContainEqual(
        expect.objectContaining({
          ...
          conformance: expect.objectContaining({
            ...
            scenarioIds: expected.scenarioIds,
          }),
        }),
      );
    }
  ...
  it('keeps fulfillment lifecycle parity artifacts and blockers machine-readable in parity specs, docs, and blocker notes', () => {
    ...
    for (const expected of expectedCapturedScenarios) {
      ...
      const fixture = JSON.parse(readFileSync(resolve(repoRoot, expected.fixturePath), 'utf8')) as { mutation?: { response?: { errors?: Array<{ message?: string }> } } };
    ...
    expect(worklist).toContain('- [x] `fulfillmentTrackingInfoUpdate`');
    expect(worklist).toContain('- [x] `fulfillmentCancel`');
    expect(worklist).toContain('safe pre-access GraphQL validation coverage is now evidence-backed');
    expect(worklist).toContain('fulfillment-lifecycle-conformance-scope-blocker.md');
