...
describe('inventory linkage parity plan scaffolds', () => {
  it('declares concrete proxy request scaffolds for the captured minimal inventoryActivate, inventoryDeactivate, and inventoryBulkToggleActivation slices', () => {
    ...
    expect(activateSpec.proxyRequest?.documentPath).toBe('config/parity-requests/inventoryActivate-parity-plan.graphql');
    expect(activateSpec.proxyRequest?.variablesPath).toBe('config/parity-requests/inventoryActivate-parity-plan.variables.json');
    expect(deactivateSpec.proxyRequest?.documentPath).toBe('config/parity-requests/inventoryDeactivate-parity-plan.graphql');
    expect(deactivateSpec.proxyRequest?.variablesPath).toBe('config/parity-requests/inventoryDeactivate-parity-plan.variables.json');
    expect(bulkSpec.proxyRequest?.documentPath).toBe('config/parity-requests/inventoryBulkToggleActivation-parity-plan.graphql');
    expect(bulkSpec.proxyRequest?.variablesPath).toBe('config/parity-requests/inventoryBulkToggleActivation-parity-plan.variables.json');
  ...
  it('does not leave the old multi-location blocker scaffold wired in once live success capture exists', () => {
    ...
    expect(existsSync(resolve(repoRoot, 'config/parity-specs/inventory-linkage-multi-location-blocker.json'))).toBe(false);
    expect(existsSync(resolve(repoRoot, 'config/parity-requests/inventory-linkage-multi-location-blocker.graphql'))).toBe(false);
    expect(existsSync(resolve(repoRoot, 'config/parity-requests/inventory-linkage-multi-location-blocker.variables.json'))).toBe(false);
