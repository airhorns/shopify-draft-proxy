  ...
  it('captures live handle de-duplication for duplicate and productSet create collisions', () => {
    const duplicateCapture = JSON.parse(
      readFileSync(
        resolve(
          repoRoot,
          'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/product-duplicate-parity.json',
        ),
        'utf8',
      ),
    ...
    const setCapture = JSON.parse(
      readFileSync(
        resolve(repoRoot, 'fixtures/conformance/very-big-test-store.myshopify.com/2025-01/product-set-parity.json'),
        'utf8',
      ),
    ) as ProductGraphCapture;
