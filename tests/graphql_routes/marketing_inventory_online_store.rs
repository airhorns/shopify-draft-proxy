...
fn media_file_lifecycle_stages_uploaded_reads_and_empty_product_media_after_delete() {
    ...
    let product = proxy.process_request(json_graphql_request(
        r#"
        mutation FileReferenceProductCreate($product: ProductCreateInput!) {
        ...
        "#,
        json!({"product": {"title": "File reference target"}}),
    ));
    assert_eq!(
    ...
    assert_eq!(
        ...
        json!({"id": product_id, "title": "File reference target", "media": {"nodes": [], "pageInfo": {"hasNextPage": false, "hasPreviousPage": false, "startCursor": null, "endCursor": null}}})
    );
    ...
    assert_eq!(
        ...
        json!({"id": product_id, "media": {"nodes": []}})
    );
}
