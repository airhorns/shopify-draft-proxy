...
fn assert_engine_resolver_failure(response: &Response, root: &str, message: &str) {
    assert_eq!(response.body["errors"][0]["message"], json!(message));
    assert_eq!(response.body["errors"][0]["path"], json!([root]));
    assert!(
        response.body["data"].is_null() || response.body["data"][root].is_null(),
        "failed resolver `{root}` should be null in the engine response: {}",
        response.body
    );
}
...
fn create_variant_for_connection_test(
    proxy: &mut DraftProxy,
    product_id: &str,
    title: &str,
    sku: &str,
    ...
    let location_id =
        staged_fulfillment_service_location_id(proxy, &format!("{sku} inventory location"));
    let response = proxy.process_request(json_graphql_request(
        r#"
        mutation CreateVariantForConnectionTest(
          $productId: ID!
        ...
        json!({
            ...
            "variants": [{
                ...
                "inventoryItem": { "sku": sku, "tracked": tracked }
            }]
        }),
    ));
    assert_eq!(response.status, 200);
    ...
    response.body["data"]["productVariantsBulkCreate"]["productVariants"][0]["id"]
        .as_str()
...
#[test]
fn product_create_live_hybrid_uses_upstream_taxonomy_category_details() {
    let category_id = "gid://shopify/TaxonomyCategory/ap-2-7";
    let upstream_calls = Arc::new(Mutex::new(Vec::new()));
    let calls = Arc::clone(&upstream_calls);
    let mut proxy = configured_proxy(ReadMode::LiveHybrid, None).with_upstream_transport(
        move |request| {
            ...
            assert_eq!(body["variables"]["id"], json!(category_id));
            assert!(
                body["query"]
                    .as_str()
                    .unwrap_or_default()
                    .contains("... on TaxonomyCategory")
            );
            Response {
                status: 200,
                headers: Default::default(),
    ...
    let create = proxy.process_request(json_graphql_request(
        r#"
        mutation HydratedCategoryCreate($product: ProductCreateInput!) {
          productCreate(product: $product) {
            product { id category { id fullName name isLeaf level parentId } }
            userErrors { field message }
          }
        ...
        json!({
            "product": {
                "title": "Hydrated category create",
                "category": category_id
            }
        }),
    ));
    ...
    let read = proxy.process_request(json_graphql_request(
        r#"
        query HydratedCategoryCreateRead($id: ID!) {
          product(id: $id) {
...
fn product_variants_connection_honors_sort_keys_and_reverse() {
    ...
    let mut proxy = snapshot_proxy().with_base_products(vec![seed_product(product_id)]);
    let zulu_id = create_variant_for_connection_test(
        &mut proxy, product_id, "Zulu", "SKU-C", "CONTINUE", 10, true,
    );
    let alpha_id = create_variant_for_connection_test(
        &mut proxy, product_id, "Alpha", "SKU-A", "DENY", 30, false,
    );
    let middle_id = create_variant_for_connection_test(
...
fn product_and_variant_media_connections_return_windowed_edges_and_page_info() {
    ...
    let variant = create_legacy_variant(&mut proxy, product_id, "MEDIA-VARIANT", "10.00");
...
fn product_variants_bulk_reorder_rejects_invalid_inputs_atomically() {
    ...
    assert!(missing_variant.body.get("data").is_none());
    assert_eq!(
        missing_variant.body["errors"][0]["extensions"]["code"],
        json!("INVALID_VARIABLE")
    );

    let unknown_variant = proxy.process_request(json_graphql_request(
        r#"
        mutation UnknownVariantPosition($productId: ID!, $positions: [ProductVariantPositionInput!]!) {
          productVariantsBulkReorder(productId: $productId, positions: $positions) {
            product { id }
            userErrors { field message code }
          }
    ...
    assert_eq!(
        unknown_variant.body["data"]["productVariantsBulkReorder"]["product"],
        Value::Null
    );
    assert_user_error_with_field_and_code(
        &unknown_variant.body["data"]["productVariantsBulkReorder"]["userErrors"],
        json!(["positions", "0", "id"]),
        "MISSING_VARIANT",
    );
    assert_eq!(
...
#[test]
fn product_variants_bulk_update_payload_overlays_staged_variant_metafields() {
    let product_id = "gid://shopify/Product/bulk-payload-metafield";
    let mut proxy = snapshot_proxy().with_base_products(vec![seed_product(product_id)]);
    let variant = create_legacy_variant(&mut proxy, product_id, "META", "10.00");
    let variant_id = variant["id"].as_str().unwrap().to_string();

    let set = proxy.process_request(json_graphql_request(
        r#"
        mutation SetBulkPayloadVariantMetafield($metafields: [MetafieldsSetInput!]!) {
    ...
    let update = proxy.process_request(json_graphql_request(
        r#"
        mutation ReadVariantMetafieldFromBulkPayload(
          $productId: ID!
          $variants: [ProductVariantsBulkInput!]!
        ) {
          productVariantsBulkUpdate(productId: $productId, variants: $variants) {
            productVariants {
              id
              price
              marker: metafield(namespace: "custom", key: "bulk_payload") { value }
            }
            userErrors { field message code }
          }
        }
        ...
        json!({
            "productId": product_id,
            "variants": [{ "id": variant_id, "price": "12.00" }]
        }),
    ));

    assert_eq!(update.status, 200);
    assert_eq!(
        update.body["data"]["productVariantsBulkUpdate"],
        json!({
            "productVariants": [{
                "id": variant_id,
                "price": "12.00",
                "marker": { "value": "staged value" }
            }],
            "userErrors": []
        })
    );
...
fn product_media_roots_without_store_backed_handlers_fail_closed() {
    ...
    for (root, query) in [
        ...
        assert_engine_resolver_failure(
            &response,
            root,
            &format!("No mutation dispatcher implemented for root field: {root}"),
        );
    }
...
fn assert_publication_create_uses_allocator_without_materializing_default_publication() {
    let mut proxy = snapshot_proxy();

    let create = proxy.process_request(json_graphql_request(
        r#"
        mutation PublicationCreateAllocatorRegression($input: PublicationCreateInput!) {
    ...
    assert_eq!(create.status, 200);
    assert_eq!(
        create.body["data"]["publicationCreate"]["userErrors"],
        json!([])
    );
    let created_id = create.body["data"]["publicationCreate"]["publication"]["id"]
        .as_str()
        .expect("publicationCreate should return an id");
    assert!(created_id.starts_with("gid://shopify/Publication/"));
    assert!(
        created_id.contains("shopify-draft-proxy=synthetic"),
        "publicationCreate should allocate from the instance synthetic allocator, got {created_id}"
    );
    assert_ne!(created_id, "gid://shopify/Publication/1");

    let state = state_snapshot(&proxy);
    let publications = state["stagedState"]["publications"]
        .as_object()
        .expect("staged publications should be an object");
    assert_eq!(publications.len(), 1);
    assert!(publications.contains_key(created_id));
    assert!(!publications.contains_key("gid://shopify/Publication/1"));

    let channels = proxy.process_request(json_graphql_request(
        r#"
        query PublicationCreateChannelsRegression($id: ID!) {
          channels(first: 10) {
            nodes { id name }
          }
          publication(id: $id) {
    ...
    assert_eq!(
        channels.body["data"]["channels"]["nodes"]
            .as_array()
            .unwrap_or_else(|| panic!("channels nodes should be an array: {}", channels.body))
            .len(),
        1
    );
    assert_eq!(
        channels.body["data"]["publication"]["id"],
        json!(created_id)
    );
    let channel_id = channels.body["data"]["channels"]["nodes"][0]["id"]
        .as_str()
        .expect("created publication channel should have an id");
    assert!(channel_id.starts_with("gid://shopify/Channel/"));
    assert!(channel_id.contains("shopify-draft-proxy=synthetic"));
    assert_ne!(channel_id, "gid://shopify/Channel/1");
    assert_eq!(
        channels.body["data"]["publicationsCount"],
        json!({ "count": 1, "precision": "EXACT" })
    );

    let delete_plain_one = proxy.process_request(json_graphql_request(
        r#"
        mutation PublicationDeletePlainOneRegression {
          publicationDelete(id: "gid://shopify/Publication/1") {
            deletedId
            userErrors { field message code }
          }
        }
    ...
    assert_eq!(delete_plain_one.status, 200);
    assert_eq!(
        delete_plain_one.body["data"]["publicationDelete"],
        json!({
...
fn assert_current_publication_reads_require_observed_current_publication() {
    let current_publication_id = "gid://shopify/Publication/268039389490";
    let current_product_id = "gid://shopify/Product/current-publication-non-one";
    let mut current_product = ProductRecord {
        id: current_product_id.to_string(),
        title: "Observed current publication product".to_string(),
        handle: "observed-current-publication-product".to_string(),
        status: "ACTIVE".to_string(),
        ..ProductRecord::default()
    };
    current_product.extra_fields.insert(
        "productPublications".to_string(),
        json!([{ "publicationId": current_publication_id }]),
    );
    let mut current_proxy = snapshot_proxy().with_base_products(vec![current_product]);
    restore_state_with(&mut current_proxy, |state| {
        state["baseState"]["publicationIds"] = json!([current_publication_id]);
        state["baseState"]["publicationCount"] = json!(5);
    ...
    let current_read = current_proxy.process_request(json_graphql_request(
        r#"
        query ObservedCurrentPublicationRead($id: ID!) {
          product(id: $id) { id publishedOnCurrentPublication }
          node(id: $id) { ... on Product { id publishedOnCurrentPublication } }
        }
        "#,
        json!({ "id": current_product_id }),
    ));
    assert_eq!(current_read.status, 200);
    assert_eq!(
        current_read.body["data"],
        json!({
            "product": {
    ...
    let default_publication_product_id = "gid://shopify/Product/null-current-default-fallback";
    let mut default_publication_product = ProductRecord {
        id: default_publication_product_id.to_string(),
        title: "Null current default fallback product".to_string(),
        handle: "null-current-default-fallback-product".to_string(),
        status: "ACTIVE".to_string(),
        ..ProductRecord::default()
    };
    default_publication_product.extra_fields.insert(
        "productPublications".to_string(),
        json!([{ "publicationId": "gid://shopify/Publication/1" }]),
    );
    let mut null_current_proxy =
        snapshot_proxy().with_base_products(vec![default_publication_product]);
    restore_state_with(&mut null_current_proxy, |state| {
        state["baseState"]["publicationIds"] = json!(["gid://shopify/Publication/1"]);
        state["baseState"]["publicationCount"] = json!(1);
        state["stagedState"]["currentChannelPublicationId"] = Value::Null;
        state["stagedState"]["currentChannelPublicationResolved"] = json!(true);
    });

    let null_current_read = null_current_proxy.process_request(json_graphql_request(
        r#"
        query NullCurrentDoesNotFallbackToDefault($id: ID!) {
          product(id: $id) { id publishedOnCurrentPublication }
        }
        "#,
        json!({ "id": default_publication_product_id }),
    ));
    assert_eq!(null_current_read.status, 200);
    assert_eq!(
        null_current_read.body["data"]["product"],
        json!({
            "id": default_publication_product_id,
            "publishedOnCurrentPublication": false
        })
    );

    let no_online_store_product_id = "gid://shopify/Product/no-online-store-publication";
    let mut no_online_store_product = ProductRecord {
        id: no_online_store_product_id.to_string(),
        title: "No Online Store publication product".to_string(),
        handle: "no-online-store-publication-product".to_string(),
        status: "ACTIVE".to_string(),
        ..ProductRecord::default()
    };
    no_online_store_product.extra_fields.insert(
        "productPublications".to_string(),
        json!([{ "publicationId": "gid://shopify/Publication/268039455026" }]),
    );
    let mut no_online_store_proxy =
        snapshot_proxy().with_base_products(vec![no_online_store_product]);
    restore_state_with(&mut no_online_store_proxy, |state| {
        state["baseState"]["publicationIds"] = json!([]);
        state["baseState"]["publicationCount"] = json!(0);
    });
    let no_online_store_read = no_online_store_proxy.process_request(json_graphql_request(
        r#"
        query NoOnlineStoreCurrentPublication($id: ID!) {
          product(id: $id) { id publishedOnCurrentPublication }
        }
        "#,
        json!({ "id": no_online_store_product_id }),
    ));
    assert_eq!(no_online_store_read.status, 200);
    assert_eq!(
        no_online_store_read.body["data"]["product"],
        json!({
            "id": no_online_store_product_id,
            "publishedOnCurrentPublication": false
        })
    );
}
...
fn product_publication_full_sync_and_feedback_tail_helpers_cover_current_behavior() {
    ...
    let created_publication_id = publication_validation.body["data"]["omittedCatalog"]
        ["publication"]["id"]
        .as_str()
        .expect("publicationCreate should return an id")
        .to_string();
    assert!(created_publication_id.contains("shopify-draft-proxy=synthetic"));
    assert_eq!(
        publication_validation.body["data"]["omittedCatalog"]["publication"]["name"],
        json!("Publication 1")
    );
    assert_eq!(
        publication_validation.body["data"]["omittedCatalog"]["publication"]["autoPublish"],
        json!(false)
    );
    assert_eq!(
        publication_validation.body["data"]["omittedCatalog"]["userErrors"],
        json!([])
    );
    assert_eq!(
    ...
    let publication_update_delete = proxy.process_request(json_graphql_request(
        r#"
        mutation RustProductPublicationUpdateDeleteValidation($createdPublicationId: ID!) {
          ...
          deletePlainOne: publicationDelete(id: "gid://shopify/Publication/1") {
            deletedId
            userErrors { field message code }
          }
          deleteCreated: publicationDelete(id: $createdPublicationId) {
            deletedId
            userErrors { field message code }
          }
        }
        "#,
        json!({ "createdPublicationId": created_publication_id.clone() }),
    ));
    ...
    assert_eq!(
        publication_update_delete.body["data"]["deletePlainOne"],
        json!({
            "deletedId": Value::Null,
            "userErrors": [{
                "field": ["id"],
    ...
    assert_eq!(
        ...
        json!({
            "deletedId": created_publication_id.clone(),
            "userErrors": []
        })
    );
    ...
    let staged_non_feed = proxy.process_request(json_graphql_request(
        ...
        "#,
        json!({ "id": created_publication_id }),
    ));
    ...
    let absent_feed = proxy.process_request(json_graphql_request(
        r#"
        mutation RustProductFullSyncAbsentFeed($id: ID!) {
          productFullSync(id: $id) { id userErrors { field message code } }
        }
        "#,
        json!({ "id": "gid://shopify/ProductFeed/US-EN" }),
    ));
    assert_eq!(absent_feed.status, 200);
    assert_eq!(
        absent_feed.body["data"]["productFullSync"],
        json!({
            "id": Value::Null,
            "userErrors": [{
                "field": ["id"],
        ...
        entries
            ...
            .any(|entry| entry["status"] == json!("staged")
                && entry["stagedResourceIds"].as_array().is_some_and(|ids| {
                    ids == &[json!("gid://shopify/ProductFeed/CA-FR")]
                })),
        "successful full sync should record the ProductFeed ID without schema-absent operation or Job resources: {log}"
    );
}
...
fn publication_update_stages_publishables_and_validates_real_input_contract() {
    ...
    let product_read = proxy.process_request(json_graphql_request(
        ...
        "#,
        json!({ "id": product_id, "publicationId": publication_id.clone() }),
    ));
    assert_eq!(
...
fn product_reorder_media_without_store_backed_handler_fails_closed() {
    ...
    assert_engine_resolver_failure(
        &mutation,
        "productReorderMedia",
        "No mutation dispatcher implemented for root field: productReorderMedia",
    );
...
fn product_create_and_delete_media_without_store_backed_handlers_fail_closed() {
    ...
    assert_engine_resolver_failure(
        &delete,
        "productDeleteMedia",
        "No mutation dispatcher implemented for root field: productDeleteMedia",
    );
...
fn product_update_media_without_store_backed_handler_fails_closed() {
    ...
    assert_engine_resolver_failure(
        &mutation,
        "productUpdateMedia",
        "No mutation dispatcher implemented for root field: productUpdateMedia",
    );
...
fn product_publish_live_hybrid_stages_seeded_product_without_upstream_write() {
    ...
    let publish = proxy.process_request(json_graphql_request(
        r#"
        mutation ProductPublishNoRuntimeWrite($input: ProductPublishInput!, $publicationId: ID!) {
          productPublish(input: $input) { product { id publishedOnPublication(publicationId: $publicationId) } userErrors { field message } }
        }
        "#,
        json!({
            "input": { "id": product_id, "productPublications": [{ "publicationId": publication_id }] },
            "publicationId": publication_id
        }),
    ));
...
fn product_publishable_mutations_return_captured_aggregate_shape() {
    ...
    let local_publication_id = create_publication.body["data"]["publicationCreate"]["publication"]
        ["id"]
        .as_str()
        .expect("publicationCreate should return an id")
        .to_string();
    assert_eq!(
        ...
        json!({
            "publication": { "id": local_publication_id.clone() },
            "userErrors": []
        })
    );
    ...
    assert_eq!(
        ...
        json!([local_publication_id.clone()])
    );
    assert_eq!(
    ...
    let staged_count = proxy.process_request(json_graphql_request(
        ...
        json!({
            "id": product_id,
            "input": [{ "publicationId": local_publication_id }]
        }),
    ));
    assert_eq!(
    ...
    assert_eq!(
        ...
        json!({
            "publishable": {
                ...
                "resourcePublications": {
                    "nodes": [{
                        "publication": { "id": current_publication_id },
                        "isPublished": true,
                        "publishable": { "id": product_id }
                    }]
                }
...
fn product_read_resolves_published_on_current_publication_from_current_channel() {
    assert_current_publication_reads_require_observed_current_publication();
...
fn publishable_mutations_validate_publication_input_locally() {
    ...
    for (query, root) in [
        ...
        assert_eq!(empty_string.body["data"][root], Value::Null);
        assert_eq!(
    ...
    "#
        ...
        assert_eq!(response.status, 200);
        assert_eq!(
            response.body["data"]["customerSegmentMembersQueryCreate"],
            Value::Null
        );
        assert_eq!(
            response.body["errors"][0]["extensions"]["code"],
...
fn products_connection_and_count_filter_common_search_fields_from_store_state() {
    ...
    let update_variant = proxy.process_request(json_graphql_request(
        r#"
        mutation ProductCommonSearchFilterBarcode(
          $productId: ID!
          $variants: [ProductVariantsBulkInput!]!
        ) {
          productVariantsBulkUpdate(productId: $productId, variants: $variants) {
        ...
        "#,
        json!({
            "productId": "gid://shopify/Product/alpha",
            "variants": [{
                "id": variant_id,
...
fn product_mutations_preserve_root_alias_response_keys() {
    ...
    let delete = proxy.process_request(graphql_request(
        "POST",
        r#"{"query":"mutation { deleteResult: productDelete(input: { id: \"gid://shopify/Product/1\" }) { deletedProductId userErrors { field message  } } }"}"#,
    ));
    assert_eq!(delete.status, 200);
    assert_eq!(
        delete.body,
...
fn saved_search_roots_support_defaults_filtering_pagination_edges_and_aliases() {
    ...
    let response = proxy.process_request(graphql_request(
        "POST",
        r#"{"query":"query { ord: orderSavedSearches(first: 2) { nodes { id name } edges { cursor node { id } } pageInfo { hasNextPage hasPreviousPage startCursor endCursor } } draftOrderSavedSearches(first: 1) { nodes { name resourceType } } productSavedSearches(first: 2) { nodes { id } edges { cursor node { id } } pageInfo { hasNextPage hasPreviousPage startCursor endCursor } } }"}"#,
    ));
...
fn saved_search_live_hybrid_overlay_windows_a_complete_opaque_baseline() {
    ...
    let upstream_first = json!({
        "id": "gid://shopify/SavedSearch/2001",
        "name": "Upstream first",
        "query": "vendor:Alpha",
        "resourceType": "PRODUCT"
    });
    let upstream_second = json!({
        "id": "gid://shopify/SavedSearch/2002",
    ...
    let create = proxy.process_request(json_graphql_request(
        r#"
        mutation CreateOverlaySavedSearch($input: SavedSearchCreateInput!) {
    ...
    let local_id = create.body["data"]["savedSearchCreate"]["savedSearch"]["id"]
        .as_str()
        .expect("local saved search id")
        .to_string();

    let before_second = proxy.process_request(json_graphql_request(
        r#"
        query SavedSearchBeforeOpaque($before: String!) {
        ...
        "#,
        json!({"before": "opaque-saved-search-two"}),
    ));
    assert_eq!(
        before_second.body["data"]["productSavedSearches"],
        json!({
            "nodes": [{"id": "gid://shopify/SavedSearch/2001", "name": "Upstream first"}],
            "edges": [{
                "cursor": "opaque-saved-search-one",
                "node": {"id": "gid://shopify/SavedSearch/2001", "name": "Upstream first"}
            }],
            "pageInfo": {
                "hasNextPage": true,
                "hasPreviousPage": false,
    ...
    let reverse_first = proxy.process_request(json_graphql_request(
        r#"
        query SavedSearchReverseOverlay {
    ...
    assert_eq!(
        reverse_first.body["data"]["productSavedSearches"]["nodes"],
        json!([{"id": local_id, "name": "Local third"}])
    );
    assert_eq!(
        reverse_first.body["data"]["productSavedSearches"]["pageInfo"]["hasNextPage"],
        json!(true)
    );

    let delete = proxy.process_request(json_graphql_request(
        r#"
        mutation DeleteAuthoritativeSavedSearch($input: SavedSearchDeleteInput!) {
        ...
        "#,
        json!({"input": {"id": "gid://shopify/SavedSearch/2002"}}),
    ));
    assert_eq!(
        delete.body["data"]["savedSearchDelete"]["deletedSavedSearchId"],
        json!("gid://shopify/SavedSearch/2002")
    );

    let after_first = proxy.process_request(json_graphql_request(
        r#"
        query SavedSearchAfterOpaque($after: String!) {
        ...
        "#,
        json!({"after": "opaque-saved-search-one"}),
    ));
    assert_eq!(
        after_first.body["data"]["productSavedSearches"]["nodes"],
        json!([{"id": local_id, "name": "Local third"}])
    );
    assert!(
        upstream_requests
...
fn saved_search_blank_name_and_input_required_user_errors_are_schema_shaped_and_aggregated() {
    ...
    assert!(null_inputs.body.get("data").is_none());
    assert_eq!(null_inputs.body["errors"].as_array().unwrap().len(), 2);
}
...
fn saved_search_required_input_omissions_return_top_level_graphql_errors() {
    ...
    let missing_name = proxy.process_request(json_graphql_request(
        ...
        mutation SavedSearchCreateMissingName {
          savedSearchCreate(input: { resourceType: PRODUCT, query: "tag:missing-name" }) {
            savedSearch { id name query resourceType }
            userErrors { field message }
          }
        }
...
fn registry_classification_without_matching_root_field_fails_closed() {
    ...
    let response = proxy.process_request(graphql_request(
        "POST",
        r#"{"query":"mutation { productVariantsBulkCreate(productId: \"gid://shopify/Product/1\", variants: []) { productVariants { id } userErrors { message } } }"}"#,
    ));

    assert_eq!(response.status, 400);
    assert_eq!(
        response.body,
        json!({ "errors": [{ "message": "No mutation dispatcher implemented for root field: productVariantsBulkCreate" }] })
    );
    assert_eq!(log_snapshot(&proxy), json!({ "entries": [] }));
}
...
fn collection_delete_payload_includes_shop_on_user_error() {
    let mut proxy = snapshot_proxy();
    let shop = restore_product_payload_shop(&mut proxy);
