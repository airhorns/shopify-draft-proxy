...
- When invoking this escape hatch:
  ...
  - add Linear reason labels before or while moving the issue to `Blocked on Human`:
    ...
    - add `broken environment` when the blocker is local environment setup or system software, such as the wrong Node, Rust, package-manager, or host runtime version,
