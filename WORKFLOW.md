...
hooks:
  after_create: |
    ...
    shared_cargo_target=/home/airhorns/.cache/shopify-draft-proxy/cargo-target
    mkdir -p "$shared_cargo_target" .cargo
    cat > .cargo/config.toml <<EOF
    # Generated by the Symphony workspace hook. Keep Rust artifacts out of
    # per-issue worktrees and share them across shopify-draft-proxy workspaces.
    [build]
    target-dir = "$shared_cargo_target"
    EOF
    printf '\n# Symphony-generated local Cargo cache config\n.cargo/config.toml\n' >> .git/info/exclude
...
- Symphony-generated workspaces write an untracked `.cargo/config.toml` that points Cargo at the host-wide shared target directory `/home/airhorns/.cache/shopify-draft-proxy/cargo-target`. Do not run bare `cargo clean` from these workspaces unless you intend to wipe the shared Rust cache for every active shopify-draft-proxy issue.
