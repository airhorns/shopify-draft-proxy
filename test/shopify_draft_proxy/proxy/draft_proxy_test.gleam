...
pub fn graphql_saved_search_create_projects_range_and_exists_filters_test() {
  ...
  let request =
    graphql_request(
      "{\"query\":\"mutation { savedSearchCreate(input: { name: \\\"Advanced filters\\\", query: \\\"inventory_total:<10 inventory_total:>2 sku:* -inventory_total:<3\\\", resourceType: PRODUCT }) { savedSearch { query searchTerms filters { __typename key value } } userErrors { field message } } }\"}",
    )
  let #(Response(status: status, body: body, ..), proxy) =
    draft_proxy.process_request(proxy, request)
  assert status == 200
  assert json.to_string(body)
    == "{\"data\":{\"savedSearchCreate\":{\"savedSearch\":{\"query\":\"inventory_total:<10 inventory_total:>2 sku:* -inventory_total:<3\",\"searchTerms\":\"\",\"filters\":[{\"__typename\":\"SearchFilter\",\"key\":\"inventory_total_max\",\"value\":\"10\"},{\"__typename\":\"SearchFilter\",\"key\":\"inventory_total_min\",\"value\":\"3\"},{\"__typename\":\"SearchFilter\",\"key\":\"sku\",\"value\":\"true\"}]},\"userErrors\":[]}}}"

  let #(Response(body: read_body, ..), _) =
    draft_proxy.process_request(
      proxy,
      graphql_request(
        "{\"query\":\"{ productSavedSearches(first: 1, reverse: true) { nodes { name query searchTerms filters { __typename key value } } } }\"}",
      ),
    )
  assert json.to_string(read_body)
    == "{\"data\":{\"productSavedSearches\":{\"nodes\":[{\"name\":\"Advanced filters\",\"query\":\"inventory_total:<10 inventory_total:>=3 sku:*\",\"searchTerms\":\"\",\"filters\":[{\"__typename\":\"SearchFilter\",\"key\":\"inventory_total_max\",\"value\":\"10\"},{\"__typename\":\"SearchFilter\",\"key\":\"inventory_total_min\",\"value\":\"3\"},{\"__typename\":\"SearchFilter\",\"key\":\"sku\",\"value\":\"true\"}]}]}}}"
}
