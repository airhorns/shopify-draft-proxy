...
import shopify_draft_proxy/proxy/proxy_state.{
  Config, Request, Response, Snapshot,
}
...
pub fn product_update_blank_title_returns_existing_product_test() {
  ...
  assert status == 200
  assert json.to_string(body)
    == "{\"data\":{\"productUpdate\":{\"product\":{\"id\":\"gid://shopify/Product/optioned\",\"title\":\"Optioned Board\",\"handle\":\"optioned-board\"},\"userErrors\":[{\"field\":[\"title\"],\"message\":\"Title can't be blank\",\"code\":null}]}}}"
...
pub fn product_create_validation_branches_return_user_errors_test() {
  ...
  assert json.to_string(blank_body)
    == "{\"data\":{\"productCreate\":{\"product\":null,\"userErrors\":[{\"field\":[\"title\"],\"message\":\"Title can't be blank\",\"code\":null}]}}}"
...
pub fn product_create_legacy_input_validation_uses_input_field_path_test() {
  ...
  assert status == 200
  assert json.to_string(body)
    == "{\"data\":{\"productCreate\":{\"product\":null,\"userErrors\":[{\"field\":[\"input\",\"title\"],\"message\":\"Title can't be blank\",\"code\":null}]}}}"
  let assert [entry] = store.get_log(next_proxy.store)
