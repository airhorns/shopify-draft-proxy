...
- If review or acceptance criteria asks for Shopify behavior evidence, use the relevant capture/recording workflow against a disposable Shopify shop; do not substitute local-runtime/proxy-generated output unless the scenario is explicitly documenting proxy-only mechanics or a live-access blocker.
- When touching parity recordings, keep the detailed procedure in the parity/conformance docs and skills, but make the PR itself obvious about which checked-in capture script produced the fixture and which strict target proves the behavior.
