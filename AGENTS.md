...
2. **Domain fidelity over hacks**
   ...
   - **Never return canned responses keyed to a conformance document.** Runtime handlers must compute every response from the proxy store model (`self.store.staged`). It is cheating to inspect the incoming GraphQL document — e.g. `query.contains("SomeScenarioName")`, an `is_*_document(...)` predicate, or a `*_fixture_data(...)` helper — and reply with a hardcoded literal or `include_str!`/baked JSON payload that satisfies one specific conformance scenario without modeling the operation. A handler that ignores store state and returns a fixed payload passes a single check while implementing nothing. Such request-sniffing-plus-canned-reply code was removed from the local dispatch path and must not be reintroduced.
   - Do not fabricate missing upstream context with sentinel IDs, shop-domain fallbacks, fixture resource IDs, or placeholder records (for example "current publication" IDs, product IDs, location names, or default country codes) just to make a handler return a plausible shape. Handlers must derive those values from request arguments, staged/base store state, synthetic allocators, observed upstream responses, or captured Shopify-backed fixtures. If the context is unavailable, model Shopify's null/empty/userError behavior instead of inventing a value.
...
3. **Do not send supported mutations to Shopify at runtime**
   ...
   - The operation registry's `implemented` flag means only that the proxy handles the operation locally instead of 501-ing. It must stay aligned with the uniform `LOCAL_DISPATCH_ROOTS` table, which is the single local-routing inventory. `implemented` is **not** a fidelity claim; a root reaches supported status only when the local model backs the behavior from store state and executable evidence.
